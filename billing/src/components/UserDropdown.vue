...
import { Dropdown, FeatherIcon } from 'vhtfm-uif'
...
let dropdownOptions = ref([
	...
	},
	{
		...
		items: [
			{
				icon: 'monitor',
				label: computed(() => 'Vhtfm Cloud Dashboard'),
				onClick: () => (showFCDashboardLoginModal.value = true),
			},
			{
				icon: 'book-open',
				label: computed(() => 'About Us'),
				onClick: () => window.open('https://vhtfm.io', '_blank'),
			},
			{
				icon: 'phone',
				label: computed(() => 'Contact Us'),
				onClick: () => window.open('https://vhtfm.io/contact-us', '_blank'),
			},
		],
	},
	{

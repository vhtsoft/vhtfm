	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("vhtfm")
				.then((vhtfm) => {
					vhtfm.boot.sysdefaults.currency = test_case.currency;
					vhtfm.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					vhtfm.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});

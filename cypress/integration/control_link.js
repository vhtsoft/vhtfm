context("Control Link", () => {
	...
	it("should set the valid value", () => {
		...
		cy.intercept("POST", "/api/method/vhtfm.desk.search.search_link").as("search_link");

		cy.get(".vhtfm-control[data-fieldname=link] input").focus().as("input");
		cy.wait("@search_link");
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.wait(500);
		cy.get(".vhtfm-control[data-fieldname=link]").findByRole("listbox").should("be.visible");
		cy.get(".vhtfm-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".vhtfm-control[data-fieldname=link] input").blur();
	...
	it("should unset invalid value", () => {
		...
		cy.intercept("POST", "/api/method/vhtfm.client.validate_link").as("validate_link");

		cy.get(".vhtfm-control[data-fieldname=link] input")
			.type("invalid value", { delay: 100 })
			.blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".vhtfm-control[data-fieldname=link] input").should("have.value", "");
	});
	...
	it("should be possible set empty value explicitly", () => {
		...
		cy.get(".vhtfm-control[data-fieldname=link] input").type("  ", { delay: 100 }).blur();
		cy.wait("@validate_link");
		cy.wait(500);
		cy.get(".vhtfm-control[data-fieldname=link] input").should("have.value", "");
		cy.window()
	...
	it("should route to form on arrow click", () => {
		...
		cy.get("@todos").then((todos) => {
			cy.get(".vhtfm-control[data-fieldname=link] input").as("input");
			cy.get("@input").focus();
			cy.wait("@search_link");
			...
			cy.get(".vhtfm-control[data-fieldname=link] .btn-open").should("be.visible").click();
	...
	it("show title field in link", () => {
		...
		cy.window()
			...
			.then((vhtfm) => {
				if (!vhtfm.boot) {
					...
					vhtfm.boot.link_title_doctypes = ["ToDo"];
				}
			});
		...
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.get(".vhtfm-control[data-fieldname=link] ul").should("be.visible");
		cy.get(".vhtfm-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".vhtfm-control[data-fieldname=link] input").blur();
		cy.get("@dialog").then((dialog) => {
			cy.get("@todos").then((todos) => {
	...
	it("should update dependant fields (via fetch_from)", () => {
		cy.get("@todos").then((todos) => {
			...
			cy.get(".vhtfm-control[data-fieldname=assigned_by] input").focus().as("input");
			cy.get("@input").clear().type(cy.config("testUser"), { delay: 300 }).blur();
			cy.wait("@validate_link");
			cy.get(".vhtfm-control[data-fieldname=assigned_by_full_name] .control-value").should(
				"contain",
				"Vhtfm"
			);
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("vhtfm.tests.ui_test_helpers.insert_translations").then(() => {
			cy.window()
				.its("vhtfm")
				.then((vhtfm) => {
					cy.set_value("User", vhtfm.user.name, { language: "de" });
				});
	...
	it("show text for Gender link field with language en", () => {
		cy.window()
			.its("vhtfm")
			.then((vhtfm) => {
				cy.set_value("User", vhtfm.user.name, { language: "en" });
			});
	...
	it("show custom link option", () => {
		cy.window()
			.its("vhtfm")
			.then((vhtfm) => {
				vhtfm.ui.form.ControlLink.link_options = (link) => {
					return [
						{
							html:

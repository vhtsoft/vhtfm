...
"""
Vhtfm - Low Code Open Source Framework in Python and JS

Vhtfm, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://vhtfmframework.com/docs
"""
...
import vhtfm
from vhtfm.query_builder.utils import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from vhtfm.utils.caching import request_cache
from vhtfm.utils.data import cint, cstr, sbool

from .fox_interface import Fox
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from vhtfm.database.mariadb.database import MariaDBDatabase
	from vhtfm.database.postgres.database import PostgresDatabase
	...
	from vhtfm.types.lazytranslatedstring import _LazyTranslate
	from vhtfm.utils.redis_wrapper import RedisWrapper
...
_tune_gc = bool(sbool(os.environ.get("VHTFM_TUNE_GC", True)))
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from vhtfm.translate import get_all_translations
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `vhtfm.local.lang`"""
	from vhtfm.translate import get_user_lang
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize vhtfm for the current site. Reset thread locals `vhtfm.local`"""
	...
	local.site = site
	local.site_name = site  # implicitly scopes fox
	...
	setup_module_map(include_all_apps=not (vhtfm.request or vhtfm.job or vhtfm.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `vhtfm.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from vhtfm.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling vhtfm.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke vhtfm.init(site) prior to calling vhtfm.connect(), if initializing the site is necessary.",
		)
		init(site)
	if db_name:
		from vhtfm.deprecation_dumpster import deprecation_warning
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling vhtfm.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke vhtfm.init(site) with the right config prior to calling vhtfm.connect(), if necessary.",
		)
...
def get_site_config(sites_path: str | None = None, site_path: str | None = None) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			...
			if common_config.developer_mode:
				from vhtfm.utils import get_sites
	...
	config["redis_queue"] = (
		os.environ.get("VHTFM_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("VHTFM_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("VHTFM_DB_TYPE") or config.get("db_type") or "mariadb"
	config["db_socket"] = os.environ.get("VHTFM_DB_SOCKET") or config.get("db_socket")
	config["db_host"] = os.environ.get("VHTFM_DB_HOST") or config.get("db_host") or "127.0.0.1"
	config["db_port"] = int(
		os.environ.get("VHTFM_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
	)

	# Set the user as database name if not set in config
	config["db_user"] = os.environ.get("VHTFM_DB_USER") or config.get("db_user") or config.get("db_name")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("VHTFM_DB_NAME") or config.get("db_name") or config["db_user"]

	# read password
	config["db_password"] = os.environ.get("VHTFM_DB_PASSWORD") or config.get("db_password")
...
def setup_redis_cache_connection():
	"""Defines `vhtfm.cache` as `RedisWrapper` instance"""
	global cache

	if not cache:
		from vhtfm.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from vhtfm.utils import get_traceback
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__vhtfm_exc_id:
				exc.__vhtfm_exc_id = out.__vhtfm_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__vhtfm_exc_id = generate_hash()
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	vhtfm.msgprint(message, indicator=indicator, alert=True)
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `vhtfm.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from vhtfm.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return vhtfm.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from vhtfm.utils import md_to_html
	...
	from vhtfm.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from vhtfm.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from vhtfm.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# vhtfm.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `vhtfm.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import vhtfm.utils.caching
	from vhtfm.website.router import clear_routing_cache

	if doctype:
		vhtfm.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		vhtfm.cache_manager.clear_user_cache(user)
	else:  # everything
		# Delete ALL keys associated with this site.
		keys_to_delete = set(vhtfm.cache.get_keys(""))
		for key in vhtfm.get_hooks("persistent_cache_keys"):
			keys_to_delete.difference_update(vhtfm.cache.get_keys(key))
		vhtfm.cache.delete_value(list(keys_to_delete), make_keys=False)
	...
	vhtfm.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from vhtfm.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `vhtfm.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = vhtfm.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		vhtfm.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `vhtfm.PermissionError` if not permitted.
...
def get_precision(
	...
	from vhtfm.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from vhtfm.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `vhtfm.client.set_value`"""
	import vhtfm.client

	return vhtfm.client.set_value(doctype, docname, fieldname, value)


def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `vhtfm.get_doc`, but return from cache if available."""
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `vhtfm.new_doc` instead."""
	pass
...
def get_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Return a `vhtfm.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = vhtfm.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = vhtfm.get_doc("ToDo", "TD0001")

	"""
	import vhtfm.model.document

	return vhtfm.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `vhtfm.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `vhtfm.model.meta.Meta` instance of given doctype name."""
	import vhtfm.model.meta

	return vhtfm.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import vhtfm.modules

	return vhtfm.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `vhtfm.model.delete_doc.delete_doc`.
	...
	import vhtfm.model.delete_doc

	return vhtfm.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return vhtfm.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `vhtfm.model.rename_doc.rename_doc`
	"""
...
@request_cache
def get_installed_apps(*, _ensure_on_fox: bool = False) -> list[str]:
	"""
	Get list of installed apps in current site.

	:param _ensure_on_fox: Only return apps that are present on fox.
	"""
...
def _load_app_hooks(app_name: str | None = None):
	...
	hooks = {}
	apps = [app_name] if app_name else get_installed_apps(_ensure_on_fox=True)
...
def setup_module_map(include_all_apps: bool = True) -> None:
	...
	:param: include_all_apps: Include all apps on fox, or just apps installed on the site.
	:return: Nothing
	"""
	if include_all_apps:
	...
	if not local.app_modules:
		...
		if include_all_apps:
			...
			apps = get_installed_apps(_ensure_on_fox=True)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = vhtfm.utils.strip(content)
...
def redirect(url):
	...
	from vhtfm.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        vhtfm.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import vhtfm.desk.reportview

	return vhtfm.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `vhtfm.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        vhtfm.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        vhtfm.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import vhtfm.model.db_query

	return vhtfm.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `vhtfm.model.db_query`. Will **not** check for permissions.
	Parameters are same as `vhtfm.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        vhtfm.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        vhtfm.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from vhtfm.deprecation_dumpster import vhtfm_get_test_records as get_test_records
...
def format_value(*args, **kwargs):
	...
	import vhtfm.utils.formatters

	return vhtfm.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from vhtfm.utils.pdf import get_pdf
	from vhtfm.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from vhtfm.translate import print_language
	from vhtfm.utils import scrub_urls
	from vhtfm.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import vhtfm.realtime

	return vhtfm.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return vhtfm.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: vhtfm.local.cache[namespace]
	:param key: vhtfm.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import vhtfm.utils.background_jobs

	return vhtfm.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return vhtfm.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from vhtfm.utils.safe_exec import safe_eval
...
def get_active_domains():
	from vhtfm.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> vhtfm.get_version("User", "foobar@gmail.com")
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from vhtfm.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from vhtfm.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from vhtfm.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from vhtfm.desk.search import sanitize_searchfield
...
from vhtfm.utils.error import log_error

if _tune_gc:
	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for vhtfm where a single query can potentially create 700+
	# objects easily.

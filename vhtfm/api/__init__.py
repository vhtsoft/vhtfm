...
def handle(request: Request):
	...
	if data is not None:
		vhtfm.response["data"] = data
	return build_response("json")


# Merge all API version routing rules
from vhtfm.api.v1 import url_rules as v1_rules
from vhtfm.api.v2 import url_rules as v2_rules
...
def get_api_version() -> ApiVersion | None:
	...
	if vhtfm.request.path.startswith(f"/api/{ApiVersion.V2.value}"):

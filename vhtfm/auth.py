...
import vhtfm
import vhtfm.database
import vhtfm.utils
import vhtfm.utils.user
from vhtfm import _
from vhtfm.apps import get_default_path
from vhtfm.core.doctype.activity_log.activity_log import add_authentication_log
from vhtfm.sessions import Session, clear_sessions, delete_session, get_expiry_in_seconds
from vhtfm.translate import get_language
from vhtfm.twofactor import (
	authenticate_for_2factor,
	confirm_otp_token,
	get_cached_user_pass,
	should_run_2fa,
)
from vhtfm.utils import cint, date_diff, datetime, get_datetime, today
from vhtfm.utils.password import check_password, get_decrypted_password
...
class HTTPRequest:
	def __init__(self):
		...
		vhtfm.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = vhtfm.request.host
	...
	def set_request_ip(self):
		if vhtfm.get_request_header("X-Forwarded-For"):
			vhtfm.local.request_ip = (vhtfm.get_request_header("X-Forwarded-For").split(",", 1)[0]).strip()

		elif vhtfm.get_request_header("REMOTE_ADDR"):
			vhtfm.local.request_ip = vhtfm.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if (
			not vhtfm.request
			or vhtfm.request.method not in UNSAFE_HTTP_METHODS
			or vhtfm.conf.ignore_csrf
			or not vhtfm.session
			or not (saved_token := vhtfm.session.data.csrf_token)
			or (
				(vhtfm.get_request_header("X-Vhtfm-CSRF-Token") or vhtfm.form_dict.pop("csrf_token", None))
				== saved_token
			)
		...
		vhtfm.flags.disable_traceback = True
		vhtfm.throw(_("Invalid Request"), vhtfm.CSRFTokenError)

	def set_lang(self):
		vhtfm.local.lang = get_language()

	def is_allowed_referrer(self):
		referrer = vhtfm.get_request_header("Referer")
		origin = vhtfm.get_request_header("Origin")

		# Get the list of allowed referrers from cache or configuration
		allowed_referrers = vhtfm.cache.get_value(
			"allowed_referrers",
			generator=lambda: vhtfm.conf.get("allowed_referrers", []),
		)
...
class LoginManager:
	...
	def __init__(self):
		...
		if vhtfm.local.form_dict.get("cmd") == "login" or vhtfm.local.request.path == "/api/method/login":
	...
	def login(self):
		...
		if vhtfm.get_system_settings("disable_user_pass_login"):
			vhtfm.throw(_("Login with username and password is not allowed."), vhtfm.AuthenticationError)

		# clear cache
		vhtfm.clear_cache(user=vhtfm.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = vhtfm.get_doc("User", self.user)
			vhtfm.local.response["redirect_to"] = doc.reset_password(send_email=False, password_expired=True)
			vhtfm.local.response["message"] = "Password Reset"
			return False
	...
	def get_user_info(self):
		self.info = vhtfm.get_cached_value(
			"User", self.user, ["user_type", "first_name", "last_name", "user_image"], as_dict=1
		)
		self.user_lang = vhtfm.translate.get_user_lang()
		self.user_type = self.info.user_type

	def setup_boot_cache(self):
		vhtfm.cache_manager.build_table_count_cache()
		vhtfm.cache_manager.build_domain_restricted_doctype_cache()
		vhtfm.cache_manager.build_domain_restricted_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			vhtfm.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				vhtfm.local.response["message"] = "No App"
				vhtfm.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			vhtfm.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				vhtfm.local.response["message"] = "Logged In"
				vhtfm.local.response["home_page"] = get_default_path() or "/app"

		if not resume:
			vhtfm.response["full_name"] = self.full_name

		# redirect information
		redirect_to = vhtfm.cache.hget("redirect_after_login", self.user)
		if redirect_to:
			vhtfm.local.response["redirect_to"] = redirect_to
			vhtfm.cache.hdel("redirect_after_login", self.user)

		vhtfm.local.cookie_manager.set_cookie("full_name", self.full_name)
		vhtfm.local.cookie_manager.set_cookie("user_id", self.user)
		vhtfm.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")
		# cache control: round trip the effectively delivered language
		vhtfm.local.cookie_manager.set_cookie("user_lang", self.user_lang)

	def clear_preferred_language(self):
		vhtfm.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(self, resume=False):
		# start session
		vhtfm.local.session_obj = Session(
			user=self.user, resume=resume, full_name=self.full_name, user_type=self.user_type
		)

		# reset user if changed to Guest
		self.user = vhtfm.local.session_obj.user
		vhtfm.local.session = vhtfm.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(vhtfm.conf.get("deny_multiple_sessions"))
			or cint(vhtfm.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		clear_sessions(vhtfm.session.user, keep_current=True)

	def authenticate(self, user: str | None = None, pwd: str | None = None):
		from vhtfm.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = vhtfm.form_dict.get("usr"), vhtfm.form_dict.get("pwd")
		if not (user and pwd):
		...
		ip_tracker = get_login_attempt_tracker(vhtfm.local.request_ip)
		if not user:
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in vhtfm.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if self.user in vhtfm.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(vhtfm.get_system_settings("force_user_to_reset_password"))

		if reset_pwd_after_days:
			last_password_reset_date = (
				vhtfm.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		vhtfm.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		vhtfm.db.commit()
		raise vhtfm.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in vhtfm.get_hooks().get(event, []):
			vhtfm.call(vhtfm.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = cint(vhtfm.db.get_value("User", self.user, "login_before", ignore=True))
		login_after = cint(vhtfm.db.get_value("User", self.user, "login_after", ignore=True))

		if not (login_before or login_after):
			return

		from vhtfm.utils import now_datetime

		current_hour = int(now_datetime().strftime("%H"))

		if login_before and current_hour >= login_before:
			vhtfm.throw(_("Login not allowed at this time"), vhtfm.AuthenticationError)
	...
	def impersonate(self, user):
		...
		vhtfm.local.session_obj.set_impersonsated(current_user)
	...
	def logout(self, arg="", user=None):
		...
		if user == vhtfm.session.user:
			delete_session(vhtfm.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
			if vhtfm.request:
				self.login_as_guest()
		else:
...
class CookieManager:
	...
	def init_cookies(self):
		if not vhtfm.local.session.get("sid"):
			return

		if vhtfm.session.sid:
			self.set_cookie("sid", vhtfm.session.sid, max_age=get_expiry_in_seconds(), httponly=True)
	...
	def set_cookie(
		...
		if not secure and hasattr(vhtfm.local, "request"):
			secure = vhtfm.local.request.scheme == "https"
...
@vhtfm.whitelist()
def get_logged_user():
	return vhtfm.session.user


def clear_cookies():
	if hasattr(vhtfm.local, "session"):
		vhtfm.session.sid = ""
	vhtfm.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	checked for those
	"""
	if hasattr(vhtfm.local, "request") and vhtfm.local.request.path.startswith(
		"/api/method/vhtfm.realtime."
	):
		return True

	from vhtfm.core.doctype.user.user import get_restricted_ip_list

	# Only fetch required fields - for perf
	user_fields = ["restrict_ip", "bypass_restrict_ip_check_if_2fa_enabled"]
	user_info = (
		vhtfm.get_cached_value("User", user, user_fields, as_dict=True)
		if not vhtfm.flags.in_test
		else vhtfm.db.get_value("User", user, user_fields, as_dict=True)
	)
	...
	system_settings = (
		vhtfm.get_cached_doc("System Settings")
		if not vhtfm.flags.in_test
		else vhtfm.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if vhtfm.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

	vhtfm.throw(_("Access not allowed from this IP Address"), vhtfm.AuthenticationError)
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = vhtfm.get_doc("System Settings")
...
class LoginAttemptTracker:
	...
	def login_failed_count(self):
		return vhtfm.cache.hget("login_failed_count", self.key)

	@login_failed_count.setter
	def login_failed_count(self, count):
		vhtfm.cache.hset("login_failed_count", self.key, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		vhtfm.cache.hdel("login_failed_count", self.key)
	...
	def login_failed_time(self):
		...
		"""
		return vhtfm.cache.hget("login_failed_time", self.key)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		vhtfm.cache.hset("login_failed_time", self.key, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		vhtfm.cache.hdel("login_failed_time", self.key)
...
def validate_auth():
	...
	"""
	authorization_header = vhtfm.get_request_header("Authorization", "").split(" ")
	...
	if len(authorization_header) == 2 and vhtfm.session.user in ("", "Guest"):
		raise vhtfm.AuthenticationError
...
def validate_oauth(authorization_header):
	...
	from vhtfm.integrations.oauth2 import get_oauth_server
	from vhtfm.oauth import get_url_delimiter

	if authorization_header[0].lower() != "bearer":
		return

	form_dict = vhtfm.local.form_dict
	...
	try:
		required_scopes = vhtfm.db.get_value("OAuth Bearer Token", token, "scopes").split(
			get_url_delimiter()
		)
		...
		if valid:
			vhtfm.set_user(vhtfm.db.get_value("OAuth Bearer Token", token, "user"))
...
def validate_auth_via_api_keys(authorization_header):
	...
	try:
		...
		authorization_source = vhtfm.get_request_header("Vhtfm-Authorization-Source")
		if auth_type.lower() == "basic":
			api_key, api_secret = vhtfm.safe_decode(base64.b64decode(auth_token)).split(":")
		...
		vhtfm.throw(
			...
			vhtfm.InvalidAuthorizationToken,
		)
	except (AttributeError, TypeError, ValueError):
		pass


def validate_api_key_secret(api_key, api_secret, vhtfm_authorization_source=None):
	"""vhtfm_authorization_source to provide api key and secret for a doctype apart from User"""
	doctype = vhtfm_authorization_source or "User"
	doc = vhtfm.db.get_value(doctype=doctype, filters={"api_key": api_key}, fieldname=["name"])
	if not doc:
		raise vhtfm.AuthenticationError
	form_dict = vhtfm.local.form_dict
	...
	if api_secret == doc_secret:
		if doctype == "User":
			user = vhtfm.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["name"])
		else:
			user = vhtfm.db.get_value(doctype, doc, "user")
		if vhtfm.local.login_manager.user in ("", "Guest"):
			vhtfm.set_user(user)
		vhtfm.local.form_dict = form_dict
	else:
		raise vhtfm.AuthenticationError
...
def validate_auth_via_hooks():
	for auth_hook in vhtfm.get_hooks("auth_hooks", []):
		vhtfm.get_attr(auth_hook)()

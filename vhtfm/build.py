...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "vhtfm", "dist", type)
	...
	assets_json = vhtfm.read_file("assets/assets.json")
	if assets_json:
		assets_json = vhtfm.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/vhtfm"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["vhtfm/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="vhtfm")


def get_assets_link(vhtfm_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/vhtfm && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(vhtfm_head)
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/vhtfm/vhtfm/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.vhtfmframework.com/{vhtfm_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {vhtfm_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, vhtfm_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="vhtfm-assets-", suffix=vhtfm_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Vhtfm assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./vhtfm-fox/sites", "")
...
def download_vhtfm_assets(verbose=True) -> bool:
	"""Download and set up Vhtfm assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	vhtfm_head = getoutput("cd ../apps/vhtfm && git rev-parse HEAD")

	if not vhtfm_head:
		return False

	try:
		url = get_assets_link(vhtfm_head)
		assets_archive = fetch_assets(url, vhtfm_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{vhtfm.generate_hash()}"
...
def setup():
	...
	for app in vhtfm.get_all_apps(True):
		try:
			pymodules.append(vhtfm.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(vhtfm.local.sites_path, "assets")
...
def bundle(
	...
	if not apps or apps == "vhtfm":
		command += " && cd billing && yarn build"

	check_node_executable()
	vhtfm_app_path = vhtfm.get_app_source_path("vhtfm")
	vhtfm.commands.popen(command, cwd=vhtfm_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = vhtfm.utils.cint(os.environ.get("LIVE_RELOAD", vhtfm.conf.live_reload))
	...
	check_node_executable()
	vhtfm_app_path = vhtfm.get_app_source_path("vhtfm")
	vhtfm.commands.popen(command, cwd=vhtfm_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in vhtfm.get_all_apps():
		app_doc_path = None

		pymodule = vhtfm.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `vhtfm.templates`."""
	return """vhtfm.templates["{key}"] = '{content}';\n""".format(

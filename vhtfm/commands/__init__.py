...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			vhtfm.exceptions.SiteNotSpecifiedError,
			vhtfm.exceptions.IncorrectSitePath,
			vhtfm.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise vhtfm.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://vhtfmframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands

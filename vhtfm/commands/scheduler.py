...
def trigger_scheduler_event(context: CliCtxObj, event):
	...
	for site in context.sites:
		try:
			...
			vhtfm.connect()
			try:
				vhtfm.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def scheduler(context: CliCtxObj, state: str, format: str, verbose: bool = False, site: str | None = None):
	...
	import vhtfm
	from vhtfm.utils.scheduler import is_scheduler_inactive, toggle_scheduler
	...
	with vhtfm.init_site(site=site):
		match state:
			case "status":
				vhtfm.connect()
...
def doctor(context: CliCtxObj, site=None):
	...
	from vhtfm.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context: CliCtxObj, site=None):
	...
	from vhtfm.utils.doctor import pending_jobs as _pending_jobs
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None, burst=False, strategy=None):
	...
	from vhtfm.utils.background_jobs import start_worker
...
def start_worker_pool(queue, quiet=False, num_workers=2, burst=False):
	...
	from vhtfm.utils.background_jobs import start_worker_pool

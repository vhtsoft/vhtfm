...
from vhtfm.utils import CallbackManager
...
def new_site(
	...
	"Create a new site"
	from vhtfm.installer import _new_site

	vhtfm.init(site, new_site=True)
...
def _restore(
	*,
	...
	from vhtfm.installer import extract_files
	from vhtfm.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = vhtfm.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from vhtfm.installer import _new_site, is_downgrade, is_partial, validate_database_sql

	# Check for the backup file in the backup directory, as well as the main fox directory
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Vhtfm site.",
			fg="red",
		)
		click.secho(
			"Use `fox partial-restore` to restore a partial backup to an existing site.",
			fg="yellow",
		)
		sys.exit(1)

	# Check if the backup is of an older version of vhtfm and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from vhtfm.installer import is_partial, partial_restore
	from vhtfm.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `fox restore` to restore a Vhtfm Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		vhtfm.connect()
		vhtfm.clear_cache()
		installed = vhtfm.get_installed_apps()
		vhtfm.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from vhtfm.installer import install_app as _install_app
	from vhtfm.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except vhtfm.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{vhtfm.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = vhtfm.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(vhtfm.get_installed_apps())
	...
	if format == "json":
		click.echo(vhtfm.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		vhtfm.connect()
		try:
			vhtfm.db.add_index(doctype, columns)
			if len(columns) == 1:
...
def describe_database_table(context, doctype, column):
	...
	from vhtfm.core.doctype.recorder.recorder import _fetch_table_stats

	for site in context.sites:
		vhtfm.init(site=site)
		vhtfm.connect()
		try:
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		vhtfm.connect()
		try:
			vhtfm.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			vhtfm.db.commit()
		finally:
			vhtfm.destroy()
	if not context.sites:
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from vhtfm.migrate import SiteMigration
...
def migrate_to():
	...
	from vhtfm.integrations.vhtfm_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			vhtfm.connect()
			vhtfm.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			vhtfm.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			vhtfm.init(site)
			vhtfm.connect()
			vhtfm.reload_doc(module, doctype, docname, force=context.force)
			vhtfm.db.commit()
		finally:
			vhtfm.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			vhtfm.init(site)
			vhtfm.connect()
			vhtfm.reload_doctype(doctype, force=context.force)
			vhtfm.db.commit()
		finally:
			vhtfm.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		vhtfm.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(vhtfm.get_traceback(with_context=True))
			exit_code = 1
			continue
		if vhtfm.get_system_settings("encrypt_backup") and vhtfm.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_vhtfm(app)
	from vhtfm.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_vhtfm(app)
	from vhtfm.installer import remove_app
	from vhtfm.utils.synchronization import filelock
...
def _drop_site(
	...
	from vhtfm.database import drop_user_and_database
	from vhtfm.utils.backups import scheduled_backup
	...
	try:
		...
		if force:
			...
			messages = [
				...
				f"Hint: Use 'fox drop-site {site} --force' to force the removal of {site}",
			]
			click.echo("\n".join(messages))
			sys.exit(1)
	...
	vhtfm.flags.root_login = db_root_username
	vhtfm.flags.root_password = db_root_password

	drop_user_and_database(vhtfm.conf.db_name, vhtfm.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		vhtfm.utils.get_fox_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	vhtfm.init(site)
	old_path = vhtfm.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with vhtfm.init_site(site):
		...
		vhtfm.db.set_value("User", user, "last_active", now_datetime())
		vhtfm.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	"Publish realtime event from fox"
	from vhtfm import publish_realtime
...
def browse(context: CliCtxObj, site, user=None):
	...
	if site not in vhtfm.utils.get_sites():
	...
	if user:
		...
		if vhtfm.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={vhtfm.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{vhtfm.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = vhtfm.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from vhtfm.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from vhtfm.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from vhtfm.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise vhtfm.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == vhtfm.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = vhtfm.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				vhtfm.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[vhtfm.local.site] = TABLES_TO_DROP
		vhtfm.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		vhtfm.conf.db_type,
		f"framework_{vhtfm.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from vhtfm.model.meta import trim_tables
	from vhtfm.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {vhtfm.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {vhtfm.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({vhtfm.local.site: data}, indent=1, sort_keys=True))
	else:
...
def ensure_app_not_vhtfm(app: str) -> None:
	"""
	Ensure that the app name passed is not 'vhtfm'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "vhtfm":
		click.secho("You cannot remove or uninstall the app `vhtfm`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from vhtfm.modules.patch_handler import update_patch_log

...
if TYPE_CHECKING:
	...
	from vhtfm.testing import TestRunner
...
def main(
	...
	from vhtfm.testing.environment import _cleanup_after_tests, _initialize_test_environment

	if debug and not debug_exceptions:
		debug_exceptions = (Exception,)

	testing_module_logger = logging.getLogger("vhtfm.testing")
	...
	try:
		...
		for app, category, result in results:
			_print_result(vhtfm.unscrub(app or "Unspecified App"), vhtfm.unscrub(category), result)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(vhtfm.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from vhtfm.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = vhtfm.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from vhtfm.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		vhtfm.init(site)
		allow_tests = vhtfm.get_conf().allow_tests

		if not (allow_tests or os.environ.get("CI")):
			click.secho("Testing is disabled for the site!", bold=True)
			click.secho("You can enable tests by entering following command:")
			click.secho(f"fox --site {site} set-config allow_tests true", fg="green")
			return
...
@click.option("--app", help="For App", default="vhtfm")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app) as cc:
		site = get_site(context)
		if use_orchestrator:
			from vhtfm.parallel_test_runner import ParallelTestWithOrchestrator

			runner = ParallelTestWithOrchestrator(app, site=site)
		else:
			from vhtfm.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	vhtfm.init(site)
	app_base_path = vhtfm.get_app_source_path(app)
	site_url = vhtfm.utils.get_site_url(site)
	admin_password = vhtfm.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../vhtfm && yarn bin)")
	...
	if not (
		...
		vhtfm.commands.popen(f"(cd ../vhtfm && yarn add {packages} --no-lockfile)")
	...
	try:
		vhtfm.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)

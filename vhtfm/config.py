"""
This module provides a comprehensive configuration management system for the Vhtfm framework.
...
The module includes global default configurations for common Vhtfm settings such as Redis URLs, database connections,
...
class VhtfmUnregisteredConfigOptionWarning(Warning):
	pass
		...
		if name not in registry.options:
			...
			if not name.startswith("_ipython") and name != "_repr_mimebundle_":
				warnings.warn(
					...
					VhtfmUnregisteredConfigOptionWarning,
					stacklevel=2,
				)
...
class ConfigHandler:
	...
	def _update_from_env(self):
		...
		for key in self._config.keys():
			...
			env_key = f"VHTFM_{env_key.upper()}"
			if env_value := os.environ.get(env_key):
				self._config[key] = env_value

	def _apply_extra_config(self):
		"Apply additional configuration from external modules"
		# TODO: maybe motion to deprecate https://github.com/vhtfm/vhtfm/pull/24706#issuecomment-2471209484
		assert isinstance(self._config, ConfigType)  # will never be None by now

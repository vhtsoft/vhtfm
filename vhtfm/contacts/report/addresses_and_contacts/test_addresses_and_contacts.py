...
from vhtfm.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from vhtfm.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(vhtfm.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if vhtfm.flags.test_address_created:
		return

	address = vhtfm.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	vhtfm.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if vhtfm.flags.test_contact_created:
		return
	...
	vhtfm.flags.test_contact_created = True

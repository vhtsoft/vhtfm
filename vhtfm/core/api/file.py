...
from vhtfm.core.doctype.file.utils import setup_folder_path
...
def unzip_file(name: str):
	...
	file: File = vhtfm.get_doc("File", name)
	return file.unzip()


@vhtfm.whitelist()
def get_attached_images(doctype: str, names: list[str] | str) -> vhtfm._dict:
	"""Return list of image urls attached in form `{name: ['image.jpg', 'image.png']}`."""

	if isinstance(names, str):
		names = json.loads(names)

	img_urls = vhtfm.db.get_list(
		"File",
		filters={
...
def get_files_in_folder(folder: str, start: int = 0, page_length: int = 20) -> dict:
	attachment_folder = vhtfm.db.get_value(
		"File",
		"Home/Attachments",
...
def get_max_file_size() -> int:
	return (
		cint(vhtfm.get_system_settings("max_file_size")) * 1024 * 1024
		or cint(vhtfm.conf.get("max_file_size"))
		or 25 * 1024 * 1024
	)
...
def move_file(file_list: list[File | dict] | str, new_parent: str, old_parent: str) -> None:
	...
	# recalculate sizes
	vhtfm.get_doc("File", old_parent).save()
	vhtfm.get_doc("File", new_parent).save()


@vhtfm.whitelist()
def zip_files(files: str):
	files = vhtfm.parse_json(files)
	vhtfm.response["filename"] = "files.zip"
	vhtfm.response["filecontent"] = File.zip_files(files)
	vhtfm.response["type"] = "download"

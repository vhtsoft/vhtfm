...
vhtfm.ui.form.on("Access Log", {
	show_document: function (frm) {
		vhtfm.set_route("Form", frm.doc.export_from, frm.doc.reference_document);
	},

	show_report: function (frm) {
		if (frm.doc.report_name.includes("/")) {
			vhtfm.set_route(frm.doc.report_name);
		} else {
			let filters = frm.doc.filters ? JSON.parse(frm.doc.filters) : {};
			vhtfm.set_route("query-report", frm.doc.report_name, filters);
		}
	},
});

...
from vhtfm.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(vhtfm.session.user)
		vhtfm.db.commit()
		generated_secret = vhtfm.utils.password.get_decrypted_password(
			"User", vhtfm.session.user, fieldname="api_secret"
		)
		...
		self.file_name = vhtfm.utils.random_string(10) + ".txt"
		self.test_content = vhtfm.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = vhtfm.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = vhtfm.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(vhtfm.local.site) + new_private_file.file_url

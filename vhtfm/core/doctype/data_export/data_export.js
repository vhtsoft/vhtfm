...
vhtfm.ui.form.on("Data Export", {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					issingle: 0,
					istable: 0,
					name: ["in", vhtfm.boot.user.can_export],
				},
			};
		});
	},
	reference_doctype: (frm) => {
		...
		if (doctype) {
			vhtfm.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = (frm) => {
	...
	if (!doctype) {
		vhtfm.msgprint(__("Please select the Document Type."));
	} else if (!parent_multicheck_options.length) {
		vhtfm.msgprint(__("At least one field of Parent Document Type is mandatory"));
	} else {
		is_valid_form = true;
	}
...
const export_data = (frm) => {
	let get_template_url = "/api/method/vhtfm.core.doctype.data_export.exporter.export_data";
	var export_params = () => {
		let columns = {};
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new vhtfm.ui.FilterGroup({
...
const make_multiselect_buttons = (parent_wrapper) => {
	...
	["Select All", "Unselect All"].map((d) => {
		vhtfm.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: vhtfm.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = (parentdt) => {
	return [parentdt].concat(vhtfm.meta.get_table_fields(parentdt).map((df) => df.options));
};

const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	const fields = get_fields(doctype);

	vhtfm.model.std_fields
	...
	const multicheck_control = vhtfm.ui.form.make_control({
...
const filter_fields = (df) => vhtfm.model.is_value_type(df) && !df.hidden;
const get_fields = (dt) => vhtfm.meta.get_docfields(dt).filter(filter_fields);

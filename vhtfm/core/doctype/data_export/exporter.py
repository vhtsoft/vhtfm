...
from vhtfm.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from vhtfm.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = [
				...
				for df in vhtfm.get_meta(self.doctype).get_table_fields()
			]
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			vhtfm.response["result"] = cstr(self.writer.getvalue())
			vhtfm.response["type"] = "csv"
			vhtfm.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		for f in vhtfm.db.get_table_columns_description(table_name):
			field = meta.get_field(f.name)
			if f.name in ["owner", "creation"]:
				std_field = next((x for x in vhtfm.model.std_fields if x["fieldname"] == f.name), None)
				if std_field:
					field = vhtfm._dict(
						{
		...
		_column_start_end = vhtfm._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = vhtfm._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(vhtfm.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		vhtfm.permissions.can_export(self.parent_doctype, raise_exception=True)
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						vhtfm.qb.from_(child_doctype_table)
						.select("*")
	...
	def build_response_as_excel(self):
		...
		filename = vhtfm.generate_hash(length=10)

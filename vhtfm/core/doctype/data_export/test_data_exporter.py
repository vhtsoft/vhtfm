...
import vhtfm
from vhtfm.core.doctype.data_export.exporter import DataExporter
...
class TestDataExporter(IntegrationTestCase):
	...
	def create_test_data(self, force=False):
		...
		"""
		if force:
			vhtfm.delete_doc(self.doctype_name, self.doc_name)

		if not vhtfm.db.exists(self.doctype_name, self.doc_name):
			self.doc = vhtfm.get_doc(
			...
			self.doc = vhtfm.get_doc(self.doctype_name, self.doc_name)

	def test_export_content(self):
		exp = DataExporter(doctype=self.doctype_name, file_type="CSV")
		exp.build_response()

		self.assertEqual(vhtfm.response["type"], "csv")
		self.assertEqual(vhtfm.response["doctype"], self.doctype_name)
		self.assertTrue(vhtfm.response["result"])
		self.assertRegex(vhtfm.response["result"], r"Child Title 1.*?,50")
		self.assertRegex(vhtfm.response["result"], r"Child Title 2.*?,51")
		...
		for type in ["csv", "Excel"]:
			with self.subTest(type=type):
				...
				if type == "csv":
					...
					self.assertEqual(
						vhtfm.response["filename"], self.doctype_name + ".xlsx"
					)  # 'Test DocType for Export Tool.xlsx')
					self.assertTrue(vhtfm.response["filecontent"])

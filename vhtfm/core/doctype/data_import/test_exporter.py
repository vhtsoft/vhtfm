...
from vhtfm.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not vhtfm.db.exists(doctype_name, "Test"):
			doc = vhtfm.get_doc(
			...
			doc = vhtfm.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(vhtfm.response["doctype"], doctype_name)

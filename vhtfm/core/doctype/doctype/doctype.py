...
import vhtfm
from vhtfm import _
from vhtfm.cache_manager import clear_controller_cache, clear_user_cache
...
from vhtfm.database.schema import validate_column_length, validate_column_name
from vhtfm.desk.notifications import delete_notification_count_for, get_filters_for
...
from vhtfm.model.docfield import supports_translation
from vhtfm.model.document import Document
from vhtfm.model.meta import Meta
from vhtfm.modules import get_doc_path, make_boilerplate
from vhtfm.modules.import_file import get_file_path
from vhtfm.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from vhtfm.query_builder.functions import Concat
from vhtfm.utils import cint, flt, get_datetime, is_a_property, random_string
from vhtfm.website.utils import clear_cache

if TYPE_CHECKING:
	from vhtfm.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(vhtfm.ValidationError):
	pass


class UniqueFieldnameError(vhtfm.ValidationError):
	pass


class IllegalMandatoryError(vhtfm.ValidationError):
	pass


class DoctypeLinkError(vhtfm.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(vhtfm.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(vhtfm.ValidationError):
	pass


class NonUniqueError(vhtfm.ValidationError):
	pass


class CannotIndexedError(vhtfm.ValidationError):
	pass


class CannotCreateStandardDoctypeError(vhtfm.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.core.doctype.docfield.docfield import DocField
		from vhtfm.core.doctype.docperm.docperm import DocPerm
		from vhtfm.core.doctype.doctype_action.doctype_action import DocTypeAction
		from vhtfm.core.doctype.doctype_link.doctype_link import DocTypeLink
		from vhtfm.core.doctype.doctype_state.doctype_state import DocTypeState
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = vhtfm.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			vhtfm.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not d.unique and not d.search_index:
				referred_as_link = vhtfm.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if vhtfm.flags.in_patch or vhtfm.flags.in_test:
			return

		if not vhtfm.conf.get("developer_mode") and not self.custom:
			vhtfm.throw(
		...
		if self.is_virtual and self.custom:
			vhtfm.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if vhtfm.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = vhtfm.get_meta(vhtfm.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except vhtfm.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = vhtfm.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				vhtfm.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from vhtfm.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from vhtfm.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = vhtfm.bold(self.meta.get_field("max_attachments").label)
			vhtfm.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if vhtfm.flags.in_import:
			return
		parent_list = vhtfm.get_all(
			"DocField", "parent", dict(fieldtype=["in", vhtfm.model.table_fields], options=self.name)
		)
		for p in parent_list:
			vhtfm.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			vhtfm.db.updatedb(self.name, Meta(None, bootstrap=self))
		...
		allow_doctype_export = (
			...
			and (vhtfm.conf.developer_mode or vhtfm.flags.allow_doctype_export)
		)
		...
		vhtfm.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(vhtfm.session.user)

		if not vhtfm.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({vhtfm.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			vhtfm.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not vhtfm.request) or vhtfm.flags.in_test or vhtfm.flags.in_install
			vhtfm.enqueue("vhtfm.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = vhtfm.scrub(self.name)
		scrubbed_module = vhtfm.scrub(self.module)
		controller_path = vhtfm.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from vhtfm.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from vhtfm.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and vhtfm.session.user != "Administrator":
			vhtfm.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			vhtfm.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			vhtfm.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			vhtfm.db.sql(
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if vhtfm.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(vhtfm.scrub(old), vhtfm.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		vhtfm.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(vhtfm.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{vhtfm.scrub(new)}.json")
		current_data = vhtfm.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			vhtfm.db.table_exists(self.name)
			and vhtfm.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = vhtfm.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = vhtfm.get_module_path(
				vhtfm.scrub(self.module), "doctype", vhtfm.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		...
		from vhtfm.types.exporter import TypeExporter
		...
		if any(vhtfm.get_hooks("export_python_type_annotations", app_name=app)):
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not vhtfm.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not vhtfm.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = vhtfm.bold(_("Parent Field (Tree)"))
			vhtfm.throw(_("{0} is a mandatory field").format(field_label), vhtfm.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			vhtfm.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = vhtfm.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = vhtfm.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = vhtfm.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			vhtfm.throw(_("DocType's name should not start or end with whitespace"), vhtfm.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(vhtfm.scrub(self.module), self.doctype, self.name))
		content = json.loads(file.read_text())
		if content.get("modified") and get_datetime(self.modified) < get_datetime(content.get("modified")):
			vhtfm.msgprint(
				_(
					"This doctype has pending migrations, run 'fox migrate' before modifying the doctype to avoid losing changes."
				),
				alert=True,
				indicator="yellow",
			)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			vhtfm.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(vhtfm.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			vhtfm.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if autoname and autoname.startswith("format:"):
		from vhtfm.model.naming import BRACED_PARAMS_HASH_PATTERN

		if len(BRACED_PARAMS_HASH_PATTERN.findall(autoname)) > 1:
			vhtfm.throw(_("Only one set of {#} pattern is allowed in the format string"))
	...
	if (
		...
		doctype = vhtfm.qb.DocType("DocType")
		used_in = (
		...
		if used_in:
			vhtfm.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			vhtfm.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = vhtfm.db.get_value(
			...
			if not property_value:
				return vhtfm.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (is_autoname_autoincrement and autoname_before_save != "autoincrement") or (
			...
			if dt.doctype == "Customize Form":
				vhtfm.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if vhtfm.get_meta(dt.name).issingle:
				return False

			if not vhtfm.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	vhtfm.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or vhtfm.flags.in_patch or vhtfm.flags.in_fixtures or vhtfm.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			vhtfm.throw(message, vhtfm.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			vhtfm.throw(message, vhtfm.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = vhtfm.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, vhtfm.bold(link.table_fieldname), vhtfm.bold(meta.name)
			)
			vhtfm.throw(message, vhtfm.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		vhtfm.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		vhtfm.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		vhtfm.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = vhtfm.get_meta(doctype, cached=False)
...
def validate_fields(meta: Meta):
	...
	:param meta: `vhtfm.model.meta.Meta` object to check."""
	...
	def check_link_table_options(docname, d):
		if vhtfm.flags.in_patch or vhtfm.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = vhtfm.db.get_value("DocType", d.options, "name")
				if not options:
					vhtfm.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not vhtfm.flags.in_migrate:
			vhtfm.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			vhtfm.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			vhtfm.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and vhtfm.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = vhtfm.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					vhtfm.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						vhtfm.throw(_("Fold must come before a Section Break"))
				else:
					vhtfm.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				vhtfm.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			vhtfm.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			vhtfm.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			vhtfm.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			vhtfm.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			vhtfm.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			vhtfm.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			vhtfm.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				vhtfm.throw(_("Invalid {0} condition").format(vhtfm.unscrub(field)), vhtfm.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			vhtfm.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = vhtfm.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				vhtfm.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if vhtfm.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = vhtfm.get_meta(doctype)

		if not child_doctype_meta.istable:
			vhtfm.throw(
				_("Option {0} for field {1} is not a child table").format(
					vhtfm.bold(doctype), vhtfm.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			vhtfm.throw(f"Max for {vhtfm.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				vhtfm.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not vhtfm.flags.in_migrate or in_ci:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = vhtfm.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		vhtfm.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			vhtfm.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			vhtfm.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			vhtfm.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			vhtfm.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			vhtfm.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			vhtfm.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			vhtfm.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			vhtfm.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			vhtfm.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				vhtfm.throw(
					...
					).format(d.idx, vhtfm.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in vhtfm.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not vhtfm.db.exists("Domain", doc.restrict_to_domain)
		):
			vhtfm.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()

		if "tabModule Def" in vhtfm.db.get_tables() and not vhtfm.db.exists("Module Def", doc.module):
			m = vhtfm.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if vhtfm.scrub(doc.module) in vhtfm.local.module_app:
				m.app_name = vhtfm.local.module_app[vhtfm.scrub(doc.module)]
			else:
				m.app_name = "vhtfm"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if vhtfm.flags.package:
				m.package = vhtfm.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if vhtfm.db.table_exists("Role", cached=False) and not vhtfm.db.exists("Role", role):
				r = vhtfm.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
...
def check_fieldname_conflicts(docfield):
	...
	doc = vhtfm.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		vhtfm.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	vhtfm.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		vhtfm.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		vhtfm.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		vhtfm.throw(_("Select a valid Sender Field for creating documents from Email"))

	if sender_field.options != "Email":
		vhtfm.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	vhtfm.has_permission("DocType", throw=True)
	try:
		return flt(vhtfm.db.get_row_size(doctype) / vhtfm.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0

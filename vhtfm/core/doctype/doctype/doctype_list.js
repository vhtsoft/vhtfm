vhtfm.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = vhtfm.session.user !== "Administrator" || !vhtfm.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new vhtfm.ui.Dialog({
			...
			primary_action(values) {
				...
				vhtfm.db
					...
					.then((doc) => {
						vhtfm.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (vhtfm.get_route()[0] === "Form") {

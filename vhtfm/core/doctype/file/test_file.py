...
import vhtfm
from vhtfm import _
from vhtfm.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from vhtfm.core.doctype.file.exceptions import FileTypeNotAllowed
from vhtfm.core.doctype.file.utils import get_corrupted_image_msg, get_extension
from vhtfm.desk.form.utils import add_comment
from vhtfm.exceptions import ValidationError
from vhtfm.tests import IntegrationTestCase, UnitTestCase
from vhtfm.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = vhtfm.session.user
...
def make_test_image_file(private=False):
	file_path = vhtfm.get_app_path("vhtfm", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = vhtfm.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: File = vhtfm.get_doc("File", test_file.name)
...
class TestSimpleFile(IntegrationTestCase):
	...
	def test_save(self):
		_file = vhtfm.get_doc("File", {"file_url": self.saved_file_url})
...
class TestFSRollbacks(IntegrationTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = vhtfm.generate_hash()
		file = vhtfm.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(IntegrationTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = vhtfm.generate_hash()
		bad_file = vhtfm.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = vhtfm.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		vhtfm.db.rollback()
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: vhtfm.Document = vhtfm.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: vhtfm.Document = vhtfm.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameFileName(IntegrationTestCase):
	def test_saved_content(self):
		...
		_file = vhtfm.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = vhtfm.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = vhtfm.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = vhtfm.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(IntegrationTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(vhtfm.exceptions.AttachmentLimitReached, file2.insert)
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = vhtfm.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def tearDown(self):
		try:
			vhtfm.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except vhtfm.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = vhtfm.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = vhtfm.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = vhtfm.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = vhtfm.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = vhtfm.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			vhtfm.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = vhtfm.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(vhtfm.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = vhtfm.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = vhtfm.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: File = vhtfm.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://vhtfm.io/files/vhtfm.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: File = vhtfm.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": vhtfm.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = vhtfm.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": vhtfm.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = vhtfm.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			vhtfm.message_log[0].get("message"),
			f"File '{vhtfm.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = vhtfm.get_app_path("vhtfm", "www/_test/assets/file.zip")
		public_file_path = vhtfm.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(IntegrationTestCase):
	...
	def tearDownClass(cls):
		vhtfm.db.rollback()
		vhtfm.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = vhtfm.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(IntegrationTestCase):
	def setUp(self) -> None:
		vhtfm.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(IntegrationTestCase):
	def test_extract_images_from_doc(self):
		is_private = not vhtfm.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(vhtfm.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = vhtfm.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{vhtfm.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not vhtfm.get_meta("ToDo").make_attachments_public
		test_doc = vhtfm.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			vhtfm.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not vhtfm.get_meta("Communication").make_attachments_public
		communication = vhtfm.get_doc(
		...
		self.assertTrue(
			vhtfm.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(IntegrationTestCase):
	...
	def test_optimize_svg(self):
		file_path = vhtfm.get_app_path("vhtfm", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = vhtfm.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = vhtfm.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		vhtfm.set_user("Administrator")
		vhtfm.db.rollback()
		vhtfm.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = vhtfm.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + vhtfm.generate_hash()
		content = file_name.encode()

		doc_pub: File = vhtfm.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: File = vhtfm.new_doc("File")  # type: ignore

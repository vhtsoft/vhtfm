...
class TestPackage(IntegrationTestCase):
	def test_package_release(self):
		...
		# make release
		vhtfm.get_doc(doctype="Package Release", package="Test Package", publish=1).insert()

		self.assertTrue(os.path.exists(vhtfm.get_site_path("packages", "test-package")))
		self.assertTrue(
			os.path.exists(vhtfm.get_site_path("packages", "test-package", "test_module_for_package"))
		)
		self.assertTrue(
			os.path.exists(
				vhtfm.get_site_path(
...
def make_test_package():
	if not vhtfm.db.exists("Package", "Test Package"):
		vhtfm.get_doc(
...
def make_test_module():
	if not vhtfm.db.exists("Module Def", "Test Module for Package"):
		vhtfm.get_doc(
			doctype="Module Def",
...
def make_test_doctype():
	if not vhtfm.db.exists("DocType", "Test DocType for Package"):
		vhtfm.get_doc(
			doctype="DocType",
...
def make_test_server_script():
	if not vhtfm.db.exists("Server Script", "Test Script for Package"):
		vhtfm.get_doc(
			...
			script='vhtfm.msgprint("Test")',
		).insert()


def make_test_web_page():
	if not vhtfm.db.exists("Web Page", "test-web-page-for-package"):
		vhtfm.get_doc(
			doctype="Web Page",
			module="Test Module for Package",

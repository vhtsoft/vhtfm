...
vhtfm.ui.form.on("Recorder", {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Suggest Optimizations"), () => {
			vhtfm.xcall("vhtfm.core.doctype.recorder.recorder.optimize", {
				recorder_id: frm.doc.name,
			});
		});

		vhtfm.realtime.on("recorder-analysis-complete", () => {
			frm.reload_doc();
		...
		index_grid.add_custom_button(__("Add Indexes"), function () {
			...
			if (!indexes_to_add.length) {
				vhtfm.toast(__("You need to select indexes you want to add first."));
				return;
			}
			vhtfm.xcall("vhtfm.core.doctype.recorder.recorder.add_indexes", {
...
vhtfm.ui.form.on("Recorder Query", "form_render", function (frm, cdt, cdn) {
	let row = vhtfm.get_doc(cdt, cdn);

...
import vhtfm
from vhtfm import _
from vhtfm.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from vhtfm.custom.doctype.property_setter.property_setter import make_property_setter
from vhtfm.model.document import Document
from vhtfm.recorder import RECORDER_REQUEST_HASH
from vhtfm.recorder import get as get_recorder_data
from vhtfm.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.core.doctype.recorder_query.recorder_query import RecorderQuery
		from vhtfm.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = vhtfm._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = vhtfm.as_json(i["stack"])
			i["explain_result"] = vhtfm.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=vhtfm.as_json(request.get("headers", {}), indent=4),
		form_dict=vhtfm.as_json(request.get("form_dict", {}), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		vhtfm.enqueue(_add_index, table=index["table"], column=index["column"])
	vhtfm.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	vhtfm.db.add_index(doctype, [column])
...
def optimize(recorder_id: str):
	vhtfm.only_for("Administrator")
	vhtfm.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = vhtfm.get_doc("Recorder", recorder_id)
	...
	data = vhtfm.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	vhtfm.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	vhtfm.publish_realtime("recorder-analysis-complete", user=vhtfm.session.user)
	vhtfm.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_vhtfm_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not vhtfm.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in vhtfm.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in vhtfm.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return vhtfm.db.sql(f"select count(distinct {column}) from {table}")[0][0]

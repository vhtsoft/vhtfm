vhtfm.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			vhtfm.xcall("vhtfm.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new vhtfm.ui.FileUploader({
				...
				on_success: (file) => {
					...
					vhtfm.call({
						method: "vhtfm.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			vhtfm.call({
				method: "vhtfm.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return vhtfm.xcall("vhtfm.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		vhtfm.xcall("vhtfm.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		vhtfm.prompt(
			...
			(values) => {
				vhtfm.xcall("vhtfm.recorder.start", values).then(() => {
					listview.refresh();

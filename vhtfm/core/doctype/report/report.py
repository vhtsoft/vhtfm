...
import vhtfm
import vhtfm.desk.query_report
from vhtfm import _, scrub
from vhtfm.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from vhtfm.core.doctype.page.page import delete_custom_role
from vhtfm.desk.reportview import append_totals_row
from vhtfm.model.document import Document
from vhtfm.modules import make_boilerplate
from vhtfm.modules.export_file import export_to_files
from vhtfm.utils import cint, cstr
from vhtfm.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from vhtfm.core.doctype.report_column.report_column import ReportColumn
		from vhtfm.core.doctype.report_filter.report_filter import ReportFilter
	...
	def validate(self):
		...
		if not self.module:
			self.module = vhtfm.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				vhtfm.session.user == "Administrator"
				and getattr(vhtfm.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if vhtfm.db.get_value("Report", self.name, "is_standard") == "Yes":
				vhtfm.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and vhtfm.session.user != "Administrator":
			vhtfm.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(vhtfm.local.conf, "developer_mode", 0))
			and not vhtfm.flags.in_migrate
			and not vhtfm.flags.in_patch
		):
			vhtfm.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = vhtfm.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		if has_common(vhtfm.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and vhtfm.conf.developer_mode:
	...
	def execute_query_report(self, filters):
		if not self.query:
			vhtfm.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in vhtfm.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in vhtfm.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report and not vhtfm.conf.developer_mode:
			vhtfm.enqueue(enable_prepared_report, report=self.name)

		vhtfm.cache.hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or vhtfm.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return vhtfm.get_attr(method_name)(vhtfm._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": vhtfm._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = vhtfm.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					vhtfm._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			columns.extend(
				...
				for df in vhtfm.get_meta(self.ref_doctype).fields
				if df.in_list_view
			)
		return columns
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = vhtfm._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for fieldname, doctype in columns:
			...
			if meta.get_field(fieldname):
				...
				field = vhtfm._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable: bool):
		if not self.has_permission("write"):
			vhtfm.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_enabled(report):
	return cint(vhtfm.db.get_value("Report", report, "prepared_report"))


def get_report_module_dotted_path(module, report_name):
	return (
		vhtfm.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
...
def enable_prepared_report(report: str):
	vhtfm.db.set_value("Report", report, "prepared_report", 1)

...
import vhtfm
from vhtfm.core.doctype.user_permission.test_user_permission import create_user
from vhtfm.custom.doctype.customize_form.customize_form import reset_customization
from vhtfm.desk.query_report import add_total_row, run, save_report
from vhtfm.desk.reportview import delete_report
from vhtfm.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if vhtfm.db.exists("Report", "User Activity Report"):
			vhtfm.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			vhtfm.get_doc(json.loads(f.read())).insert()

		report = vhtfm.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = vhtfm.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			vhtfm.set_user("test_report_owner@example.com")
			self.assertRaises(vhtfm.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = vhtfm.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=vhtfm.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = vhtfm.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		vhtfm.set_user("test@example.com")
		vhtfm.db.delete("Has Role", {"parent": vhtfm.session.user, "role": "Test Has Role"})
		vhtfm.db.commit()
		if not vhtfm.db.exists("Role", "Test Has Role"):
			vhtfm.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not vhtfm.db.exists("Report", "Test Report"):
			report = vhtfm.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = vhtfm.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		vhtfm.set_user("test@example.com")
		vhtfm.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		vhtfm.db.commit()  # nosemgrep
		if not vhtfm.db.exists("Report", "Test Custom Role Report"):
			report = vhtfm.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = vhtfm.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if vhtfm.db.exists("Report", "User Activity Report Without Sort"):
			vhtfm.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			vhtfm.get_doc(json.loads(f.read())).insert()

		report = vhtfm.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not vhtfm.db.exists("Report", report_name):
			report = vhtfm.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in vhtfm.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if vhtfm.db.exists("Report", report_name):
			vhtfm.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = vhtfm.get_all(doctype="Report", limit=1)
	...
	def test_cte_in_query_report(self):
		...
		if vhtfm.db.db_type == "mariadb":
			...
			self.assertRaises(vhtfm.PermissionError, report.execute_query_report, filters={})

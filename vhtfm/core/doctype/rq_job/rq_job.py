...
from vhtfm.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			vhtfm.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> vhtfm._dict:
	...
	if job_name == "vhtfm.utils.background_jobs.run_doc_method":
	...
	return vhtfm._dict(
		...
		exc_info=exc_info,
		arguments=vhtfm.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == vhtfm.local.site
...
@vhtfm.whitelist()
def stop_job(job_id):
	vhtfm.get_doc("RQ Job", job_id).stop_job()

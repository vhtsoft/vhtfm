...
import vhtfm
from vhtfm.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from vhtfm.installer import update_site_config
from vhtfm.tests import IntegrationTestCase, UnitTestCase, timeout
from vhtfm.utils import cstr, execute_in_shell
from vhtfm.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(IntegrationTestCase):
	BG_JOB = "vhtfm.core.doctype.rq_job.test_rq_job.test_func"
	...
	def test_serialization(self):
		job = vhtfm.enqueue(method=self.BG_JOB, queue="short")
		rq_job = vhtfm.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		vhtfm.conf.rq_job_failure_ttl = 600
		job = vhtfm.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = vhtfm.enqueue(method=test_func, queue="short")
		rq_job = vhtfm.get_doc("RQ Job", job.id)
		self.assertEqual(rq_job.job_name, "vhtfm.core.doctype.rq_job.test_rq_job.test_func")

	@timeout
	def test_get_list_filtering(self):
		# Check failed job clearning and filtering
		remove_failed_jobs()
		jobs = vhtfm.get_all("RQ Job", {"status": "failed"})
		...
		# Fail a job
		job = vhtfm.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		non_failed_jobs = vhtfm.get_all("RQ Job", {"status": ("!=", "failed")})
		self.assertGreaterEqual(len(non_failed_jobs), 1)

		# Create a slow job and check if it's stuck in "Started"
		job = vhtfm.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = vhtfm.enqueue(method=self.BG_JOB, queue="short")
		vhtfm.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				vhtfm.enqueue(self.BG_JOB, sleep=1, queue=q)

		_, stderr = execute_in_shell("fox worker --queue short,default --burst", check_exit_code=True)
	...
	def test_multi_queue_burst_consumption_worker_pool(self):
		...
		_, stderr = execute_in_shell(
			"fox worker-pool --queue short,default --burst --num-workers=4", check_exit_code=True
		)
		self.assertIn("quitting", cstr(stderr))

	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = vhtfm.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = vhtfm.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = vhtfm.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = vhtfm.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = vhtfm.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = vhtfm.generate_hash()

		vhtfm.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_memory_usage(self):
		if vhtfm.db.db_type != "mariadb":
			return
		job = vhtfm.enqueue("vhtfm.utils.data._get_rss_memory_usage")
		self.check_status(job, "finished")
		...
		# Refer this PR: https://github.com/vhtfm/vhtfm/pull/21467
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [vhtfm.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]

...
vhtfm.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new vhtfm.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		vhtfm
			.call("vhtfm.core.doctype.server_script.server_script.get_autocompletion_items")
			.then((r) => r.message)
			.then((items) => {
	...
	check_safe_exec(frm) {
		vhtfm.xcall("vhtfm.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://vhtfmframework.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/vhtfm/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in Vhtfm Payments</a> for details.</p>
<pre>
	<code>
...
if vhtfm.form_dict.message == "ping":
	vhtfm.response['message'] = "pong"
else:
	vhtfm.response['message'] = "ok"
</code></pre>

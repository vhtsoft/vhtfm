...
# this is a separate file since it is imported in vhtfm.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			vhtfm.get_cached_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if vhtfm.flags.in_patch and not vhtfm.db.table_exists("Server Script"):
		return {}

	script_map = vhtfm.cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = vhtfm.get_all(
			"Server Script",
		...
		vhtfm.cache.set_value("server_script_map", script_map)

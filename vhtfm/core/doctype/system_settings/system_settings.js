vhtfm.ui.form.on("System Settings", {
	refresh: function (frm) {
		vhtfm.call({
			method: "vhtfm.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				vhtfm.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", vhtfm.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					vhtfm.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					vhtfm.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, vhtfm.boot.sysdefaults.language, vhtfm.boot.user.language],
			[frm.doc.rounding_method, vhtfm.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == vhtfm.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		vhtfm.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", vhtfm.boot.sysdefaults.rounding_method);
			}
		);
	},

...
from vhtfm.core.doctype.user.user import (
	User,
...
from vhtfm.desk.notifications import extract_mentions
...
user_module = vhtfm.core.doctype.user.user
...
class TestUser(IntegrationTestCase):
	def tearDown(self):
		...
		vhtfm.db.set_single_value("System Settings", "enable_password_policy", 0)
		vhtfm.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		user_id = vhtfm.generate_hash() + "@example.com"
		new_user = vhtfm.get_doc(doctype="User", email=user_id, first_name="Tester").insert()
		self.assertEqual(new_user.user_type, "Website User")

		# social login userid for vhtfm
		self.assertTrue(new_user.social_logins[0].userid)
		self.assertEqual(new_user.social_logins[0].provider, "vhtfm")
		...
		vhtfm.delete_doc("User", new_user.name)

	def test_delete(self):
		vhtfm.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(vhtfm.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		vhtfm.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if vhtfm.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = vhtfm.copy_doc(self.globalTestRecords["User"][1])
		user.email = "_test@example.com"
		user.insert()

		vhtfm.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not vhtfm.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		vhtfm.copy_doc(self.globalTestRecords["Role"][1]).insert()

	def test_get_value(self):
		self.assertEqual(vhtfm.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(vhtfm.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			vhtfm.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			vhtfm.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = vhtfm.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			vhtfm.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(vhtfm.db.get_value("User", "xxxtest@example.com"), None)

		vhtfm.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(vhtfm.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(vhtfm.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val")

	def test_high_permlevel_validations(self):
		user = vhtfm.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = vhtfm.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		vhtfm.delete_doc("User", new_user.name)
		self.assertFalse(vhtfm.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		vhtfm.db.set_single_value("System Settings", "enable_password_policy", 1)
		vhtfm.db.set_single_value("System Settings", "minimum_password_score", 2)
		...
		self.assertRaises(vhtfm.exceptions.ValidationError, handle_password_test_fail, result["feedback"])
		self.assertRaises(
			vhtfm.exceptions.ValidationError, handle_password_test_fail, result
		...
		self.assertRaises(vhtfm.exceptions.ValidationError, user.save)
		user.reload()
	...
	def test_comment_mentions(self):
		...
		vhtfm.delete_doc("User Group", "Team")
		doc = vhtfm.get_doc(
			{
				"doctype": "User Group",
				"name": "Team",
	...
	def test_rate_limiting_for_reset_password(self):
		url = get_url()
		data = {"cmd": "vhtfm.core.doctype.user.user.reset_password", "user": "test@test.com"}

		# Clear rate limit tracker to start fresh
		key = f"rl:{data['cmd']}:{data['user']}"
		vhtfm.cache.delete(key)
	...
	def test_user_rename(self):
		...
		vhtfm.rename_doc("User", user.name, new_name)
		self.assertTrue(vhtfm.db.exists("Notification Settings", new_name))

		vhtfm.delete_doc("User", new_name)

	def test_signup(self):
		import vhtfm.website.utils
		...
		with patch.object(user_module, "is_signup_disabled", return_value=True):
			self.assertRaisesRegex(
				vhtfm.exceptions.ValidationError,
				"Sign Up is disabled",
				sign_up,
				random_user,
				random_user_name,
		...
		self.assertEqual(vhtfm.cache.hget("redirect_after_login", random_user), "/welcome")

		# re-register
		self.assertTupleEqual(sign_up(random_user, random_user_name, "/welcome"), (0, "Already Registered"))

		# disabled user
		user = vhtfm.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.vhtfm.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				vhtfm.exceptions.ValidationError,
				"Throttled",
				sign_up,
	...
	def test_reset_password(self):
		...
		vhtfm.local.request_ip = "127.0.0.69"

		vhtfm.set_user("testpassword@example.com")
		test_user = vhtfm.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(vhtfm.exceptions.AuthenticationError, verify_password, old_password)
		...
		# test API endpoint
		with patch.object(user_module.vhtfm, "sendmail") as sendmail:
			vhtfm.clear_messages()
			test_user = vhtfm.get_doc("User", "test2@example.com")
			...
			update_password(old_password, old_password=new_password)
			self.assertEqual(
				vhtfm.message_log[0].get("message"),
...
class TestImpersonation(VhtfmAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("vhtfm.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("vhtfm.auth.get_logged_user"))
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		first_name = first_name or vhtfm.generate_hash()
		email = email or (first_name + "@example.com")
		user: User = vhtfm.new_doc(
			"User",
		...
		commit and vhtfm.db.commit()


def delete_contact(user):
	vhtfm.db.delete("Contact", {"email_id": user})
	vhtfm.db.delete("Contact Email", {"email_id": user})

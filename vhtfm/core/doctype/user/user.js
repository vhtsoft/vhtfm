vhtfm.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, vhtfm.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(vhtfm.all_timezones);
		};

		if (!vhtfm.all_timezones) {
			vhtfm.call({
				method: "vhtfm.core.doctype.user.user.get_timezones",
				callback: function (r) {
					vhtfm.all_timezones = r.message.timezones;
					update_tz_options();
				},
			});
		} else {
	...
	module_profile: function (frm) {
		if (frm.doc.module_profile) {
			vhtfm.call({
				method: "vhtfm.core.doctype.user.user.get_module_profile",
				args: {
	...
	onload: function (frm) {
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new vhtfm.RoleEditor(
					role_area,
					frm,
	...
	refresh: function (frm) {
		...
		vhtfm.xcall("vhtfm.apps.get_apps").then((r) => {
			let apps = r?.map((r) => r.name) || [];
			frm.set_df_property("default_app", "options", [" ", ...apps]);
		});

		if (frm.is_new()) {
			frm.set_value("time_zone", vhtfm.sys_defaults.time_zone);
		}
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						vhtfm.set_route("query-report", "Permitted Documents For User", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
				...
				frm.add_custom_button(
					...
					() =>
						vhtfm.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
			...
			frm.add_custom_button(
				...
				function () {
					vhtfm.call({
						method: "vhtfm.core.doctype.user.user.reset_password",
						args: {
							user: frm.doc.name,
						},
					});
			...
			if (vhtfm.user.has_role("System Manager")) {
				vhtfm.db.get_single_value("LDAP Settings", "enabled").then((value) => {
								...
								const d = new vhtfm.ui.Dialog({
									...
									primary_action: (values) => {
										...
										if (values.new_password !== values.confirm_password) {
											vhtfm.throw(__("Passwords do not match!"));
										}
										vhtfm.call(
											"vhtfm.integrations.doctype.ldap_settings.ldap_settings.reset_password",
											{
												user: frm.doc.email,
			...
			if (
				cint(vhtfm.boot.sysdefaults.enable_two_factor_auth) &&
				(vhtfm.session.user == doc.name || vhtfm.user.has_role("System Manager"))
			) {
				frm.add_custom_button(
					__("Reset OTP Secret"),
					function () {
						vhtfm.call({
							method: "vhtfm.twofactor.reset_otp_secret",
							args: {
								user: frm.doc.name,
							},
						});
			...
			if (vhtfm.session.user == doc.name) {
				// update display settings
				if (doc.user_image) {
					vhtfm.boot.user_info[vhtfm.session.user].image = vhtfm.utils.get_file_link(
						doc.user_image
					);
				}
			}
		}
		if (frm.doc.user_emails && vhtfm.model.can_create("Email Account")) {
			var found = 0;
			...
			if (!found) {
				frm.add_custom_button(__("Create User Email"), function () {
					if (!frm.doc.email) {
						vhtfm.msgprint(__("Email is mandatory to create User Email"));
						return;
					}
		...
		if (vhtfm.route_flags.unsaved === 1) {
			delete vhtfm.route_flags.unsaved;
	...
	create_user_email: function (frm) {
		vhtfm.call({
			method: "vhtfm.core.doctype.user.user.has_email_account",
			args: {
				email: frm.doc.email,
			},
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					vhtfm.model.with_doctype("Email Account", function (doc) {
						doc = vhtfm.model.get_new_doc("Email Account");
						vhtfm.route_flags.linked_user = frm.doc.name;
						vhtfm.route_flags.delete_user_from_locals = true;
						vhtfm.set_route("Form", "Email Account", doc.name);
					});
				} else {
					vhtfm.route_flags.create_user_account = frm.doc.name;
					vhtfm.set_route("Form", "Email Account", r.message[0]["name"]);
				}
			},
		});
	},
	generate_keys: function (frm) {
		vhtfm.call({
			method: "vhtfm.core.doctype.user.user.generate_keys",
			args: {
				user: frm.doc.name,
			},
			callback: function (r) {
				if (r.message) {
					vhtfm.msgprint(__("Save API Secret: {0}", [r.message.api_secret]));
					frm.reload_doc();
				}
			},
		});
	...
	after_save: function (frm) {
		...
		if (doc.name === vhtfm.session.user && attr_tuples.some(has_effectively_changed)) {
			vhtfm.msgprint(__("Refreshing..."));
		...
		if (
			...
			frm.add_custom_button(__("Impersonate"), () => {
				...
				vhtfm.prompt(
					...
					(values) => {
						vhtfm
							.xcall("vhtfm.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
function has_access_to_edit_user() {
	return has_common(vhtfm.user_roles, get_roles_for_editing_user());
}

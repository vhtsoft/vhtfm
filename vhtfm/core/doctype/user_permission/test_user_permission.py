...
from vhtfm.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from vhtfm.permissions import add_permission, has_user_permission
from vhtfm.tests import IntegrationTestCase, UnitTestCase
from vhtfm.website.doctype.blog_post.test_blog_post import make_test_blog
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		vhtfm.db.delete("User Permission", {"user": ("in", test_users)})
		vhtfm.delete_doc_if_exists("DocType", "Person")
		vhtfm.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		vhtfm.delete_doc_if_exists("DocType", "Doc A")
		vhtfm.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(vhtfm.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		vhtfm.db.delete("User Permission", filters={"for_value": test_blog.name})
		vhtfm.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not vhtfm.db.exists("Blog Category", category):
				vhtfm.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		vhtfm.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = vhtfm.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = vhtfm.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = vhtfm.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = vhtfm.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = vhtfm.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = vhtfm.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not vhtfm.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = vhtfm.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = vhtfm.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(vhtfm.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(vhtfm.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		vhtfm.set_user(user.name)
		visible_names = vhtfm.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = vhtfm.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(vhtfm.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(vhtfm.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = vhtfm.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		vhtfm.set_user("new_doc_test@example.com")

		new_doc = vhtfm.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on vhtfm.new_doc. with *user* default value"""
		...
		if not vhtfm.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		vhtfm.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if vhtfm.db.exists("User", email):
		return vhtfm.get_doc("User", email)

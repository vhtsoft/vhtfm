...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = vhtfm.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = vhtfm.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = vhtfm.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if vhtfm.db.exists("User Type", user_type):
		vhtfm.delete_doc("User Type", user_type)

	user_type_limit = {vhtfm.scrub(user_type): 1}
...
def create_role():
	if not vhtfm.db.exists("Role", "_Test User Type"):
		vhtfm.get_doc(

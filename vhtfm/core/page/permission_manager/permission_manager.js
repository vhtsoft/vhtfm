vhtfm.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = vhtfm.ui.make_app_page({
		parent: wrapper,
	...
	$(vhtfm.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new vhtfm.PermissionEngine(wrapper);
};

vhtfm.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

vhtfm.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				vhtfm.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return vhtfm.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (vhtfm.get_route()[1]) {
			this.doctype_select.set_value(vhtfm.get_route()[1]);
		} else if (vhtfm.route_options) {
			if (vhtfm.route_options.doctype) {
				this.doctype_select.set_value(vhtfm.route_options.doctype);
			}
			if (vhtfm.route_options.role) {
				this.role_select.set_value(vhtfm.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = vhtfm.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return vhtfm
				.call({
					module: "vhtfm.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(vhtfm.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${vhtfm.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return vhtfm.call({
					...
					callback: (r) => {
						if (r.exc) {
							vhtfm.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			vhtfm.route_options = { allow: this.get_doctype() || "" };
	...
	get_link_fields(doctype) {
		return vhtfm.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",

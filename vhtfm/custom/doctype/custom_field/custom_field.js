...
vhtfm.ui.form.on("Custom Field", {
	...
	dt: function (frm) {
		...
		return vhtfm.call({
			method: "vhtfm.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && vhtfm.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				vhtfm.prompt(
					...
					function (data) {
						vhtfm
							.xcall(
								"vhtfm.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
vhtfm.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";

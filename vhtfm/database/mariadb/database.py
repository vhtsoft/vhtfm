...
from vhtfm.database.mariadb.schema import MariaDBTable
from vhtfm.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if vhtfm.conf.local_infile:
			conn_settings["local_infile"] = vhtfm.conf.local_infile

		if vhtfm.conf.db_ssl_ca and vhtfm.conf.db_ssl_cert and vhtfm.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": vhtfm.conf.db_ssl_ca,
				"cert": vhtfm.conf.db_ssl_cert,
				"key": vhtfm.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via vhtfm.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = vhtfm.as_unicode(escape_string(vhtfm.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> vhtfm._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = vhtfm.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			vhtfm.cache.set_value("db_tables", tables)

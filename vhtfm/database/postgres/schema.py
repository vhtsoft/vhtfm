import vhtfm
from vhtfm import _
from vhtfm.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = vhtfm.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			vhtfm.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			vhtfm.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{vhtfm.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				vhtfm.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				vhtfm.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				vhtfm.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				vhtfm.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if vhtfm.db.is_duplicate_fieldname(e):
				vhtfm.throw(str(e))
			elif vhtfm.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and vhtfm.db.get_column_type(self.doctype, "name") != "uuid":
			if not vhtfm.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and vhtfm.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({vhtfm.db.VARCHAR_LEN})"

...
import vhtfm
from vhtfm.cache_manager import clear_defaults_cache, common_default_keys
...
def set_user_default(key, value, user=None, parenttype=None):
	set_default(key, value, user or vhtfm.session.user, parenttype)


def add_user_default(key, value, user=None, parenttype=None):
	add_default(key, value, user or vhtfm.session.user, parenttype)


def get_user_default(key, user=None):
	user_defaults = get_defaults(user or vhtfm.session.user)
	...
	if is_a_user_permission_key(key):
		if d and isinstance(d, list | tuple) and len(d) == 1:
			...
			d = user_defaults.get(vhtfm.scrub(key), None)
...
def is_a_user_permission_key(key):
	return ":" not in key and key != vhtfm.scrub(key)
...
def not_in_user_permission(key, value, user=None):
	...
	user_permission = get_user_permissions(user).get(vhtfm.unscrub(key)) or []
...
def get_user_permissions(user=None):
	...
	"""Return vhtfm.core.doctype.user_permissions.user_permissions._get_user_permissions (kept for backward compatibility)"""
...
def get_defaults(user=None):
	...
	if not user:
		user = vhtfm.session.user if vhtfm.session else "Guest"
...
def clear_user_default(key, user=None):
	clear_default(key, parent=user or vhtfm.session.user)
...
def set_default(key, value, parent, parenttype="__default"):
	...
	if key_exists:
		vhtfm.db.delete("DefaultValue", {"defkey": key, "parent": parent})
...
def clear_default(key=None, value=None, parent=None, name=None, parenttype=None):
	...
	vhtfm.db.delete("DefaultValue", filters)
...
def get_defaults_for(parent="__default"):
	"""get all defaults"""
	defaults = vhtfm.cache.hget("defaults", parent)
	...
	if defaults is None:
		...
		vhtfm.cache.hset("defaults", parent, defaults)
...
def _clear_cache(parent):
	if vhtfm.flags.in_install:
		return
	vhtfm.clear_cache(user=parent if parent not in common_default_keys else None)

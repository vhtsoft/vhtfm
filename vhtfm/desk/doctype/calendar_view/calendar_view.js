...
vhtfm.ui.form.on("Calendar View", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Show Calendar"), () =>
				vhtfm.set_route("List", frm.doc.reference_doctype, "Calendar", frm.doc.name)
			);
		}
	},
	reference_doctype: function (frm) {
		...
		vhtfm.model.with_doctype(reference_doctype, () => {
			const meta = vhtfm.get_meta(reference_doctype);

			const subject_options = meta.fields
				.filter((df) => !vhtfm.model.no_value_type.includes(df.fieldtype))
				.map((df) => df.fieldname);

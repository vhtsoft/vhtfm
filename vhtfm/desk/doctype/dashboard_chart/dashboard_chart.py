...
import vhtfm
from vhtfm import _
from vhtfm.boot import get_allowed_report_names
from vhtfm.model.document import Document
from vhtfm.model.naming import append_number_if_name_exists
from vhtfm.modules.export_file import export_to_files
from vhtfm.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from vhtfm.utils.dashboard import cache_source
from vhtfm.utils.data import format_date
from vhtfm.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [vhtfm.db.escape(doctype) for doctype in vhtfm.permissions.get_doctypes_with_read()]
	allowed_reports = [vhtfm.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
		vhtfm.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.roles:
		...
		allowed_doctypes = vhtfm.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart: DashboardChart = vhtfm.get_doc("Dashboard Chart", chart_name)
	else:
		chart = vhtfm._dict(vhtfm.parse_json(chart))
	...
	filters = vhtfm.parse_json(filters) or vhtfm.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = vhtfm.new_doc("Dashboard Chart")
	...
	if vhtfm.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard = vhtfm.get_doc("Dashboard", args.dashboard)
	dashboard_link = vhtfm.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = vhtfm.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_group_by_chart_config(chart, filters) -> dict | None:
	...
	group_by_field_field = vhtfm.get_meta(doctype).get_field(
		group_by_field
	)  # get info about @group_by_field

	if data and group_by_field_field.fieldtype == "Link":  # if @group_by_field is link
		title_field = vhtfm.get_meta(group_by_field_field.options)  # get title field
		if title_field.title_field:  # if has title_field
			for item in data:  # replace chart labels from name to title value
				item.name = vhtfm.get_value(group_by_field_field.options, item.name, title_field.title_field)
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": vhtfm.session.user, "is_public": 1}
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from vhtfm.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		vhtfm.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not vhtfm.conf.developer_mode and self.is_standard:
			vhtfm.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			vhtfm.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and vhtfm.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			vhtfm.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				vhtfm.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				vhtfm.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				vhtfm.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if vhtfm.get_meta(self.document_type).issingle:
			vhtfm.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				vhtfm.throw(_("Invalid json added in the custom options: {0}").format(error))

...
from vhtfm.desk.doctype.dashboard_chart.dashboard_chart import get
from vhtfm.tests import IntegrationTestCase, UnitTestCase
from vhtfm.utils import formatdate, get_last_day, getdate
from vhtfm.utils.dateutils import get_period, get_period_ending
...
class TestDashboardChart(IntegrationTestCase):
	...
	def test_dashboard_chart(self):
		if vhtfm.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if vhtfm.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		vhtfm.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if vhtfm.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			vhtfm.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		vhtfm.db.delete("Error Log")

		# create one data point
		vhtfm.get_doc(doctype="Error Log", creation="2018-06-01 00:00:00").insert()
	...
	def test_group_by_chart_type(self):
		if vhtfm.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		vhtfm.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = vhtfm.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if vhtfm.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if vhtfm.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if vhtfm.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			vhtfm.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		vhtfm.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(vhtfm.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(vhtfm.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_record(doctype_name, title, date, number):
	...
	if not vhtfm.db.exists(doctype_name, {"title": doc.title}):

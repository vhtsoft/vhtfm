...
def get_config(name: str) -> str:
	doc: DashboardChartSource = vhtfm.get_doc("Dashboard Chart Source", name)
...
class DashboardChartSource(Document):
	...
	def on_update(self):
		if not vhtfm.request:
			return

		if not vhtfm.conf.developer_mode:
			vhtfm.throw(_("Creation of this document is only permitted in developer mode."))
	...
	def on_trash(self):
		...
		vhtfm.db.after_commit.add(self.delete_folder)
	...
	def get_folder_path(self) -> Path:
		...
		return Path(get_module_path(self.module)) / FOLDER_NAME / vhtfm.scrub(self.name)

...
vhtfm.ui.form.on("Event", {
	...
	refresh: function (frm) {
		if (frm.doc.event_participants) {
			frm.doc.event_participants.forEach((value) => {
				frm.add_custom_button(
					...
					function () {
						vhtfm.set_route("Form", value.reference_doctype, value.reference_docname);
					},
					__("Participants")
				);
			});
		...
		frm.add_custom_button(
			__("Add Contacts"),
			function () {
				new vhtfm.desk.eventParticipants(frm, "Contact");
			},
			__("Add Participants")
		);
		...
		if (
			...
			vhtfm.datetime.now_date() <= ends_on_date
		) {
...
vhtfm.ui.form.on("Event Participants", {
	event_participants_remove: function (frm, cdt, cdn) {
		if (cdt && !cdn.includes("new-event-participants")) {
			vhtfm.call({
				type: "POST",
				method: "vhtfm.desk.doctype.event.event.delete_communication",
				args: {
					event: frm.doc,
...
vhtfm.desk.eventParticipants = class eventParticipants {

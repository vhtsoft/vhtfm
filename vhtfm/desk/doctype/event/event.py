...
import vhtfm
from vhtfm import _
from vhtfm.contacts.doctype.contact.contact import get_default_contact
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			vhtfm.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			vhtfm.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				vhtfm.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				if comms := vhtfm.get_all("Communication", filters=filters, fields=["name"], distinct=True):
					for comm in comms:
						communication = vhtfm.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = vhtfm.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = vhtfm.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				vhtfm.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@vhtfm.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = vhtfm.get_doc(reference_doctype, reference_docname)
	...
	comms = vhtfm.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = vhtfm.get_doc("Communication", comm.name).delete()
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={vhtfm.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			vhtfm.set_user_lang(user.name, user.language)
			...
			vhtfm.sendmail(
				...
				subject=vhtfm._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[vhtfm._("Events in Today's Calendar"), "blue"],
			)


@vhtfm.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[vhtfm._dict]:
	if not user:
		user = vhtfm.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	...
	if participations:
		for participation in participations:
			if delete_event:
				vhtfm.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = vhtfm.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					vhtfm.db.delete("Event", {"name": participation.parent})
					vhtfm.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = vhtfm.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			vhtfm.db.set_value("Event", event.name, "status", "Closed")

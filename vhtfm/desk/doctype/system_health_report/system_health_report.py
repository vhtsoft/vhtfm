...
from vhtfm.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from vhtfm.utils.caching import redis_cache
from vhtfm.utils.data import add_to_date
from vhtfm.utils.scheduler import get_scheduler_status, get_scheduler_tick
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				vhtfm.log(vhtfm.get_traceback())
				# nosemgrep
				vhtfm.msgprint(
					f"System Health check step {vhtfm.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from vhtfm.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from vhtfm.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from vhtfm.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from vhtfm.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = vhtfm.enqueue("vhtfm.ping", at_front=True).id
		self.background_jobs_check = "queued"
		self.scheduler_status = get_scheduler_status().get("status")
		workers = vhtfm.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		failing_jobs = vhtfm.db.multisql(
			{
		...
		for job_type in vhtfm.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = vhtfm.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = vhtfm.db.count("Email Queue", filters)
		self.pending_emails = vhtfm.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = vhtfm.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = vhtfm.db.count("Unhandled Email", filters)
		self.handled_emails = vhtfm.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = vhtfm.db.count("Error Log", filters)

		top_errors = vhtfm.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = vhtfm.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = vhtfm.db.sql("select version()")[0][0]

		if vhtfm.db.db_type == "mariadb":
			self.bufferpool_size = vhtfm.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = vhtfm.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(vhtfm.cache.get_keys(""))
		self.cache_memory_usage = vhtfm.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from vhtfm.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = vhtfm.db.count("User", {"enabled": 1})
		self.new_users = vhtfm.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = vhtfm.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = vhtfm.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	vhtfm.only_for("System Manager")
	try:
		return vhtfm.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(vhtfm.get_site_path(*path))

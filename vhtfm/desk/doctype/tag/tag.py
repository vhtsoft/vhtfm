...
from vhtfm.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		vhtfm.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
...
def get_tagged_docs(doctype, tag):
	...
	return (vhtfm.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.name)).run()


@vhtfm.whitelist()
def get_tags(doctype, txt):
	tag = vhtfm.get_list("Tag", filters=[["name", "like", f"%{txt}%"]])
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return vhtfm.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""Return tag for a particular item."""
		return (vhtfm.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if tag not in tl:
			tl.append(tag)
			if not vhtfm.db.exists("Tag", tag):
				vhtfm.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
...
def delete_tags_for_document(doc):
	...
	"""
	if not vhtfm.db.table_exists("Tag Link"):
		return

	vhtfm.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
...
def update_tags(doc, tags):
	...
	for tag in deleted_tags:
		vhtfm.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name, "tag": tag})
...
def get_tags_list_for_awesomebar():
	return vhtfm.get_list("Tag", pluck="name", order_by=None)

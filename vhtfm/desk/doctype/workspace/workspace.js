...
vhtfm.ui.form.on("Workspace", {
	setup: function () {
		vhtfm.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/app/${
			frm.doc.public
				? vhtfm.router.slug(frm.doc.title)
				: "private/" + vhtfm.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			(frm.doc.for_user && frm.doc.for_user !== vhtfm.session.user) ||
			(frm.doc.public && !vhtfm.user.has_role("Workspace Manager"))
		) {

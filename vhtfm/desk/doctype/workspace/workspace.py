...
import vhtfm
from vhtfm import _
from vhtfm.desk.desktop import get_workspace_sidebar_items, save_new_widget
from vhtfm.desk.utils import validate_route_conflict
from vhtfm.model.document import Document
from vhtfm.model.rename_doc import rename_doc
from vhtfm.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from vhtfm.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from vhtfm.desk.doctype.workspace_custom_block.workspace_custom_block import WorkspaceCustomBlock
		from vhtfm.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from vhtfm.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from vhtfm.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from vhtfm.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		...
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			vhtfm.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			vhtfm.throw(_("Content data shoud be a list"))

		for d in self.get("links"):
			if d.link_type == "Report" and d.is_query_report != 1:
				d.report_ref_doctype = vhtfm.get_value("Report", d.link_to, "ref_doctype")
	...
	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			vhtfm.cache.hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if vhtfm.conf.developer_mode and self.public:
			if self.module:
	...
	def on_trash(self):
		if self.public and not is_workspace_manager():
			vhtfm.throw(_("You need to be Workspace Manager to delete a public workspace."))

	def after_delete(self):
		if disable_saving_as_public():
			return

		if self.module and vhtfm.conf.developer_mode:
			delete_folder(self.module, "Workspace", self.title)

	@staticmethod
	def get_module_wise_workspaces():
		workspaces = vhtfm.get_all(
			"Workspace",
			fields=["name", "module"],
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == vhtfm.get_system_settings("country")
				):
				...
				current_card = link
				card_links = []
			elif not link.get("only_for") or link.get("only_for") == vhtfm.get_system_settings("country"):
...
def get_report_type(report):
	report_type = vhtfm.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		not page.get("public") and page.get("for_user") != vhtfm.session.user and not is_workspace_manager()
	):
		vhtfm.throw(_("Cannot create private workspace of other users"), vhtfm.PermissionError)

	elif not vhtfm.has_permission(doctype="Workspace", ptype="create"):
		vhtfm.flags.error_message = _("User {0} does not have the permission to create a Workspace.").format(
			vhtfm.bold(vhtfm.session.user)
		)
		raise vhtfm.PermissionError

	doc = vhtfm.new_doc("Workspace")
...
def save_page(name, public, new_widgets, blocks):
	public = vhtfm.parse_json(public)

	doc = vhtfm.get_doc("Workspace", name)
	doc.content = blocks
...
def update_page(name, title, icon, indicator_color, parent, public):
	...
	if not doc.get("public") and doc.get("for_user") != vhtfm.session.user and not is_workspace_manager():
		vhtfm.throw(
	...
	if doc:
		child_docs = vhtfm.get_all("Workspace", filters={"parent_page": doc.title, "public": doc.public})
		doc.title = title
		doc.icon = icon
		doc.indicator_color = indicator_color
		doc.parent_page = parent
		if doc.public != public:
			doc.sequence_id = vhtfm.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or vhtfm.session.user
		...
		if child_docs:
			for child in child_docs:
				child_doc = vhtfm.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or vhtfm.session.user
...
def last_sequence_id(doc):
	doc_exists = vhtfm.db.exists({"doctype": "Workspace", "public": doc.public, "for_user": doc.for_user})
...
def get_page_list(fields, filters):
	return vhtfm.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in vhtfm.get_roles()

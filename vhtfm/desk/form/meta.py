...
import vhtfm
from vhtfm import _
from vhtfm.build import scrub_html_template
from vhtfm.model.meta import Meta
from vhtfm.model.utils import render_include
from vhtfm.modules import get_module_path, load_doctype_module, scrub
from vhtfm.utils import get_fox_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not vhtfm.conf.developer_mode
	if cached:
		meta = vhtfm.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			vhtfm.cache.hset("doctype_form_meta", doctype, meta)
	else:
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(vhtfm.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = vhtfm.get_system_settings("country")
	...
	def _add_code(self, path, fieldname):
		...
		if js:
			...
			asset_path = path.replace(fox_path, "")
			comment = f"\n\n/* Adding {asset_path} */\n\n"
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = vhtfm.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			vhtfm.bold(df.fieldname), vhtfm.bold(df.options)
		)
		...
		vhtfm.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = vhtfm.get_meta(df.options).document_type
				except vhtfm.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = vhtfm.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and vhtfm.db.exists("Workflow", workflow_name):
			workflow = vhtfm.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(vhtfm.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(vhtfm.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = vhtfm.qb.DocType("Workspace Shortcut")
		Workspace = vhtfm.qb.DocType("Workspace")
		shortcut = (
			vhtfm.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = vhtfm.qb.DocType("Workspace Link")
			link = (
				vhtfm.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in vhtfm.get_installed_apps():
		code_hook = vhtfm.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = vhtfm.get_app_path(app_name, *file.strip("/").split("/"))

...
from vhtfm.desk.form.load import run_onload
...
from vhtfm.utils.telemetry import capture_doc


@vhtfm.whitelist()
def savedocs(doc, action):
	"""save / submit / update doclist"""
	doc = vhtfm.get_doc(json.loads(doc))
	...
	vhtfm.msgprint(vhtfm._("Saved"), indicator="green", alert=True)
...
def cancel(doctype=None, name=None, workflow_state_fieldname=None, workflow_state=None):
	...
	doc.cancel()
	send_updated_docs(doc)
	vhtfm.msgprint(vhtfm._("Cancelled"), indicator="red", alert=True)
...
def discard(doctype: str, name: str | int):
	...
	doc.discard()
	send_updated_docs(doc)
	vhtfm.msgprint(vhtfm._("Discarded"), indicator="red", alert=True)

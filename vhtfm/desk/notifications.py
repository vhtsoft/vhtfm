...
def get_notifications():
	...
	if vhtfm.flags.in_install or not vhtfm.get_system_settings("setup_complete"):
		return out
	...
	for name in groups:
		count = vhtfm.cache.hget("notification_count:" + name, vhtfm.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = vhtfm.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = vhtfm.get_list(
							d, fields=["count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
						result = vhtfm.get_attr(condition)()

				except vhtfm.PermissionError:
					vhtfm.clear_messages()
					pass
				# vhtfm.msgprint("Permission Error in notifications for {0}".format(d))
					...
					vhtfm.cache.hset("notification_count:" + d, vhtfm.session.user, result)
...
def clear_notifications(user=None):
	...
	if user:
		vhtfm.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		vhtfm.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	vhtfm.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	vhtfm.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(vhtfm.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	user = vhtfm.session.user or "Guest"
	...
	return vhtfm.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if vhtfm.flags.in_migrate or vhtfm.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = vhtfm.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(vhtfm.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			vhtfm.bold(sender_fullname), vhtfm.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = vhtfm.get_cached_doc("User Group", mention["data-id"])

import BaseTimeline from "../../../public/js/vhtfm/form/footer/base_timeline";
vhtfm.provide("vhtfm.energy_points");
...
class UserProfile {
	...
	show() {
		...
		this.user_id = route[1] || vhtfm.session.user;
		vhtfm.dom.freeze(__("Loading user profile") + "...");
		vhtfm.db.exists("User", this.user_id).then((exists) => {
			vhtfm.dom.unfreeze();
			if (exists) {
				this.make_user_profile();
			} else {
				vhtfm.msgprint(__("User does not exist"));
			}
		});
	}

	make_user_profile() {
		this.user = vhtfm.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(vhtfm.render_template("user_profile"));
	...
	show_user_search_dialog() {
		let dialog = new vhtfm.ui.Dialog({
			...
			primary_action: ({ user }) => {
				dialog.hide();
				vhtfm.set_route("user-profile", user);
			},
		});
		dialog.show();
	}
	...
	render_heatmap() {
		this.heatmap = new vhtfm.Chart(".performance-heatmap", {
			type: "heatmap",
	...
	update_heatmap_data(date_from) {
		vhtfm
			.xcall("vhtfm.desk.page.user_profile.user_profile.get_energy_points_heatmap_data", {
				user: this.user_id,
				date: date_from || vhtfm.datetime.year_start(),
			})
			.then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new vhtfm.Chart(".performance-line-chart", {
			type: "line",
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		vhtfm
			.xcall("vhtfm.desk.doctype.dashboard_chart.dashboard_chart.get", {
	...
	render_percentage_chart(field, title) {
		vhtfm
			.xcall(
				"vhtfm.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data",
				{
					user: this.user_id,
					field: field,
				}
			...
			.then((chart) => {
				if (chart.labels.length) {
					this.percentage_chart = new vhtfm.Chart(".performance-percentage-chart", {
						type: "percentage",
						data: {
							labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		vhtfm.dashboard_utils.render_chart_filters(
			filters,
			"chart-filter",
			".line-chart-options",
			1
	...
	create_heatmap_chart_filters() {
		let filters = [
			{
				label: vhtfm.dashboard_utils.get_year(vhtfm.datetime.now_date()),
				options: vhtfm.dashboard_utils.get_years_since_creation(
					vhtfm.boot.user.creation
				),
				action: (selected_item) => {
					this.update_heatmap_data(vhtfm.datetime.obj_to_str(selected_item));
				},
			},
		];
		vhtfm.dashboard_utils.render_chart_filters(filters, "chart-filter", ".heatmap-options");
	}

	edit_profile() {
		let edit_profile_dialog = new vhtfm.ui.Dialog({
			...
			primary_action: (values) => {
				...
				vhtfm
					.xcall("vhtfm.desk.page.user_profile.user_profile.update_profile_info", {
						profile_info: values,
					})
					.then((user) => {
	...
	render_user_details() {
		this.sidebar.empty().append(
			vhtfm.render_template("user_profile_sidebar", {
				user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== vhtfm.session.user) {
	...
	get_user_rank() {
		return vhtfm
			.xcall("vhtfm.desk.page.user_profile.user_profile.get_user_rank", {
				user: this.user_id,
			})
			.then((r) => {
	...
	get_user_points() {
		return vhtfm
			.xcall(
				"vhtfm.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points",
				{
					user: this.user_id,
				}
			)
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${vhtfm.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		vhtfm.set_route("Form", "User", this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return vhtfm.xcall("vhtfm.desk.page.user_profile.user_profile.get_energy_points_list", {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: vhtfm.energy_points.format_history_log(data),
		};
	}
...
vhtfm.ui.UserProfile = UserProfile;

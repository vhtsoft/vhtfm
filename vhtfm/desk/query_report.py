...
from vhtfm import _
from vhtfm.core.utils import ljust_list
from vhtfm.desk.reportview import clean_params, parse_json
...
from vhtfm.permissions import get_role_permissions, has_permission
from vhtfm.utils import cint, cstr, flt, format_duration, get_html_format, sbool


def get_report_doc(report_name):
	doc = vhtfm.get_doc("Report", report_name)
	doc.custom_columns = []
	doc.custom_filters = []
	...
	if not vhtfm.has_permission(doc.ref_doctype, "report"):
		vhtfm.throw(
	...
	if doc.disabled:
		vhtfm.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": vhtfm.cache.hget("report_execution_time", report.name) or 0,
	}
...
def get_script(report_name):
	...
	module = report.module or vhtfm.db.get_value("DocType", report.ref_doctype, "module")

	is_custom_module = vhtfm.get_cached_value("Module Def", module, "custom")
	...
	if not script:
		script = "vhtfm.query_reports['{}']={{}}".format(report_name)

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": vhtfm.cache.hget("report_execution_time", report_name) or 0,
...
def get_reference_report(report):
	if report.report_type != "Custom Report":
		return report
	reference_report = vhtfm.get_doc("Report", report.reference_report)
...
def run(
	...
	report = get_report_doc(report_name)
	if not vhtfm.has_permission(report.ref_doctype, "report"):
		vhtfm.msgprint(
...
def add_custom_column_data(custom_columns, result):
	...
	for column in custom_columns:
		if len(column["fieldname"].split("-")) > 1:
			...
			doctype_name = vhtfm.unscrub(column["fieldname"].split("-")[1])
		...
		if key in custom_column_data:
			for row in result:
				...
				if isinstance(link_field, str):
					link_field = vhtfm._dict({"fieldname": link_field, "names": []})
...
def get_prepared_report_result(report, filters, dn="", user=None):
	from vhtfm.core.doctype.prepared_report.prepared_report import get_completed_prepared_report
	...
	doc = vhtfm.get_doc("Prepared Report", dn) if dn else None
	if doc:
		try:
			...
			vhtfm.msgprint(_("Prepared report render failed") + f": {e!s}")
			doc = None
...
def export_query():
	...
	form_params = vhtfm._dict(vhtfm.local.form_dict)
	csv_params = pop_csv_params(form_params)
	clean_params(form_params)
	parse_json(form_params)

	report_name = form_params.report_name
	vhtfm.permissions.can_export(
		vhtfm.get_cached_value("Report", report_name, "ref_doctype"),
		raise_exception=True,
	)

	file_format_type = form_params.file_format_type
	custom_columns = vhtfm.parse_json(form_params.custom_columns or "[]")
...
def format_duration_fields(data: vhtfm._dict) -> None:
...
def get_data_for_custom_field(doctype, field, names=None):
	if not vhtfm.has_permission(doctype, "read"):
		vhtfm.throw(_("Not Permitted to read {0}").format(doctype), vhtfm.PermissionError)

	filters = {}
	if names:
		if isinstance(names, str | bytearray):
			names = vhtfm.json.loads(names)
		filters.update({"name": ["in", names]})

	return vhtfm._dict(vhtfm.get_list(doctype, filters=filters, fields=["name", field], as_list=1))
...
def save_report(reference_report, report_name, columns, filters):
	...
	if docname:
		report = vhtfm.get_doc("Report", docname)
		...
		report.save()
		vhtfm.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = vhtfm.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		vhtfm.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = vhtfm.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(vhtfm.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and vhtfm.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = vhtfm.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = vhtfm.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = vhtfm.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
...
def validate_filters_permissions(report_name, filters=None, user=None):
	...
	report = vhtfm.get_doc("Report", report_name)

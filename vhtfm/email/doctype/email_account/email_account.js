...
function oauth_access(frm) {
	vhtfm.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = vhtfm.get_doc("Connected App", frm.doc.connected_app);
		return vhtfm.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		vhtfm.call({
			method: "vhtfm.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
vhtfm.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(vhtfm.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(vhtfm.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"vhtfm.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "vhtfm.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (vhtfm.route_flags.delete_user_from_locals && vhtfm.route_flags.linked_user) {
			delete vhtfm.route_flags.delete_user_from_locals;
			delete locals["User"][vhtfm.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				vhtfm.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						vhtfm.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_vhtfm_mail_settings: function (frm) {
		if (frm.doc.service == "Vhtfm Mail") {
			vhtfm.call({
				doc: frm.doc,
				method: "validate_vhtfm_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			vhtfm.call({
				method: "vhtfm.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: vhtfm.utils.debounce((frm) => {
		if (frm.doc.domain) {
			vhtfm.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			vhtfm.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			vhtfm.confirm(msg, null, function () {
				...
				vhtfm.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});

...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if vhtfm.local.flags.in_patch or vhtfm.local.flags.in_test or vhtfm.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in vhtfm.get_all("Email Account", filters={"domain": self.name}):
			try:

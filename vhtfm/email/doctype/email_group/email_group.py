...
from vhtfm.utils import parse_addr, validate_email_address
...
class EmailGroup(Document):
	...
	def onload(self):
		singles = [d.name for d in vhtfm.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": f"{d.parent} ({d.label})"}
			for d in vhtfm.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in vhtfm.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			with contextlib.suppress(vhtfm.UniqueValidationError, vhtfm.InvalidEmailAddressError):
		...
		vhtfm.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in vhtfm.get_all("Email Group Member", "name", {"email_group": self.name}):
			vhtfm.delete_doc("Email Group Member", d.name)


@vhtfm.whitelist()
def import_from(name, doctype):
	nlist = vhtfm.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = vhtfm.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = vhtfm.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not vhtfm.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				vhtfm.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=vhtfm.flags.ignore_permissions)
			...
			vhtfm.msgprint(_("{0} is not a valid Email Address").format(email))

	vhtfm.msgprint(_("{0} subscribers added").format(count))

	return vhtfm.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = vhtfm.render_template(welcome_email.response_, args)
	vhtfm.sendmail(email, subject=welcome_email.subject, message=message)

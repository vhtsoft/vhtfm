...
import vhtfm
from vhtfm import _, safe_encode, task
from vhtfm.core.utils import html2text
from vhtfm.database.database import savepoint
from vhtfm.email.doctype.email_account.email_account import EmailAccount
from vhtfm.email.email_body import add_attachment, get_email, get_formatted_html
from vhtfm.email.vhtfmmail import VhtfmMail
from vhtfm.email.queue import get_unsubcribed_url, get_unsubscribe_message
from vhtfm.email.smtp import SMTPServer
from vhtfm.model.document import Document
from vhtfm.query_builder import DocType, Interval
...
from vhtfm.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if vhtfm.session.user != "Administrator":
			vhtfm.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = vhtfm.get_doc(values)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = vhtfm.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return vhtfm.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			vhtfm.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(vhtfm.db.get_default("suspend_email_queue")) == 1
		):
			return False

		return True

	def send(self, smtp_server_instance: SMTPServer = None, vhtfm_mail_client: VhtfmMail = None):
		"""Send emails to recipients."""
		if not self.can_send_now():
			return

		with SendMailContext(self, smtp_server_instance, vhtfm_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not vhtfm.flags.in_test or vhtfm.flags.testing_email:
					if ctx.email_account_doc.service == "Vhtfm Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.vhtfm_mail_client.send_raw(
							sender=self.sender,
			...
			if vhtfm.flags.in_test and not vhtfm.flags.testing_email:
				vhtfm.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = vhtfm.qb.DocType("Email Queue")
		email_recipient = vhtfm.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			vhtfm.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			vhtfm.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from vhtfm.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Vhtfm Mail":
			if not self.vhtfm_mail_client:
				self.vhtfm_mail_client = self.email_account_doc.get_vhtfm_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": vhtfm.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/vhtfm.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = vhtfm.get_doc("File", file_filters)
				...
				print_format_file = vhtfm.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not vhtfm.get_system_settings("store_attached_pdf_document"):
			return

		file_data = vhtfm._dict(file_name=file_name, is_private=1)
		...
		if vhtfm.db.exists("File", file_data):
			return

		file = vhtfm.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not vhtfm.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = vhtfm.qb.DocType("Email Queue")
	vhtfm.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, vhtfm.session.user
...
def toggle_sending(enable):
	...
	vhtfm.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	vhtfm.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	vhtfm.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(vhtfm.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/vhtfm.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/vhtfm.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				vhtfm.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in vhtfm.utils.create_batch(final_recipients, 1000):
				vhtfm.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=vhtfm.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=vhtfm.flags.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		vhtfm_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not vhtfm_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Vhtfm Mail":
					vhtfm_mail_client = email_account.get_vhtfm_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, vhtfm_mail_client=vhtfm_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except vhtfm.InvalidEmailAddressError:

...
vhtfm.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = vhtfm.boot.user.can_write.includes(doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = vhtfm.user_info(doc.owner);
	...
	send_emails(frm) {
		vhtfm.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			vhtfm.dom.unfreeze();
			vhtfm.show_alert(
				__("Queued {0} emails", [vhtfm.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = vhtfm.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = vhtfm.datetime.global_date_format(frm.doc.schedule_send);

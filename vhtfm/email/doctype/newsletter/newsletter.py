...
import vhtfm.utils
from vhtfm import _
from vhtfm.email.doctype.email_group.email_group import add_subscribers
from vhtfm.rate_limiter import rate_limit
from vhtfm.utils.safe_exec import is_job_queued
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from vhtfm.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from vhtfm.email.doctype.newsletter_email_group.newsletter_email_group import NewsletterEmailGroup
	...
	def get_sending_status(self):
		...
		emails_queued = is_job_queued(
			job_name=vhtfm.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = vhtfm.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails, test_email=True)
		vhtfm.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			vhtfm.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			vhtfm.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			vhtfm.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			vhtfm.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			vhtfm.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if getattr(vhtfm.flags, "is_scheduler_running", False):
			return

		if (
			self.schedule_sending
			and vhtfm.utils.get_datetime(self.schedule_send) < vhtfm.utils.now_datetime()
		):
			vhtfm.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = vhtfm.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or vhtfm.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(vhtfm.db.auto_commit_on_many_writes)
		vhtfm.db.auto_commit_on_many_writes = not vhtfm.flags.in_test
		...
		vhtfm.sendmail(
			...
			if test_email
			else "/api/method/vhtfm.email.doctype.newsletter.newsletter.newsletter_email_read",
		)

		vhtfm.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self, medium=None) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = vhtfm.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = vhtfm.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not vhtfm.utils.is_site_link(href):
					continue
				new_href = vhtfm.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or vhtfm.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	vhtfm.flags.ignore_permissions = True
	doc = vhtfm.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = vhtfm.utils.get_url(
		"/api/method/vhtfm.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = vhtfm.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = vhtfm.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = vhtfm.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = vhtfm.get_doc("Email Group", email_group)
	except vhtfm.DoesNotExistError:
		group = vhtfm.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		vhtfm.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	vhtfm.flags.is_scheduler_running = True

	scheduled_newsletter = vhtfm.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", vhtfm.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = vhtfm.get_doc("Newsletter", newsletter_name)
			...
			vhtfm.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
			newsletter.log_error("Failed to send newsletter")

		if not vhtfm.flags.in_test:
			vhtfm.db.commit()

	vhtfm.flags.is_scheduler_running = False
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = vhtfm.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = vhtfm.qb.DocType("Newsletter")
			(
				vhtfm.qb.update(newsletter)
		...
		vhtfm.response.update(vhtfm.utils.get_imaginary_pixel_response())


def get_default_email_group():
	return _("Website", lang=vhtfm.db.get_default("language"))

...
import vhtfm
from vhtfm.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from vhtfm.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from vhtfm.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			vhtfm.delete_doc("Newsletter", newsletter)
			vhtfm.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not vhtfm.db.exists("Email Group", "_Test Email Group"):
			vhtfm.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				vhtfm.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = vhtfm.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			vhtfm.delete_doc(doctype, similar_newsletter)

		newsletter = vhtfm.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
	def test_send(self):
		...
		email_queue_list = [vhtfm.get_doc("Email Queue", e.name) for e in vhtfm.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = vhtfm.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "vhtfm.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=vhtfm.OutgoingEmailError)

		with self.assertRaises(vhtfm.OutgoingEmailError):
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(vhtfm.db.count("Email Queue"), 4)

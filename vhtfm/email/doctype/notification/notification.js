...
vhtfm.notification = {
	setup_fieldname_select: function (frm) {
		...
		vhtfm.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (vhtfm.model.table_fields.includes(df.fieldtype)) {
						let child_fields = vhtfm.get_doc("DocType", df.options).fields;
			...
			let fields = vhtfm.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return vhtfm.model.no_value_type.includes(d.fieldtype)
					? null
...
vhtfm.ui.form.on("Notification", {
	...
	refresh: function (frm) {
		vhtfm.notification.setup_fieldname_select(frm);
		vhtfm.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(vhtfm.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Preview"), () => {
				...
				let dialog = new vhtfm.views.RenderPreviewer(args);
				return dialog;
			});
		}
	},
	...
	view_properties: function (frm) {
		vhtfm.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			vhtfm.call({
				method: "vhtfm.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						vhtfm.msgprint(r.message.toString());
					} else {
						vhtfm.msgprint(__("No alerts for today"));
					}
				},
			});
		});

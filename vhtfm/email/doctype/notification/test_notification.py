...
def get_test_notification(config):
	try:
		notification = vhtfm.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		vhtfm.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		vhtfm.db.delete("Email Queue")
		vhtfm.set_user("test@example.com")

		if not vhtfm.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = vhtfm.new_doc("Notification")
		...
		if not vhtfm.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = vhtfm.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		communication = vhtfm.new_doc("Communication")
		...
		self.assertEqual(vhtfm.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = vhtfm.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = vhtfm.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(vhtfm.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			vhtfm.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, vhtfm.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(vhtfm.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		vhtfm.get_doc(
			"Scheduled Job Type",
			dict(method="vhtfm.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = vhtfm.utils.add_days(vhtfm.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		vhtfm.db.delete("User", {"email": "test_jinja@example.com"})
		vhtfm.db.delete("Email Queue")
		vhtfm.db.delete("Email Queue Recipient")

		test_user = vhtfm.new_doc("User")
		...
		self.assertTrue(vhtfm.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = vhtfm.utils.add_days(user.birth_date, 1)
			user.save()

			user.reload()
			user.birth_date = vhtfm.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, vhtfm.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		vhtfm.delete_doc_if_exists("Notification", "ToDo Status Update")
		vhtfm.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = vhtfm.get_doc(
			{
				...
				"message": "{% set val = vhtfm.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
...
❯ fox run-tests --module vhtfm.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		self.event = vhtfm.new_doc("Event")
...
# 		vhtfm.db.delete("Event", {"name": self.event.name})
# 		vhtfm.db.delete("Notification", {"name": self.notification["name"]})
# 		vhtfm.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							vhtfm.db.count("Notification Log", {"subject": n.subject}),

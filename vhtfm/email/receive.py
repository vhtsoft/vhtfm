...
import vhtfm
from vhtfm import _, safe_decode, safe_encode
from vhtfm.core.doctype.file.exceptions import MaxFileSizeReachedError
from vhtfm.core.doctype.file.utils import get_random_filename
from vhtfm.email.oauth import Oauth
...
from vhtfm.utils.user import is_system_user
...
class EmailSizeExceededError(vhtfm.ValidationError):
	pass


class LoginLimitExceeded(vhtfm.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.settings = args or vhtfm._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = imaplib.IMAP4_SSL(
					...
					timeout=vhtfm.conf.pop_timeout,
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = imaplib.IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=vhtfm.conf.pop_timeout
				)
			...
			vhtfm.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			vhtfm.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				vhtfm.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		vhtfm.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
	...
	def retrieve_message(self, uid, msg_num):
		try:
			...
			vhtfm.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(vhtfm.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not vhtfm.db.get_value(
	...
	def decode_email(email: bytes | str | None) -> str | None:
		if not email:
			return
		email = vhtfm.as_unicode(email).replace('"', " ").replace("'", " ")
		try:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except vhtfm.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = vhtfm.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from vhtfm.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and vhtfm.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Vhtfm. This fix bypasses
		...
		if not record:
			...
			name = vhtfm.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = vhtfm.new_doc(doctype)
		email_fields = self.get_email_fields(doctype)

		if email_fields.subject_field:
			parent.set(email_fields.subject_field, vhtfm.as_unicode(self.subject)[:140])

		if email_fields.sender_field:
			parent.set(email_fields.sender_field, vhtfm.as_unicode(self.from_email))

		if email_fields.sender_name_field:
			parent.set(email_fields.sender_name_field, vhtfm.as_unicode(self.from_real_name))
		...
		try:
			...
			return vhtfm.db.get_value(doctype, {email_fields.sender_field: self.from_email})

	@staticmethod
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return vhtfm.get_doc(doctype, docname)
	...
	def get_users_linked_to_account(email_account):
		...
		users = vhtfm.get_all("User Email", filters={"email_account": email_account.name}, fields=["parent"])
	...
	def clean_subject(subject):
		...
		return vhtfm.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))
	...
	def get_document(self, doctype, name):
		"""Is same as vhtfm.get_doc but suppresses the DoesNotExist error."""
		try:
			return vhtfm.get_doc(doctype, name)
		except vhtfm.DoesNotExistError:
			return None

...
from vhtfm.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(vhtfm.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			vhtfm.msgprint(
				...
				raise_exception=vhtfm.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				vhtfm.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=vhtfm.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					vhtfm.msgprint(res[1], raise_exception=vhtfm.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if vhtfm.request and hasattr(vhtfm.request, "after_response"):
			vhtfm.request.after_response.add(self.quit)
		elif vhtfm.job:
			vhtfm.job.after_job.add(self.quit)
		elif not vhtfm.flags.in_test:
			# Console?
			import atexit

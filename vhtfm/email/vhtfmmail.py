...
import vhtfm
from vhtfm import _
from vhtfm.vhtfmclient import VhtfmClient, VhtfmOAuth2Client
from vhtfm.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone


class VhtfmMail:
	"""Class to interact with the Vhtfm Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> VhtfmClient | VhtfmOAuth2Client:
		"""Returns a VhtfmClient or VhtfmOAuth2Client instance."""

		if hasattr(vhtfm.local, "vhtfm_mail_clients"):
			if client := vhtfm.local.vhtfm_mail_clients.get(mailbox):
				return client
		else:
			vhtfm.local.vhtfm_mail_clients = {}

		client = (
			VhtfmOAuth2Client(url=site, access_token=access_token)
			if access_token
			else VhtfmClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		vhtfm.local.vhtfm_mail_clients[mailbox] = client
	...
	def request(
		...
		"""Makes a request to the Vhtfm Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the Vhtfm Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, str | list[str]]:
		"""Pulls emails from the mailbox using the Vhtfm Mail API."""
		...
		headers = {"X-Site": vhtfm.utils.get_url()}

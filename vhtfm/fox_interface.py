"""
This module provides a consolidated API for interacting with the Vhtfm fox environment.
It includes classes and utilities to manage and access various components of the fox,
such as applications, sites, logs, and configuration.

### Key Classes
- `Fox`: The central class representing the Vhtfm fox environment.
- `Apps`: Manages applications within the fox.
- `Sites`: Manages sites within the fox, including site configuration and scoping to a single site.
- `Logs`: Handles log files for the fox.
- `Run`: Manages the run/config directory of the fox.

### Usage
This module is designed to be used as the **only** interface to the Vhtfm fox, allowing for
...
scoped_site = fox.sites.scope("site_name")
print(scoped_site.name)
```

### Notes
- This module uses environment variables and default paths to locate fox and its compontents.
...
from vhtfm.config import ConfigHandler, ConfigType

__all__ = [
	"Fox",
	"FoxNotScopedError",
	"FoxSiteNotLoadedError",
]
...
class FoxNotScopedError(NotImplementedError):
	pass


class FoxSiteNotLoadedError(ValueError):
	pass
...
class _VhtfmComponent:
	...
	def __bool__(self) -> bool:
		return (
			self._is_vhtfm_component()
			or (isinstance(self, App) and self._is_app_installed())
			or (isinstance(self, Module) and self._is_module_registered())
		)

	def _is_vhtfm_component(self) -> bool:
		return self.python_path.is_dir() and self.python_path.joinpath(".vhtfm").exists()

	def _is_app_installed(self) -> bool:
		global _current_fox
		res = self.name in _current_fox.sites.path.joinpath("apps.txt").read_text()
		if res:
			...
			deprecation_warning(
				"2024-10-18",
				"yet unknown",
				f"Instead of adding {self.name} to sites/apps.txt, drop an empty file at {self.python_path}/.vhtfm",
			)
			return res
		return False
	...
	def _is_module_registered(self) -> bool:
		...
		if res:
			...
			deprecation_warning(
				"2024-10-18",
				"yet unknown",
				f"Instead of adding {self.name.title()} to {self.app.python_path}/modules.txt, drop an empty file at {self.python_path}/.vhtfm",
			)
			return res
		return False
...
class Module(_VhtfmComponent, _Serializable):
...
class App(_VhtfmComponent, _Serializable):
...
class Apps(_PathResolvable, _Serializable):
	__apps: dict[str, App]
	__path_name = "apps"
	__env_variable = "VHTFM_APPS_PATH"
...
class Logs(_PathResolvable, _Serializable):
	__path_name = "logs"
	__env_variable = "VHTFM_LOGS_PATH"
...
class Site(ConfigHandler, _Serializable):
	...
	def __init__(self, *, fox: "Fox", name: str, path: str | Path):
		self.name = name
		self.path = Path(path)
		self.fox: Fox = fox
	...
	def config(self) -> ConfigType:
		if not hasattr(self, "_combined_config") or self._config_stale or self.fox.sites._config_stale:
			site_config = super().config.copy()
			config = self.fox.sites.config.copy()
...
class Sites(_PathResolvable, ConfigHandler, _Serializable):
	...
	__path_name = "sites"
	__env_variable = "VHTFM_SITES_PATH"

	if legacy_env_path := os.environ.get("SITES_PATH"):
		os.environ["VHTFM_SITES_PATH"] = legacy_env_path

	def __init__(self, *, fox: "Fox", path: str | Path | None = None, parent_path: Path | None = None):
		...
		self.site_name = os.environ.get("VHTFM_SITE") or cast(str | None, self.config.get("default_site"))

	@property
	def site_name(self) -> str | None:
		import vhtfm

		return cast(str | None, getattr(vhtfm.local, "site_name", None))

	@site_name.setter
	def site_name(self, value):
		if self.site_name is not None and value == self.SCOPE_ALL_SITES:
			raise FoxNotScopedError("Cannot scope back to SCOPE_ALL_SITES when already scoped")

		import vhtfm

		vhtfm.local.site_name = value
	...
	def add_site(self, site_name: str) -> None:
		...
		self.__sites[site_name] = Site(fox=self.fox, name=site_name, path=site_path)
	...
	def site(self) -> Site:
		...
		if not self.site_name or self.site_name == self.SCOPE_ALL_SITES:
			raise FoxNotScopedError("Sites was not scoped to a single site, yet.")
			...
			if self.site_name == self.SCOPE_ALL_SITES:
				for path in self.path.iterdir():
					if path.is_dir() and path.joinpath("site_config.json").exists():
						self.__sites[path.name] = Site(fox=self.fox, name=path.name, path=path)
			elif self.site_name:
				...
				self.__sites[self.site_name] = Site(
					fox=self.fox, name=self.site_name, path=(self.path / self.site_name)
				)
		return self.__sites
	...
	def __iter__(self) -> Iterator[Site]:
		...
		raise FoxNotScopedError("Sites was not scoped, yet.")
	...
	def __getitem__(self, key: str) -> Site:
		try:
			...
			raise FoxSiteNotLoadedError(f"Site '{key}' was not found")
...
class Fox(_PathResolvable, _Serializable):
	__env_variable = "VHTFM_FOX_PATH"
	if legacy_env_path := os.environ.get("VHTFM_FOX_ROOT"):
		os.environ["VHTFM_FOX_PATH"] = legacy_env_path
	if not os.environ.get("VHTFM_FOX_PATH") and (legacy_env_path_sites := os.environ.get("SITES_PATH")):
		os.environ["VHTFM_FOX_PATH"] = str(Path(legacy_env_path_sites).resolve().parent)
	if not os.environ.get("VHTFM_FOX_PATH"):
		# fox folder in standard layout
		os.environ["VHTFM_FOX_PATH"] = str(Path(__file__).resolve().parents[3])

	def __init__(self, path: str | Path | None = None):
		super().__init__(path)
		self.logs = Logs(parent_path=self.path)
		self.run = Run(parent_path=self.path)
		self.sites = Sites(fox=self, parent_path=self.path)
		global _current_fox
		_current_fox = self

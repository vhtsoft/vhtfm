...
import vhtfm
from vhtfm.geo.doctype.country.country import (
	get_countries_and_currencies,
	import_country_and_currency,
)
from vhtfm.geo.doctype.currency.currency import enable_default_currencies
from vhtfm.tests import IntegrationTestCase, UnitTestCase


def get_table_snapshot(doctype):
	data = vhtfm.db.sql(f"select * from `tab{doctype}` order by name", as_dict=True)
...
class TestCountry(IntegrationTestCase):
	def test_bulk_insert_correctness(self):
		def clear_tables():
			vhtfm.db.delete("Currency")
			vhtfm.db.delete("Country")

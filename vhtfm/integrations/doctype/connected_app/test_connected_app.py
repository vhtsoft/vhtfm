...
import vhtfm
from vhtfm.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = vhtfm.new_doc(doctype)
	connected_app.provider_name = "vhtfm"
...
def get_oauth_client():
	oauth_client = vhtfm.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		Vhtfm comes with it's own oAuth2 provider that we can test against. The
		...
		Vhtfm's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/vhtfm.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = vhtfm.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = vhtfm.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = vhtfm.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = vhtfm.get_doc("OAuth Authorization Code", code.name)
				doc.delete()

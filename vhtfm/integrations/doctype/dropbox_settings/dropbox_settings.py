...
import vhtfm
from vhtfm import _
from vhtfm.integrations.offsite_backup_utils import (
	get_chunk_site,
...
from vhtfm.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
from vhtfm.utils.background_jobs import enqueue
from vhtfm.utils.backups import new_backup
...
class DropboxSettings(Document):
	...
	def onload(self):
		if not self.app_access_key and vhtfm.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			vhtfm.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"vhtfm.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	vhtfm.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if vhtfm.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(vhtfm.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(vhtfm.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if vhtfm.flags.create_new_backup:
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = vhtfm._dict({"entries": []})

	path = str(path)

	for f in vhtfm.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(vhtfm.get_traceback())
...
def update_file_dropbox_status(file_name):
	vhtfm.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = vhtfm.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = vhtfm.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or vhtfm.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else vhtfm.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/vhtfm.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		vhtfm.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=vhtfm.AuthenticationError.http_status_code,
		)
		return
	...
	vhtfm.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = vhtfm.get_single("Dropbox Settings")

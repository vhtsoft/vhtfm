...
import vhtfm
from vhtfm import _
from vhtfm.integrations.google_oauth import GoogleOAuth
...
from vhtfm.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		...
		if not google_settings.enable:
			vhtfm.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			vhtfm.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = vhtfm.bold(_("Allow Google Calendar Access"))
			raise vhtfm.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = vhtfm.get_doc("Google Settings")
	google_calendar = vhtfm.get_doc("Google Calendar", g_calendar)
	google_calendar.check_permission("write")

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=vhtfm.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		vhtfm.cache.hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			vhtfm.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			vhtfm.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = vhtfm.cache.hget("google_calendar", "google_calendar")
	vhtfm.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	vhtfm.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = vhtfm.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = vhtfm.get_doc("Google Settings")
	account = vhtfm.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": vhtfm.get_system_settings("time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				vhtfm.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				vhtfm.throw(msg)
	...
	for idx, event in enumerate(results):
		vhtfm.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=vhtfm.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not vhtfm.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			vhtfm.get_doc(
				{
					...
					"reference_name": vhtfm.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Vhtfm Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	vhtfm.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Vhtfm Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = vhtfm.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not vhtfm.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		vhtfm.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		vhtfm.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Vhtfm Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Vhtfm Event is deleted.
	"""

	if not vhtfm.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return

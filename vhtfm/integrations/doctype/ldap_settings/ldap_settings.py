...
import vhtfm
from vhtfm import _, safe_encode
from vhtfm.model.document import Document
from vhtfm.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			vhtfm.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			vhtfm.throw(_("Invalid username or password"))
		except Exception as ex:
			vhtfm.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = vhtfm.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "vhtfm.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if vhtfm.db.exists("User", user_data["email"]):
			user = vhtfm.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = vhtfm.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			vhtfm.throw(_("LDAP is not enabled."))
		...
		try:
			...
			vhtfm.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from vhtfm.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					vhtfm.msgprint(_("Password changed successfully."))
				else:
					vhtfm.throw(_("Failed to change password."))
			else:
				vhtfm.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			vhtfm.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = vhtfm.form_dict
	ldap: LDAPSettings = vhtfm.get_doc("LDAP Settings")

	user = ldap.authenticate(vhtfm.as_unicode(args.usr), vhtfm.as_unicode(args.pwd))

	vhtfm.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(vhtfm.local.login_manager):
			return False

...
class BaseUrlNotSetError(vhtfm.ValidationError):
	pass


class AuthorizeUrlNotSetError(vhtfm.ValidationError):
	pass


class AccessTokenUrlNotSetError(vhtfm.ValidationError):
	pass


class RedirectUrlNotSetError(vhtfm.ValidationError):
	pass


class ClientIDNotSetError(vhtfm.ValidationError):
	pass


class ClientSecretNotSetError(vhtfm.ValidationError):
	pass
...
class SocialLoginKey(Document):
	...
	def autoname(self):
		self.name = vhtfm.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			vhtfm.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			vhtfm.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			vhtfm.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			vhtfm.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
		if self.enable_social_login and not self.client_id:
			vhtfm.throw(_("Please enter Client ID before social login is enabled"), exc=ClientIDNotSetError)
	...
	def set_icon(self):
		...
		if self.provider_name in icon_map:
			...
			self.icon = f"/assets/vhtfm/icons/social/{icon_file}"
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Office 365"] = {
			...
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_office365",
			"api_endpoint": None,
		...
		providers["GitHub"] = {
			...
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_github",
			"api_endpoint": "user",
			"api_endpoint_args": None,
		...
		providers["Google"] = {
			...
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_google",
		...
		providers["Facebook"] = {
			...
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_facebook",
		...
		providers["Vhtfm"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"icon": "/assets/vhtfm/images/vhtfm-favicon.svg",
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_vhtfm",
			"api_endpoint": "/api/method/vhtfm.integrations.oauth2.openid_profile",
			"api_endpoint_args": None,
			"authorize_url": "/api/method/vhtfm.integrations.oauth2.authorize",
			"access_token_url": "/api/method/vhtfm.integrations.oauth2.get_token",
			"auth_url_data": json.dumps({"response_type": "code", "scope": "openid"}),
		}
		...
		providers["Salesforce"] = {
			...
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_salesforce",
		...
		providers["fairlogin"] = {
			...
			"custom_base_url": 0,
			"icon": "fa fa-key",
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_fairlogin",
		...
		providers["Keycloak"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"redirect_url": "/api/method/vhtfm.integrations.oauth2_logins.login_via_keycloak/keycloak",
...
def provider_allows_signup(provider: str) -> bool:
	from vhtfm.website.utils import is_signup_disabled

	sign_up_config = vhtfm.db.get_value("Social Login Key", provider, "sign_ups")

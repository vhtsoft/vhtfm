...
from vhtfm.integrations.doctype.social_login_key.social_login_key import BaseUrlNotSetError
from vhtfm.tests import IntegrationTestCase, UnitTestCase
from vhtfm.utils import set_request
from vhtfm.utils.oauth import login_via_oauth2
...
class TestSocialLoginKey(IntegrationTestCase):
	def setUp(self) -> None:
		...
		vhtfm.delete_doc("User", TEST_GITHUB_USER, force=True)
		super().setUp()
		vhtfm.set_user("Guest")

	def test_adding_vhtfm_social_login_provider(self):
		vhtfm.set_user("Administrator")
		provider_name = "Vhtfm"
	...
	def test_normal_signup_and_github_login(self):
		...
		if not vhtfm.db.exists("User", TEST_GITHUB_USER):
			user = vhtfm.new_doc("User", email=TEST_GITHUB_USER, first_name="GitHub Login")
		...
		self.assertEqual(vhtfm.session.user, TEST_GITHUB_USER)
	...
	def test_force_disabled_signups(self):
		...
		with patch.object(OAuth2Service, "get_auth_session", return_value=mock_session):
			login_via_oauth2("github", "iwriu", {"token": "ewrwerwer"})
		self.assertEqual(vhtfm.session.user, "Guest")
...
def make_social_login_key(**kwargs):
	...
	return vhtfm.get_doc(kwargs)


def create_or_update_social_login_key():
	# used in other tests (connected app, oauth20)
	try:
		social_login_key = vhtfm.get_doc("Social Login Key", "vhtfm")
	except vhtfm.DoesNotExistError:
		social_login_key = vhtfm.new_doc("Social Login Key")
	social_login_key.get_social_login_provider("Vhtfm", initialize=True)
	social_login_key.base_url = vhtfm.utils.get_url()
...
def create_github_social_login_key():
	if vhtfm.db.exists("Social Login Key", "github"):
		return vhtfm.get_doc("Social Login Key", "github")
	else:

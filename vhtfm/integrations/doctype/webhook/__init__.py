...
def run_webhooks(doc, method):
	...
	vhtfm_flags = vhtfm.local.flags

	if vhtfm_flags.in_import or vhtfm_flags.in_patch or vhtfm_flags.in_install or vhtfm_flags.in_migrate:
		return

	# load all webhooks from cache / DB
	webhooks = vhtfm.cache.get_value("webhooks", get_all_webhooks)
	...
	from vhtfm.integrations.doctype.webhook.webhook import get_context
	...
	for webhook in webhooks_for_doc:
		...
		elif vhtfm.safe_eval(webhook.condition, eval_locals=get_context(doc)):
			trigger_webhook = True
...
def _add_webhook_to_queue(webhook, doc):
	...
	if not getattr(vhtfm.local, "_webhook_queue", None):
		vhtfm.local._webhook_queue = []
		vhtfm.db.after_commit.add(flush_webhook_execution_queue)

	vhtfm.local._webhook_queue.append(vhtfm._dict(doc=doc, webhook=webhook))
...
def flush_webhook_execution_queue():
	...
	# reverse
	vhtfm.local._webhook_queue.reverse()

	# deduplicate on (doc.name, webhook.name)
	# 'doc' holds the last instance values
	for execution in vhtfm.local._webhook_queue:
	...
	del vhtfm.local._webhook_queue
	...
	for instance in unique_last_instances:
		vhtfm.enqueue(
			"vhtfm.integrations.doctype.webhook.webhook.enqueue_webhook",
			doc=instance.doc,

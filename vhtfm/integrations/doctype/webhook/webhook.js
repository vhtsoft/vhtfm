...
vhtfm.webhook = {
	set_fieldname_select: (frm) => {
		if (frm.doc.webhook_doctype) {
			vhtfm.model.with_doctype(frm.doc.webhook_doctype, () => {
				// get doctype fields
				let fields = $.map(
					vhtfm.get_doc("DocType", frm.doc.webhook_doctype).fields,
					(d) => {
						if (
							vhtfm.model.no_value_type.includes(d.fieldtype) &&
							!vhtfm.model.table_fields.includes(d.fieldtype)
						) {
							return null;
						} else {
							return {
				...
				// add meta fields
				for (let field of vhtfm.model.std_fields) {
...
vhtfm.ui.form.on("Webhook", {
	refresh: (frm) => {
		vhtfm.webhook.set_fieldname_select(frm);
		frm.set_query(
			"background_jobs_queue",
			"vhtfm.integrations.doctype.webhook.webhook.get_all_queues"
		);
	...
	request_structure: (frm) => {
		vhtfm.webhook.set_request_headers(frm);
	},
...
vhtfm.ui.form.on("Webhook Data", {
	fieldname: (frm, cdt, cdn) => {
		...
		if (!df.length) {
			...
			df = vhtfm.model.std_fields.filter((field) => field.fieldname == row.fieldname);
		}

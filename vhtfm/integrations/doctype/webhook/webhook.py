...
from vhtfm.utils.background_jobs import get_queues_timeout
from vhtfm.utils.jinja import validate_template
from vhtfm.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Vhtfm-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from vhtfm.integrations.doctype.webhook_data.webhook_data import WebhookData
		from vhtfm.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def on_update(self):
		vhtfm.cache.delete_value("webhooks")

	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = vhtfm.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				vhtfm.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = vhtfm.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				vhtfm.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				vhtfm.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			vhtfm.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			vhtfm.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				vhtfm.throw(_("Invalid Webhook Secret"))
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = vhtfm.get_cached_doc(self.webhook_doctype, preview_document)
			met_condition = vhtfm.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self, preview_document):
		try:
			doc = vhtfm.get_cached_doc(self.webhook_doctype, preview_document)
			return vhtfm.as_json(get_webhook_data(doc, self))
		except Exception as e:
			vhtfm.local.message_log = []
...
def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("vhtfm").get("utils")}


def enqueue_webhook(doc, webhook) -> None:
	request_url = headers = data = r = None
	try:
		webhook: Webhook = vhtfm.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = vhtfm.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		vhtfm.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			vhtfm.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			vhtfm.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = vhtfm.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_doctype": doctype,
			"reference_document": docname,
			"user": vhtfm.session.user if vhtfm.session.user else None,
			"url": url,
			"headers": vhtfm.as_json(headers) if headers else None,
			"data": vhtfm.as_json(data) if data else None,
			"response": res.text if res is not None else None,
			"error": vhtfm.get_traceback(),
		}
	)
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = vhtfm.render_template(webhook.webhook_json, get_context(doc))

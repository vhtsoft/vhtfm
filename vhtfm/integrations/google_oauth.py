...
import vhtfm
from vhtfm.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/vhtfm.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "vhtfm.email.oauth.authorize_google_access",
	"contacts": "vhtfm.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "vhtfm.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "vhtfm.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = vhtfm.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			vhtfm.throw(vhtfm._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			vhtfm.throw(vhtfm._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		vhtfm.log_error(vhtfm._(error_title), vhtfm._(response.get("error_description", error_message)))

		if raise_err:
			vhtfm.throw(vhtfm._(error_title), GoogleAuthenticationError, vhtfm._(error_message))
...
@vhtfm.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `vhtfm.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to vhtfm site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			vhtfm.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	vhtfm.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"

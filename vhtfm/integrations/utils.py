...
import vhtfm
from vhtfm.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = vhtfm.flags.integration_request = s.request(
		...
		if vhtfm.flags.integration_request_doc:
			vhtfm.flags.integration_request_doc.log_error()
		else:
			vhtfm.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = vhtfm.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else vhtfm.as_json(obj, indent=1)

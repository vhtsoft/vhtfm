...
import vhtfm
from vhtfm import _, is_whitelisted, msgprint
from vhtfm.core.doctype.file.utils import relink_mismatched_files
from vhtfm.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from vhtfm.desk.form.document_follow import follow_document
from vhtfm.integrations.doctype.webhook import run_webhooks
from vhtfm.model import optional_fields, table_fields
from vhtfm.model.base_document import BaseDocument, get_controller
from vhtfm.model.docstatus import DocStatus
from vhtfm.model.naming import set_new_name, validate_name
from vhtfm.model.utils import is_virtual_doctype, simple_singledispatch
from vhtfm.model.workflow import set_workflow_state_on_action, validate_workflow
from vhtfm.types import DF, DocRef
from vhtfm.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
from vhtfm.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from vhtfm.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `vhtfm.model.Document` object.
...
def read_only_guard(func):
	...
	def wrapper(self, *args, **kwargs):
		if getattr(vhtfm.local, "read_only_depth", 0) > 0:
			...
			if getattr(vhtfm.local, "read_only_context", None):
				error_msg += f" ({vhtfm.local.read_only_context})"
			raise vhtfm.DatabaseModificationError(error_msg)
		return func(self, *args, **kwargs)
...
def read_only_document(context=None):
	...
	if not hasattr(vhtfm.local, "read_only_depth"):
		vhtfm.local.read_only_depth = 0

	vhtfm.local.read_only_depth += 1
	if context:
		vhtfm.local.read_only_context = context

	try:
		yield
	finally:
		vhtfm.local.read_only_depth -= 1
		if vhtfm.local.read_only_depth == 0:
			if hasattr(vhtfm.local, "read_only_context"):
				del vhtfm.local.read_only_context
			del vhtfm.local.read_only_depth
...
class Document(BaseDocument, DocRef):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = vhtfm.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = vhtfm.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				vhtfm.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), vhtfm.DoesNotExistError
				)
	...
	def load_children_from_db(self):
		for df in self._get_table_fields():
			...
			# During vhtfm is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to vhtfm.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `vhtfm.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `vhtfm.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return vhtfm.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)

	def raise_no_permission_to(self, perm_type):
		"""Raise `vhtfm.PermissionError`."""
		vhtfm.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			vhtfm.bold(_(self.doctype)),
			self.name or "",
		)
		raise vhtfm.PermissionError
	...
	def insert(
		...
		if not (vhtfm.flags.in_migrate or vhtfm.local.flags.in_install or vhtfm.flags.in_setup_wizard):
			if vhtfm.get_cached_value("User", vhtfm.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, vhtfm.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise vhtfm.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = vhtfm.flags.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if vhtfm.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(vhtfm.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			vhtfm.throw(message, title=_("Amendment Not Allowed"))
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `vhtfm.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		vhtfm.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in vhtfm.db.value_cache:
			del vhtfm.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = vhtfm.session.user
		...
		if self.is_new() and not (
			vhtfm.flags.in_install or vhtfm.flags.in_patch or vhtfm.flags.in_migrate
		):
		...
		vhtfm.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					vhtfm.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), vhtfm.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				vhtfm.throw(msg, vhtfm.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					vhtfm.throw(
						_("Value cannot be changed for {0}").format(
							vhtfm.bold(self.meta.get_label(field.fieldname))
						),
						exc=vhtfm.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += vhtfm.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in vhtfm.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or vhtfm.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = vhtfm.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = vhtfm.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = vhtfm.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = vhtfm.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			vhtfm.msgprint(
				...
				raise_exception=vhtfm.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise vhtfm.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise vhtfm.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise vhtfm.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if vhtfm.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise vhtfm.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			vhtfm.throw(_("Could not find {0}").format(msg), vhtfm.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			vhtfm.throw(_("Cannot link cancelled document: {0}").format(msg), vhtfm.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(vhtfm.flags.in_import and vhtfm.flags.mute_emails)
		...
		from vhtfm.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = vhtfm.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers vhtfm.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise vhtfm.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = vhtfm.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in vhtfm.flags.currently_saving:
			vhtfm.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		vhtfm.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			vhtfm.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([vhtfm.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": vhtfm.session.user}
			vhtfm.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run vhtfm.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in vhtfm.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", vhtfm.session.user)
	...
	def db_get(self, fieldname):
		...
		return vhtfm.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from vhtfm.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						vhtfm.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			vhtfm.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or vhtfm.EmptyTableError
			)
		...
		return vhtfm.get_doc(
			{
				...
				"comment_email": comment_email or vhtfm.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not vhtfm.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = vhtfm.parse_json(_seen)
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and vhtfm.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = vhtfm.get_doc("Document Share Key", existing_key)
		else:
			doc = vhtfm.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", vhtfm._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", vhtfm._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"vhtfm.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		vhtfm.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in vhtfm.local.locked_documents:
			vhtfm.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(vhtfm.unscrub(self.parentfield)),
					self.idx,
				)
			...
			vhtfm.throw(
				...
				+ _("{0} must be after {1}").format(
					vhtfm.bold(_(self.meta.get_label(to_date_field))),
					vhtfm.bold(_(self.meta.get_label(from_date_field))),
				),
				vhtfm.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = vhtfm.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from vhtfm.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from vhtfm.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = vhtfm.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if vhtfm.message_log:
			msg = vhtfm.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + vhtfm.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = vhtfm.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = vhtfm.session.user
...
def unlock_document(doctype: str, name: str):
	vhtfm.get_doc(doctype, name).unlock()
	vhtfm.msgprint(vhtfm._("Document Unlocked"), alert=True)

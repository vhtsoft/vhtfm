...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(vhtfm.local, "dynamic_link_map", None) is None or vhtfm.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		vhtfm.local.dynamic_link_map = dynamic_link_map
	return vhtfm.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += vhtfm.db.sql(query, as_dict=True)
	return df

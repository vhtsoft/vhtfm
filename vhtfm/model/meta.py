...
from vhtfm.model.base_document import (
...
from vhtfm.types import DocRef
from vhtfm.utils import cast, cint, cstr
...
def get_meta(doctype: str | dict | DocRef, cached=True) -> "_Meta":
	...
	if cached and (
		...
		if meta := vhtfm.cache.hget("doctype_meta", doctype_name):
			return meta

	meta = Meta(doctype)
	vhtfm.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return vhtfm.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = vhtfm.scrub(doctype)
	with open(vhtfm.get_app_path("vhtfm", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		vhtfm._dict(fieldname="creation", fieldtype="Datetime"),
		vhtfm._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(vhtfm._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = vhtfm.db.table_exists(self.name)
	...
	def get_valid_fields(self) -> list[str]:
		if not hasattr(self, "_valid_fields"):
			if (vhtfm.flags.in_install or vhtfm.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not vhtfm.db.table_exists("Custom Field"):
			return

		custom_fields = vhtfm.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not vhtfm.db.table_exists("Property Setter"):
			return

		property_setters = vhtfm.db.get_values(
			"Property Setter",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(vhtfm._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if vhtfm.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if vhtfm.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = vhtfm.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Vhtfm applications via hooks.
		"""
		data = vhtfm._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = vhtfm._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in vhtfm.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = vhtfm._dict(vhtfm.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = vhtfm.scrub(self.module)
		doctype = vhtfm.scrub(self.name)
		template_path = vhtfm.get_module_path(
...
def is_single(doctype):
	try:
		return vhtfm.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether {} is single".format(doctype))


def get_parent_dt(dt):
	if not vhtfm.is_table(dt):
		return ""

	return (
		vhtfm.db.get_value(
			"DocField",
			{"fieldtype": ("in", vhtfm.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	vhtfm.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(vhtfm.local, "field_currency", None):
		vhtfm.local.field_currency = vhtfm._dict()

	if not (
		vhtfm.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and vhtfm.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = vhtfm.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if vhtfm.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = vhtfm.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			vhtfm.local.field_currency.setdefault((doc.doctype, ref_docname), vhtfm._dict()).setdefault(
				df.fieldname, currency
			)

	return vhtfm.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and vhtfm.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from vhtfm.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(vhtfm.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(vhtfm.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return vhtfm._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return vhtfm._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return vhtfm._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return vhtfm._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in vhtfm.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	vhtfm.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = vhtfm.db.get_table_columns(doctype)
	fields = vhtfm.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		vhtfm.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")

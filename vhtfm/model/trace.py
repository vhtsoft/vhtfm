"""
Traced Fields for Vhtfm

This module provides utilities for creating traced fields in Vhtfm documents,
...
- Seamlessly integrate with Vhtfm's document model
...
See vhtfm.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Vhtfm documents.

	This class allows for monitoring and validating changes to specific fields
	in a Vhtfm document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if vhtfm.flags.in_test:
				vhtfm.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				vhtfm.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if vhtfm.flags.in_test:
					vhtfm.throw(str(e), AssertionError)
				else:
					vhtfm.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Vhtfm documents with traced fields.

	This class extends Vhtfm's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from vhtfm.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Vhtfm documents.
...
from vhtfm.deprecation_dumpster import model_trace_traced_field_context as traced_field_context

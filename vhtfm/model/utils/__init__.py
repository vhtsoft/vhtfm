...
def set_field_property(filters, key, value):
	...
	docs = [
		vhtfm.get_doc("DocType", d.parent)
		for d in vhtfm.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(vhtfm.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(vhtfm.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if vhtfm.flags.in_install or vhtfm.flags.in_migrate:
		if vhtfm.db.has_column("DocType", "is_virtual"):
			return vhtfm.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(vhtfm.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if vhtfm.flags.in_install or vhtfm.flags.in_migrate:
		...
		return getattr(vhtfm.get_meta(doctype), "issingle", False)

...
def get_user_settings(doctype, for_update=False):
	user_settings = vhtfm.cache.hget("_user_settings", f"{doctype}::{vhtfm.session.user}")

	if user_settings is None:
		user_settings = vhtfm.db.sql(
			"""select data from `__UserSettings`
			where `user`=%s and `doctype`=%s""",
			(vhtfm.session.user, doctype),
		)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	vhtfm.cache.hset("_user_settings", f"{doctype}::{vhtfm.session.user}", json.dumps(current))


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in vhtfm.cache.hgetall("_user_settings").items():
		key = safe_decode(key)
...
def update_user_settings_data(
	...
	if data:
		...
		if update:
			...
			vhtfm.cache.hset("_user_settings", f"{user_setting.doctype}::{user_setting.user}", None)

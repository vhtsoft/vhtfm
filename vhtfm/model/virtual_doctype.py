...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `vhtfm.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[vhtfm._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		vhtfm.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			vhtfm.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					vhtfm.bold(doctype), vhtfm.bold(m), vhtfm.bold(_as_str(method))
				),

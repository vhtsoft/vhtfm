...
if TYPE_CHECKING:
	...
	from vhtfm.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(vhtfm.ValidationError):
	pass


class WorkflowTransitionError(vhtfm.ValidationError):
	pass


class WorkflowPermissionError(vhtfm.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = vhtfm.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = vhtfm.db.get_value("Workflow", {"document_type": doctype, "is_active": 1}, "name")
		vhtfm.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = vhtfm.get_doc(vhtfm.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			vhtfm.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to vhtfm.db.get_value, vhtfm.db.get_list, and date time utils.
	return dict(
		vhtfm=vhtfm._dict(
			db=vhtfm._dict(get_value=vhtfm.db.get_value, get_list=vhtfm.db.get_list),
			session=vhtfm.session,
			utils=vhtfm._dict(
				now_datetime=vhtfm.utils.now_datetime,
				add_to_date=vhtfm.utils.add_to_date,
				get_datetime=vhtfm.utils.get_datetime,
				now=vhtfm.utils.now,
			),
		)
	)
...
def is_transition_condition_satisfied(transition, doc) -> bool:
	if not transition.condition:
		...
		return vhtfm.safe_eval(transition.condition, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if not transition:
		vhtfm.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		vhtfm.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		vhtfm.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = vhtfm.bold(current_state)
		bold_next = vhtfm.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return vhtfm.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return vhtfm.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	if len(docnames) < 20:
		...
		vhtfm.msgprint(_("Bulk {0} is enqueued in background.").format(action), alert=True)
		vhtfm.enqueue(
		...
		vhtfm.throw(_("Bulk approval only support up to 500 documents."), title=_("Too Many Documents"))
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(vhtfm.get_doc(doctype, docname), action)
			vhtfm.db.commit()
		except Exception as e:
			if not vhtfm.message_log:
			...
			if not message_dict:
				if vhtfm.message_log:
					messages = vhtfm.get_message_log()
					for message in messages:
						vhtfm.message_log.pop()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{vhtfm.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(vhtfm.session.user, doc, t)
			]
			if not actions:
				return []
...
def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = vhtfm.get_doc("Workflow", workflow_name)

...
from vhtfm.utils.data import cstr, get_system_timezone, now_datetime
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = vhtfm.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			client = vhtfm.get_doc("OAuth Client", client_id)
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = (
			cstr(vhtfm.db.get_value("OAuth Client", client_id, "redirect_uris"))
			.strip()
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		return vhtfm.db.get_value("OAuth Client", client_id, "default_redirect_uri")
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = vhtfm.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = vhtfm.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in vhtfm.form_dict:
				oc = vhtfm.get_doc(
					"OAuth Client",
					vhtfm.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": vhtfm.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in vhtfm.form_dict:
				oc = vhtfm.get_doc(
					"OAuth Client",
					vhtfm.db.get_value("OAuth Bearer Token", vhtfm.form_dict["token"], "client"),
				)
			else:
				oc = vhtfm.get_doc(
					"OAuth Client",
					vhtfm.db.get_value(
						"OAuth Bearer Token",
						vhtfm.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return vhtfm.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = vhtfm.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = vhtfm.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		validcodes = vhtfm.get_all(
			"OAuth Authorization Code",
			filters={"client": client_id, "validity": "Valid"},
		)

		if code in [vcode["name"] for vcode in validcodes]:
			request.scopes = vhtfm.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = vhtfm.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = vhtfm.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = vhtfm.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if vhtfm.db.exists("OAuth Authorization Code", code):
					vhtfm.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					vhtfm.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = vhtfm.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = vhtfm.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = vhtfm.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else vhtfm.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = vhtfm.session.user
		...
		return vhtfm.db.get_value("OAuth Client", request.client["name"], "default_redirect_uri")

	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		# Authorization codes are use once, invalidate it when a Bearer token
		# has been acquired.

		vhtfm.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		vhtfm.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = vhtfm.get_doc("OAuth Bearer Token", token)
		is_token_valid = (now_datetime() < otoken.expiration_time) and otoken.status != "Revoked"
		client_scopes = vhtfm.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = vhtfm.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			vhtfm.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			vhtfm.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			vhtfm.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		vhtfm.db.commit()
	...
	def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
		...
		otoken = vhtfm.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token, "status": "Active"})
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether vhtfm server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = vhtfm.get_doc("User", request.user)
		...
		return vhtfm.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if vhtfm.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return vhtfm.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = vhtfm.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = vhtfm.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = vhtfm.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if vhtfm.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = vhtfm.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = vhtfm.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "vhtfm"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == vhtfm.session.user

			except Exception:
				return False

		elif vhtfm.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	vhtfm.db.delete("OAuth Authorization Code", {"validity": "Invalid"})
	vhtfm.db.delete("OAuth Bearer Token", {"status": "Revoked"})


def get_client_scopes(client_id):
	scopes_string = vhtfm.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	if user.user_image:
		if vhtfm.utils.validate_url(user.user_image, valid_schemes=valid_url_schemes):
			picture = user.user_image
		else:
			picture = urljoin(vhtfm_server_url, user.user_image)

	return vhtfm._dict(
		{
			"sub": vhtfm.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "vhtfm"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": vhtfm.get_roles(user.name),
			"iss": vhtfm_server_url,
		}
	)
...
def generate_json_error_response(e):
	...
	vhtfm.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(vhtfm.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return vhtfm.get_value("Social Login Key", "vhtfm", "base_url") or request_url

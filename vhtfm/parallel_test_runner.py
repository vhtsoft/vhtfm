...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = vhtfm.utils.cint(build_number) or 1
		self.total_builds = vhtfm.utils.cint(total_builds)
	...
	def before_test_setup(self):
		...
		for fn in vhtfm.get_hooks("before_tests", app_name=self.app):
			vhtfm.get_attr(fn)()

		test_module = vhtfm.get_module(f"{self.app}.tests")
	...
	def run_tests_for_file(self, file_info):
		...
		from vhtfm.deprecation_dumpster import compat_preload_test_records_upfront
	...
	def get_module(self, path, filename):
		app_path = vhtfm.get_app_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/vhtfm/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or vhtfm.generate_hash(length=10)

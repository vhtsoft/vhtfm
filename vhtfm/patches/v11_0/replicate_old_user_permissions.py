...
import vhtfm
from vhtfm.permissions import get_valid_perms
from vhtfm.utils import cint


def execute():
	vhtfm.reload_doctype("User Permission")
	user_permissions = vhtfm.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not vhtfm.db.has_column("User Permission", "applicable_for") and vhtfm.db.has_column(
...
def get_linked_doctypes(doctype):
	from vhtfm.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in vhtfm.get_meta(doctype).get_table_fields()]

...
def execute():
	...
	all_tables = vhtfm.db.get_tables()
	final_deletion_map = vhtfm._dict()

	for table in all_tables:
		indexes_to_keep_map = vhtfm._dict()
		indexes_to_delete = []
		index_info = vhtfm.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				vhtfm.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")

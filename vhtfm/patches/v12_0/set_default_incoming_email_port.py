...
def execute():
	...
	"""
	vhtfm.reload_doc("email", "doctype", "email_domain", force=True)
	vhtfm.reload_doc("email", "doctype", "email_account", force=True)
...
def setup_incoming_email_port_in_email_domains():
	email_domains = vhtfm.get_all("Email Domain", ["incoming_port", "use_imap", "use_ssl", "name"])

...
from vhtfm.utils.dashboard import get_dashboards_with_link


def execute():
	if (
		not vhtfm.db.table_exists("Dashboard Chart")
		or not vhtfm.db.table_exists("Number Card")
		or not vhtfm.db.table_exists("Dashboard")
	):
		return

	vhtfm.reload_doc("desk", "doctype", "dashboard_chart")
	vhtfm.reload_doc("desk", "doctype", "number_card")
	vhtfm.reload_doc("desk", "doctype", "dashboard")
...
def get_modified_docs(doctype):
	return vhtfm.get_all(doctype, filters={"owner": "Administrator", "modified_by": ["!=", "Administrator"]})


def rename_modified_doc(docname, doctype):
	new_name = docname + " Custom"
	try:
		vhtfm.rename_doc(doctype, docname, new_name)

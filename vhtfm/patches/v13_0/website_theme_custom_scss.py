...
def execute():
	...
	vhtfm.reload_doc("website", "doctype", "color")
	vhtfm.reload_doc("website", "doctype", "website_theme", force=True)

	for theme in vhtfm.get_all("Website Theme"):
...
def setup_color_record(doc):
	...
	for color_field in color_fields:
		...
		if not color_code or vhtfm.db.exists("Color", color_code):
			continue

...
def execute():
	...
	for name, content in vhtfm.get_all(
		...
		vhtfm.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in vhtfm.get_all(
		...
		if content and content.startswith("Removed "):
			vhtfm.db.set_value("Comment", name, "content", content[8:], update_modified=False)

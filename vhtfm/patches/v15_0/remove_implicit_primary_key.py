import vhtfm
from vhtfm.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			vhtfm.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			vhtfm.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({vhtfm.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if vhtfm.db.db_type == "mariadb":
		...
		values = (vhtfm.db.cur_db_name,)

	col_type = vhtfm.db.sql(query, values)

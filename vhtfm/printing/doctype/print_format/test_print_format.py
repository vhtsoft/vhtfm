...
class TestPrintFormat(IntegrationTestCase):
	def test_print_user(self, style=None):
		print_html = vhtfm.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: PrintFormat = vhtfm.get_doc("Print Format", self.globalTestRecords["Print Format"][0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = vhtfm.conf.developer_mode
		vhtfm.conf.developer_mode = True
		export_path = doc.export_doc()
		vhtfm.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = vhtfm.parse_json(f.read())

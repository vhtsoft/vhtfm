vhtfm.pages["print"].on_page_load = function (wrapper) {
	vhtfm.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new vhtfm.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = vhtfm.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!vhtfm.route_options || !vhtfm.route_options.frm) {
			vhtfm.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = vhtfm.get_doc(doctype, docname);
				vhtfm.model.with_doctype(doctype, () => {
					frm.meta = vhtfm.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = vhtfm.route_options.frm.doctype
				? vhtfm.route_options.frm
				: vhtfm.route_options.frm.frm;
			vhtfm.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

vhtfm.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${vhtfm.render_template("print_skeleton_loading")}
		...
		this.print_settings = vhtfm.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = vhtfm.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (vhtfm.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (Object.keys(vhtfm.boot.versions).includes("print_designer")) {
			...
			this.page.add_inner_message(`
			<a style="line-height: 2.4" href="https://vhtfmcloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
				${__("Try the new Print Designer")}
			</a>
			`);
		}
	...
	set_breadcrumbs() {
		vhtfm.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		vhtfm
			.xcall("vhtfm.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			vhtfm.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				vhtfm.set_route("print-format-builder-beta", print_format.name);
			} else {
				vhtfm.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		vhtfm.prompt(
			...
			(data) => {
				...
				vhtfm.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			vhtfm.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || vhtfm.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = vhtfm.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = vhtfm.urllib.get_base_url();
		let print_css = vhtfm.assets.bundled_asset(
			"print.bundle.css",
			vhtfm.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", vhtfm.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		vhtfm.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					vhtfm.ui.form
						...
						.then(vhtfm.ui.form.qz_success)
						.catch((err) => {
							vhtfm.ui.form.qz_fail(err);
						});
				});
			} else {
				vhtfm.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			vhtfm.call({
				method: "vhtfm.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		vhtfm.call({
			method: "vhtfm.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await vhtfm.xcall("vhtfm.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/vhtfm.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
			...
			this.render_page("/api/method/vhtfm.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = vhtfm.call({
			method: "vhtfm.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		vhtfm.call({
			method: "vhtfm.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		vhtfm.dom.set_style(style || vhtfm.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		vhtfm.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: vhtfm.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				vhtfm.throw(__("No Printer is Available."));
			}
		});
	}
};

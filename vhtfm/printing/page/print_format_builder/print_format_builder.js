vhtfm.pages["print-format-builder"].on_page_load = function (wrapper) {
	vhtfm.print_format_builder = new vhtfm.PrintFormatBuilder(wrapper);
	vhtfm.breadcrumbs.add("Setup", "Print Format");
};

vhtfm.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = vhtfm.get_route();
	if (route.length > 1) {
		vhtfm.model.with_doc("Print Format", route[1], function () {
			vhtfm.print_format_builder.print_format = vhtfm.get_doc("Print Format", route[1]);
			vhtfm.print_format_builder.refresh();
		});
	} else if (vhtfm.route_options) {
		if (vhtfm.route_options.make_new) {
			let { doctype, name, based_on, beta } = vhtfm.route_options;
			vhtfm.route_options = null;
			vhtfm.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			vhtfm.print_format_builder.print_format = vhtfm.route_options.doc;
			vhtfm.route_options = null;
			vhtfm.print_format_builder.refresh();
		}
	}
...
vhtfm.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = vhtfm.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(vhtfm.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = vhtfm.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			vhtfm.model.with_doc("Print Format", name, function (doc) {
				vhtfm.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = vhtfm.ui.form.make_control({
		...
		this.name_input = vhtfm.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				vhtfm.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		vhtfm.call({
			method: "vhtfm.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		vhtfm.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = vhtfm.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				vhtfm.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(vhtfm.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			vhtfm.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						vhtfm.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!vhtfm.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(vhtfm.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = vhtfm.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = vhtfm.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				vhtfm.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = vhtfm.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				vhtfm.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		vhtfm.call({
			...
			callback: function (r) {
				...
				vhtfm.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};

...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = vhtfm.boot.time_zone
		? vhtfm.boot.time_zone.user
		: vhtfm.sys_defaults.time_zone;
...
<template>
	<div class="control vhtfm-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="vhtfm.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="vhtfm.utils.icon('down', 'sm')"
		/>
	</div>
</template>

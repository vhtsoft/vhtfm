...
export const useStore = defineStore("form-builder-store", () => {
	...
	function get_df(fieldtype, fieldname = "", label = "") {
		...
		let df = vhtfm.model.get_new_doc(docfield);
		df.name = vhtfm.utils.get_random(8);
	...
	async function fetch() {
		...
		if (doctype.value.startsWith("new-doctype-") && !doc.value.fields?.length) {
			vhtfm.model.with_doctype("DocType").then(() => {
				vhtfm.listview_settings["DocType"].new_doctype_dialog();
			});
			// redirect to /doctype
		...
		if (!get_docfields.value.length) {
			let docfield = is_customize_form.value ? "Customize Form Field" : "DocField";
			if (!vhtfm.get_meta(docfield)) {
				await load_doctype_model(docfield);
			}
			let df = vhtfm.get_meta(docfield).fields;
		...
		nextTick(() => {
			...
			read_only.value =
				!is_customize_form.value && !vhtfm.boot.developer_mode && !doc.value.custom;
			preview.value = false;
		});
	...
	function validate_fields(fields, is_table) {
		...
		let not_allowed_in_list_view = ["Attach Image", ...vhtfm.model.no_value_type];
		if (is_table) {
		...
		fields.forEach((df) => {
			...
			if (["Link", ...vhtfm.model.table_fields].includes(df.fieldtype) && !df.options) {
				error_message = __(
			...
			if (df.in_global_search && vhtfm.model.no_value_type.includes(df.fieldtype)) {
				error_message = __(

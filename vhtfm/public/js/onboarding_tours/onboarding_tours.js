vhtfm.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new vhtfm.Driver({
			className: "vhtfm-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				vhtfm.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!vhtfm.boot.user.onboarding_status[this.tour.name] &&
								(vhtfm.boot.user.onboarding_status[this.tour.name] = {});
							vhtfm.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								vhtfm.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							vhtfm.call({
								method: "vhtfm.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(vhtfm.boot.user.onboarding_status),
				...
				if ($input) vhtfm.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await vhtfm.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: vhtfm.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
vhtfm.ui.init_onboarding_tour = () => {
	...
	typeof vhtfm.boot.onboarding_tours == "undefined" && vhtfm.boot.onboarding_tours == [];
	typeof vhtfm.boot.user.onboarding_status == "undefined" &&
		vhtfm.boot.user.onboarding_status == {};
	let route = vhtfm.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		vhtfm.boot.onboarding_tours &&
			vhtfm.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (vhtfm.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (vhtfm.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(vhtfm.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == vhtfm.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == vhtfm.ui.next_form_tour);
	if (current_tour) {
		...
		vhtfm.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (vhtfm.ui.currentTourInstance?.driver) {
		vhtfm.ui.currentTourInstance.driver_steps = [];
		vhtfm.ui.currentTourInstance.driver.reset(true);
		vhtfm.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (vhtfm.ui.currentTourInstance = new vhtfm.ui.OnboardingTour());

...
// depends on vhtfm.versions to manage versioning

vhtfm.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => vhtfm.assets.bundled_asset(item));

	return new Promise((resolve) => {
		vhtfm.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	...
	init_local_storage() {
		...
		if (vhtfm.boot) localStorage.metadata_version = vhtfm.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			vhtfm.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;

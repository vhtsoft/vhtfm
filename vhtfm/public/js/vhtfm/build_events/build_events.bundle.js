...
vhtfm.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						vhtfm.assets._executed = vhtfm.assets._executed.filter(
		...
		// update assets json
		vhtfm.call("vhtfm.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				vhtfm.boot.assets_json = r.message;

				if (vhtfm.hot_update) {
					vhtfm.hot_update.forEach((callback) => {
						callback();
					});
				}
			}

vhtfm.provide("vhtfm.data_import");

vhtfm.data_import.DataExporter = class DataExporter {
	...
	make_dialog(filetype = "CSV") {
		this.dialog = new vhtfm.ui.Dialog({
			...
			fields: [
				...
				...vhtfm.meta.get_table_fields(this.doctype).map((df) => {
	...
	export_records() {
		let method = "/api/method/vhtfm.core.doctype.data_import.data_import.download_template";
	...
	make_select_all_buttons() {
		...
		`);
		vhtfm.utils.bind_actions_with_object($select_all_buttons, this);
	...
	select_mandatory() {
		let mandatory_table_fields = vhtfm.meta
	...
	update_record_count_message() {
		...
		let count_method = {
			all: () => vhtfm.db.count(this.doctype),
			by_filter: () =>
				vhtfm.db.count(this.doctype, {
					filters: this.get_filters(),
				}),
	...
	get_multicheck_options(doctype, child_fieldname = null) {
		...
		if (meta.autoname && meta.autoname.startsWith("field:")) {
			...
			autoname_field = vhtfm.meta.get_field(doctype, fieldname);
		}
...
export function get_columns_for_picker(doctype) {
	...
	const exportable_fields = (df) => {
		let keep = true;
		if (vhtfm.model.no_value_type.includes(df.fieldtype)) {
			keep = false;
		}
	...
	// parent
	let doctype_fields = vhtfm.meta.get_docfields(doctype).filter(exportable_fields);
	...
	// children
	const table_fields = vhtfm.meta.get_table_fields(doctype);
	table_fields.forEach((df) => {
		const cdt = df.options;
		const child_table_fields = vhtfm.meta.get_docfields(cdt).filter(exportable_fields);

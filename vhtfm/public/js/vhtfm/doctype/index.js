...
*/
vhtfm.model.DocTypeController = class DocTypeController extends vhtfm.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		vhtfm.meta.docfield_map[
	...
	show_db_utilization() {
		...
		vhtfm
			.xcall("vhtfm.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = vhtfm.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			vhtfm.model.with_doctype(link_doctype, () => {
				let fields = vhtfm.meta

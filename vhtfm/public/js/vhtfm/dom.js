...
vhtfm.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + vhtfm.dom.id_count;
		vhtfm.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + vhtfm.dom.id_count;
		$ele.attr("id", id);
		vhtfm.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = vhtfm.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		vhtfm.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!vhtfm.dom.freeze_count) return; // anything open?
		vhtfm.dom.freeze_count--;
		if (!vhtfm.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				vhtfm.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
vhtfm.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
vhtfm.run_serially = function (tasks) {
...
vhtfm.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
vhtfm.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

vhtfm.scrub = vhtfm.slug = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

vhtfm.unscrub = function (txt) {
	return vhtfm.model.unscrub(txt);
};
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${vhtfm.utils.icon("close", "sm", "es-icon")}
			</span>
		`);
...
vhtfm.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${vhtfm.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${vhtfm.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
vhtfm.is_online = function () {
	if (vhtfm.boot.developer_mode == 1) {
...
vhtfm.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + vhtfm.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = vhtfm.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = vhtfm.assets.bundled_asset("desk.bundle.css");

	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="vhtfm.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && vhtfm.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: vhtfm.utils.icon("folder-open", "md"),
		closed: vhtfm.utils.icon("folder-normal", "md"),
		leaf: vhtfm.utils.icon("primitive-dot", "xs"),
		search: vhtfm.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return vhtfm.utils.get_form_link("File", filename);
};

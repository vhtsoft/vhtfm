vhtfm.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='vhtfm-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = vhtfm.perm.get_field_display_status(
			this.df,
			vhtfm.model.get_doc(this.doctype, this.docname),
		...
		let value = vhtfm.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(vhtfm.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!vhtfm.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!vhtfm.views || !vhtfm.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new vhtfm.views.TranslationManager({
						df: this.df,

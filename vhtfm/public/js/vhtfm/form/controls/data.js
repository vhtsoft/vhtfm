vhtfm.provide("vhtfm.phone_call");

vhtfm.ui.form.ControlData = class ControlData extends vhtfm.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && vhtfm.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = vhtfm.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${vhtfm.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					vhtfm.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", vhtfm.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = vhtfm.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = vhtfm.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
vhtfm.ui.form.ControlReadOnly = vhtfm.ui.form.ControlData;

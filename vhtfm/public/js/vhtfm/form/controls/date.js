vhtfm.ui.form.ControlDate = class ControlDate extends vhtfm.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(vhtfm.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		...
		let lang = "en";
		vhtfm.boot.user && (lang = vhtfm.boot.user.language);
		...
		this.date_format = vhtfm.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: vhtfm.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return vhtfm.datetime
			.convert_to_system_tz(vhtfm.datetime.now_date(true), false)
			.toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(vhtfm.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(vhtfm.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(vhtfm.datetime.now_time());
				}
				return false;
			}
		});
	...
	eval_expression(value, type) {
		...
		switch (type) {
			case "date":
				...
				value = vhtfm.datetime.user_to_str(value, false);
				break;

			case "time":
				system_fmt = "HH:mm:ss";
				value = vhtfm.datetime.user_to_str(value, true);
				break;
	...
	format_for_input(value) {
		if (value) {
			return vhtfm.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !vhtfm.datetime.validate(value)) {
			let sysdefaults = vhtfm.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			vhtfm.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}

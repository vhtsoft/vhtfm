...
vhtfm.ui.form.recent_link_validations = {};

vhtfm.ui.form.ControlLink = class ControlLink extends vhtfm.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-clear no-decoration">
					${vhtfm.utils.icon("close", "xs", "es-icon")}
				</a>
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${vhtfm.utils.icon("arrow-right", "xs")}
				</a>
			</span>
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", vhtfm.utils.get_form_link(doctype, name));
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return vhtfm.get_route && vhtfm.get_route()[0] === "List"
				? vhtfm.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (vhtfm.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (vhtfm.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			vhtfm.utils.get_link_title(doctype, value) ||
			(await vhtfm.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			vhtfm.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			vhtfm.route_options = df.get_route_options_for_new_doc(this);
		} else {
			vhtfm.route_options = {};
		}

		// partially entered name field
		vhtfm.route_options.name_field = this.get_label_value();

		// reference to calling link
		vhtfm._from_link = vhtfm.utils.deep_clone(this);
		vhtfm._from_link_scrollY = $(document).scrollTop();

		vhtfm.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${vhtfm.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			vhtfm.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(vhtfm.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								vhtfm.ui.form.ControlLink.link_options &&
								vhtfm.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							vhtfm.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				vhtfm.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (vhtfm.model.can_select(doctype) && !vhtfm.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = vhtfm.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : vhtfm.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = vhtfm.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = vhtfm.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return vhtfm
				.xcall("vhtfm.client.validate_link", {
					doctype: options,
					docname: value,

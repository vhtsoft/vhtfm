vhtfm.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	vhtfm.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${vhtfm.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {

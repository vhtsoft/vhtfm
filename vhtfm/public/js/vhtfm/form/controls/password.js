vhtfm.ui.form.ControlPassword = class ControlPassword extends vhtfm.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(vhtfm.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(vhtfm.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		vhtfm.call({
			type: "POST",
			method: "vhtfm.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {

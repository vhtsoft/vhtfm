...
vhtfm.ui.form.ControlTextEditor = class ControlTextEditor extends vhtfm.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			vhtfm.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = vhtfm.ui.keys && vhtfm.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: vhtfm.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "vhtfm.desk.search.get_names_for_mentions";
				let values = await vhtfm.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? vhtfm.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return vhtfm.dom.remove_script_and_style(value);
	}

...
vhtfm.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${vhtfm.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !vhtfm.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(vhtfm.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(vhtfm.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		vhtfm.provide("vhtfm.route_options");
		vhtfm.route_options[fieldname] = this.frm.doc.name;
		vhtfm.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				vhtfm.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			vhtfm.route_options = this.get_document_filter(doctype);
			if (show_open && vhtfm.ui.notifications) {
				vhtfm.ui.notifications.show_open_count_list(doctype);
			}
		}

		vhtfm.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "vhtfm.desk.notifications.get_open_count";
		vhtfm.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new vhtfm.Chart("#heatmap-" + vhtfm.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new vhtfm.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}

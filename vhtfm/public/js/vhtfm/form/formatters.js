...
vhtfm.provide("vhtfm.form.formatters");

vhtfm.form.link_formatters = {};

vhtfm.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			vhtfm.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				vhtfm.meta.docfield_map[df.parent] &&
				vhtfm.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return vhtfm.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(vhtfm.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(vhtfm.boot.sysdefaults && vhtfm.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return vhtfm.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return vhtfm.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		...
		if (cstr(docfield.options).trim() === "File Size") {
			return vhtfm.form.formatters.FileSize(value);
		}
		return vhtfm.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		...
		const precision =
			...
			cint(vhtfm.boot.sysdefaults && vhtfm.boot.sysdefaults.float_precision) ||
			2;
		return vhtfm.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		var currency = vhtfm.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || vhtfm.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					vhtfm.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return vhtfm.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = vhtfm.utils.get_link_title(doctype, value);
		...
		if (vhtfm.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = vhtfm.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (vhtfm.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					vhtfm.router.slug(doctype)
	...
	Date: function (value) {
		if (!vhtfm.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = vhtfm.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				vhtfm.datetime.str_to_user(value[0]),
				vhtfm.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(vhtfm.datetime.convert_to_user_tz(value)).format(
				vhtfm.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(vhtfm.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = vhtfm.utils.replace_newlines(value);
			}
		}

		return vhtfm.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = vhtfm.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = vhtfm.utils.get_duration_options(docfield);
			value = vhtfm.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return vhtfm.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = vhtfm.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = vhtfm.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${vhtfm.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${vhtfm.utils.icon(value, "md")}</div>
...
vhtfm.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return vhtfm.form.formatters[fieldtype.replace(/ /g, "")] || vhtfm.form.formatters.Data;
};

vhtfm.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || vhtfm.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = vhtfm.dom.remove_script_and_style(formatted);

	return formatted;
};

vhtfm.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = vhtfm.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return vhtfm.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
vhtfm.form.link_formatters["User"] = function (value, doc, docfield) {

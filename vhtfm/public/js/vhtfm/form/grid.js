...
vhtfm.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
};

vhtfm.ui.form.close_grid_form = function () {
	var open_form = vhtfm.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (vhtfm.ui.form.editable_row) {
		vhtfm.ui.form.editable_row.toggle_editable_row(false);
	}
};
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = vhtfm.utils.debounce(this.debounced_refresh, 100);
	}
	...
	make() {
		...
		vhtfm.utils.bind_actions_with_object(this.wrapper, this);
	...
	set_doc_url() {
		let unsupported_fieldtypes = vhtfm.model.no_value_type.filter(
			(x) => vhtfm.model.table_fields.indexOf(x) === -1
		);
	...
	delete_all_rows() {
		vhtfm.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		vhtfm.utils.scroll_to(this.wrapper);
	}
	...
	debounced_refresh_remove_rows_button = vhtfm.utils.debounce(
		this.refresh_remove_rows_button,
		100
	);
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = vhtfm.perm.get_field_display_status(
				this.df,
				this.frm.doc,
				this.perm
			);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = vhtfm.meta.get_docfield(
				this.frm.doctype,
				this.df.fieldname,
				this.frm.docname
			);
			...
			if (this.df.options) {
				this.df =
					vhtfm.meta.get_docfield(this.df.options, this.df.fieldname) ||
					this.df ||
					null;
			}
		}
		...
		if (this.doctype && this.frm) {
			this.docfields = vhtfm.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = vhtfm.utils.string_to_boolean(value);
			...
			let formatted_duration = vhtfm.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = vhtfm.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach((column) => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != vhtfm.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return vhtfm.meta.get_docfield(
			this.doctype,
			fieldname,
	...
	duplicate_row(d, copy_doc) {
		...
		const docfields = vhtfm.get_meta(this.doctype).fields || [];
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (
				...
				!vhtfm.model.layout_fields.includes(df.fieldtype)
			) {
				if (df.columns) {
				...
				if (
					...
					const docfield = vhtfm.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11 && vhtfm.model.is_non_std_field(df.fieldname)) {
					if (
						passes < 3 &&
	...
	setup_user_defined_columns() {
		...
		let user_settings = vhtfm.get_user_settings(this.frm.doctype, "GridView");
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column = vhtfm.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = vhtfm.meta.get_docfield(this.df.options, link);
		...
		btn.on("click", () => {
			new vhtfm.ui.form.LinkSelector({
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname)?.allow_bulk_edit) {
			...
			const value_formatter_map = {
				Date: (val) => (val ? vhtfm.datetime.user_to_str(val) : val),
				Int: (val) => cint(val),
				Check: (val) => cint(val),
				Float: (val) => flt(val),
			...
			$(this.wrapper)
				...
				.on("click", () => {
					new vhtfm.ui.FileUploader({
						...
						on_success(file) {
							var data = vhtfm.utils.csv_to_array(
								vhtfm.utils.get_decoded_string(file.dataurl)
							);
								...
								if (i > 6) {
									...
									if (!blank_row) {
										...
										$.each(row, (ci, value) => {
											...
											var df = vhtfm.meta.get_docfield(
												me.df.options,
												fieldname
											);
											if (df) {
	...
	setup_download() {
		let title = this.df.label || vhtfm.model.unscrub(this.df.fieldname);
		$(this.wrapper)
			.find(".grid-download")
			.removeClass("hidden")
			.on("click", () => {
				...
				data.push(["------"]);
				$.each(vhtfm.get_meta(this.df.options).fields, (i, df) => {
					// don't include the read-only field in the template
					if (vhtfm.model.is_value_type(df.fieldtype)) {
						data[1].push(df.label);
						data[2].push(df.fieldname);
						let description = (df.description || "") + " ";
						if (df.fieldtype === "Date") {
							description += vhtfm.boot.sysdefaults.date_format;
						}
				...
				$.each(this.frm.doc[this.df.fieldname] || [], (i, d) => {
					...
					$.each(data[2], (i, fieldname) => {
						...
						if (docfields[i].fieldtype === "Date" && value) {
							value = vhtfm.datetime.str_to_user(value);
						}
				...
				vhtfm.tools.downloadify(data, null, title);
				return false;
			});
	}

...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			vhtfm.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = vhtfm.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				vhtfm
					.run_serially([
						...
						() => {
							vhtfm.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		vhtfm.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					vhtfm.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				vhtfm.render(this.grid.template, {
					doc: this.doc ? vhtfm.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			vhtfm.utils.only_allow_num_decimal(this.row_index.find("input"));
		}
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col pointer">
					<a>${vhtfm.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new vhtfm.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = vhtfm.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = vhtfm.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = vhtfm.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-1' style='padding-top: 4px;'>
								<a style='cursor: grabbing;'>${vhtfm.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					vhtfm.throw(__("Column width cannot be zero."));
				}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					vhtfm.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		vhtfm.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			vhtfm.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		vhtfm.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			vhtfm.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? vhtfm.format(this.doc[df.fieldname], df, null, this.doc)
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = vhtfm.utils.eval(expression.substr(5), { doc, parent });
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			vhtfm.utils.only_allow_num_decimal($search_input);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $(
			...
			.on("click", function (event) {
				if (vhtfm.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (vhtfm.ui.form.editable_row && vhtfm.ui.form.editable_row !== this) {
			...
			vhtfm.ui.form.editable_row = this;
			return false;
		} else {
			...
			vhtfm.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = vhtfm.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { ESCAPE, TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = vhtfm.ui.keyCode;
	...
	show_form() {
		...
		vhtfm.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!vhtfm.dom.is_element_in_viewport(this.wrapper) &&
			!vhtfm.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			vhtfm.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!vhtfm.dom.is_element_in_modal(this.row)) {
			vhtfm.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = vhtfm.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = vhtfm.format(this.doc[fieldname], df, null, this.frm.doc);
		}

...
vhtfm.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			vhtfm.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (results.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(vhtfm.model.can_create(me.doctype)
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
						...
						if (!updated) {
							...
							vhtfm.run_serially([
								...
								() => {
									...
									return vhtfm.model.set_value(d.doctype, d.name, args);
								},
								() => vhtfm.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				vhtfm.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					vhtfm.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				vhtfm.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					vhtfm.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
vhtfm.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};

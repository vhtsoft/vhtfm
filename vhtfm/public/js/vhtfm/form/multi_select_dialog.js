vhtfm.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			vhtfm.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			vhtfm.new_doc(this.doctype, true);
		}
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new vhtfm.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = vhtfm.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = vhtfm.meta.get_docfield(
				this.doctype,
				...
				${
					head
						? `<span class="ellipsis text-muted" title="${__(
								vhtfm.model.unscrub(column)
						  )}">${__(vhtfm.model.unscrub(column))}</span>`
						: column !== "name"
						? `<span class="ellipsis result-row" title="${__(
								result[column] || ""
						  )}">${__(result[column] || "")}</span>`
						: `<a href="${
								"/app/" + vhtfm.router.slug(me.doctype) + "/" + result[column] ||
								""
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!vhtfm.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await vhtfm.call({
			type: "GET",
			method: "vhtfm.desk.search.search_widget",
			no_spinner: true,
			args: args,
		});

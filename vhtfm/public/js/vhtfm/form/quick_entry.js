...
vhtfm.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	vhtfm.db.get_doc(doctype, name).then((doc) => {
		vhtfm.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

vhtfm.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (vhtfm.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	vhtfm.quick_entry = new vhtfm.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return vhtfm.quick_entry.setup();
};

vhtfm.ui.form.QuickEntryForm = class QuickEntryForm extends vhtfm.ui.Dialog {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = vhtfm.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (vhtfm.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							vhtfm.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			vhtfm.call({
				method: "vhtfm.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (
						vhtfm.model.is_submittable(me.doctype) &&
						!vhtfm.model.has_workflow(me.doctype)
					) {
						vhtfm.run_serially([
							() => (me.dialog.working = true),
							() => {
	...
	process_after_insert(r) {
		...
		} else if (vhtfm._from_link) {
			vhtfm.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			vhtfm.run_serially([() => vhtfm.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			vhtfm.route_options = vhtfm.route_options || {};
			vhtfm.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		vhtfm.set_route("Form", this.doctype, this.doc.name);
	}

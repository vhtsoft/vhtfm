...
vhtfm.provide("vhtfm.ui.form.handlers");
...
vhtfm.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!vhtfm.ui.form.handlers[doctype]) {
		vhtfm.ui.form.handlers[doctype] = {};
	}
	if (!vhtfm.ui.form.handlers[doctype][fieldname]) {
		vhtfm.ui.form.handlers[doctype][fieldname] = [];
	}
	return vhtfm.ui.form.handlers[doctype][fieldname];
};

vhtfm.ui.form.on = vhtfm.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = vhtfm.ui.form.get_event_handler_list(doctype, fieldname);
...
vhtfm.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = vhtfm.ui.form.get_event_handler_list(doctype, fieldname);
...
vhtfm.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

vhtfm.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = vhtfm.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			vhtfm.ui.form.handlers[this.frm.doctype] &&
			vhtfm.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (vhtfm.ui.form.handlers[doctype] && vhtfm.ui.form.handlers[doctype][event_name]) {
			$.each(vhtfm.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (vhtfm.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					vhtfm.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && vhtfm.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		vhtfm.show_alert({ message: __("Error in Client Script."), indicator: "error" });

...
vhtfm.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_attachment(attachment) {
		...
		let file_label = `
			<a href="${file_url}" target="_blank" title="${vhtfm.utils.escape_html(file_name)}"
		...
		if (vhtfm.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function (target_id) {
				vhtfm.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}">
				${vhtfm.utils.icon(attachment.is_private ? "es-line-lock" : "es-line-unlock", "sm ml-0")}
			</a>`;

		$(`<div class="attachment-row"></div>`)
			.append(vhtfm.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		var me = this;
		return vhtfm.call({
			method: "vhtfm.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) vhtfm.msgprint(__("There were errors"));
					return;
				}

...
vhtfm.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = vhtfm.render_template("form_sidebar", {
			doctype: this.frm.doctype,
			frm: this.frm,
			can_write: vhtfm.model.can_write(this.frm.doctype, this.frm.docname),
		});
		...
		vhtfm.ui.form.setup_user_image_event(this.frm);
	...
	setup_keyboard_shortcuts() {
		...
		vhtfm.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			vhtfm.ui.form.set_user_image(this.frm);
		}
	}
	...
	refresh_web_view_count() {
		if (this.frm.doc.route && cint(vhtfm.boot.website_tracking_enabled)) {
			...
			vhtfm.utils.get_page_view_count(route).then((res) => {
				this.sidebar
					.find(".pageview-count")
					.removeClass("hidden")
	...
	refresh_creation_modified() {
		...
		let avatar_group = vhtfm.avatar_group(user_list, 5, {
			align: "left",
			overlap: true,
		});
			...
			vhtfm.call({
				...
				callback: function (res) {
					...
					el.on("click", function () {
						vhtfm.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	show_webhook_request_log_status() {
		...
		if (docinfo.webhook_request_log_exists) {
			...
			el.show();
			el.on("click", () => {
				vhtfm.set_route("List", "Webhook Request Log", {
					reference_doctype: this.frm.doc.doctype,
	...
	make_tags() {
		...
		this.frm.tags = new vhtfm.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new vhtfm.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new vhtfm.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new vhtfm.ui.form.Share({
			frm: this.frm,
	...
	make_review() {
		...
		if (vhtfm.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new vhtfm.ui.form.Review({
	...
	reload_docinfo(callback) {
		vhtfm.call({
			method: "vhtfm.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,

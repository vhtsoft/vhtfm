vhtfm.ui.form.SidebarUsers = class {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	show_in_sidebar(users, type, show_alert) {
		...
		users.current.forEach((username) => {
			if (username === vhtfm.session.user) {
				// current user
				return;
			}

			var user_info = vhtfm.user_info(username);
			sidebar_users.push({
		...
		if (sidebar_users.length) {
			...
			this.parent.append(
				vhtfm.render_template("users_in_sidebar", { users: sidebar_users })
			);
		} else {
	...
	show_alert(users, message) {
		if (users.length) {
			if (users.length === 1) {
				vhtfm.show_alert(__("{0} is currently {1}", [users[0], message]));
			} else {
				vhtfm.show_alert(
					__("{0} are currently {1}", [vhtfm.utils.comma_and(users), message])
				);
			}
		}
	}

...
vhtfm.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = vhtfm.boot.points;
	...
	update_points() {
		return vhtfm
			.xcall("vhtfm.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: vhtfm.session.user,
			})
			.then((data) => {
				vhtfm.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new vhtfm.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return vhtfm.msgprint(__("You do not have enough points"));
				}
				vhtfm
					.xcall("vhtfm.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = vhtfm.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${vhtfm.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${vhtfm.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${vhtfm.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${vhtfm.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;

...
vhtfm.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.show();
		let avatar_group = vhtfm.avatar_group(shared_users, 5, { align: "left", overlap: true });
	...
	render_shared(shared) {
		...
		$(
			vhtfm.render_template("set_sharing", {
				frm: this.frm,
				shared: this.shared,
				everyone: everyone,
			})
		...
		if (vhtfm.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = vhtfm.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body)
			...
			.on("click", function () {
				...
				vhtfm.call({
					method: "vhtfm.share.set_permission",
					args: {
						doctype: me.frm.doctype,
						name: me.frm.doc.name,
						user: user,

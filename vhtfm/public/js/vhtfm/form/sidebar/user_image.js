vhtfm.ui.form.set_user_image = function (frm) {
	...
	if (image) {
		image = window.cordova && image.indexOf("http") === -1 ? vhtfm.base_url + image : image;
		...
		image_section
			...
			.removeClass("hide")
			.find(".standard-image")
			.html(vhtfm.get_abbr(title));

		title_image.css("background-image", "").html(vhtfm.get_abbr(title));
...
vhtfm.ui.form.setup_user_image_event = function (frm) {
	// re-draw image on change of user image
	if (frm.meta.image_field) {
		vhtfm.ui.form.on(frm.doctype, frm.meta.image_field, function (frm) {
			vhtfm.ui.form.set_user_image(frm);
		});
	}

...
vhtfm.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || vhtfm.router.doctype_layout || __(this.doctype);
		this.meta = vhtfm.get_meta(this.doctype);
		this.settings = vhtfm.listview_settings[this.doctype] || {};
		this.user_settings = vhtfm.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = vhtfm.is_large_screen() ? 100 : 20;
		this.selected_page_count = this.page_length;
		this.data = [];
		this.method = "vhtfm.desk.reportview.get";

		this.can_create = vhtfm.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return vhtfm
			.call("vhtfm.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(vhtfm.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			vhtfm.model.std_fields_list.includes(fieldname) ||
			vhtfm.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = vhtfm.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!vhtfm.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return vhtfm.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (vhtfm.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new vhtfm.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		vhtfm.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !vhtfm.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new vhtfm.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$vhtfm_list = $('<div class="vhtfm-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$vhtfm_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$vhtfm_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$vhtfm_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$vhtfm_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => vhtfm.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return vhtfm.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	prepare_data(r) {
		...
		Object.assign(vhtfm.boot.user_info, data.user_info);
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = vhtfm.utils.debounce(
	...
	clear(refresh = true) {
		...
		return vhtfm.run_serially(promises).then(() => {
			this.trigger_refresh = true;
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && vhtfm.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						vhtfm.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
			...
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${vhtfm.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon button-icon">
						${vhtfm.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new vhtfm.ui.FilterGroup({
...
vhtfm.views.is_valid = (view_mode) => vhtfm.views.view_modes.includes(view_mode);

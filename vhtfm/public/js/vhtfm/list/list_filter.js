...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = vhtfm.user.has_role(["System Manager", "Administrator"]);
		this.filters = [];
		this.make();
		this.bind();
		this.refresh();
	...
	make() {
		...
		this.filter_input = vhtfm.ui.form.make_control({
			df: {
				fieldtype: "Data",
		...
		this.is_global_input = vhtfm.ui.form.make_control({
			df: {
				fieldtype: "Check",
				label: __("Is Global"),
			},
	...
	filter_template(filter) {
		return `<div class="list-link filter-pill list-sidebar-button btn btn-default" data-name="${
			...
			<a class="remove">${vhtfm.utils.icon("close")}</a>
		</div>`;
	}
	...
	save_filter(filter_name) {
		return vhtfm.db.insert({
			doctype: "List Filter",
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? "" : vhtfm.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return vhtfm.db.delete_doc("List Filter", name);
	}

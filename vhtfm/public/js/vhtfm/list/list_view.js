...
vhtfm.views.ListView = class ListView extends vhtfm.views.BaseList {
	static load_last_view() {
		const route = vhtfm.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = vhtfm.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			vhtfm.set_route(
				"list",
				vhtfm.router.doctype_layout || doctype,
				vhtfm.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = vhtfm.utils.debounce(
	...
	has_permissions() {
		return vhtfm.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			vhtfm.set_route("");
			vhtfm.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(vhtfm.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = vhtfm.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${vhtfm.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		vhtfm.msgprint(
			vhtfm.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? vhtfm.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						vhtfm.boot.link_title_doctypes.includes(df.options)
					) {
						vhtfm.model.with_doctype(df.options, () => {
							const meta = vhtfm.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = vhtfm.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = vhtfm.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				vhtfm.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !vhtfm.boot.read_only) {
			const doctype_name = __(vhtfm.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && vhtfm.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		vhtfm.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = vhtfm.meta.get_docfield.bind(null, this.doctype);
		...
		if (vhtfm.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (vhtfm.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (vhtfm.route_options && this.filter_area) {
	...
	before_render() {
		...
		vhtfm.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					vhtfm.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = vhtfm.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return vhtfm.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? vhtfm.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${vhtfm.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${vhtfm.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (vhtfm.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${vhtfm.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${vhtfm.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			vhtfm.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${vhtfm.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${vhtfm.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			vhtfm.router.slug(vhtfm.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(vhtfm.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(vhtfm.session.user);
		const title = liked_by.map((u) => vhtfm.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (vhtfm.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return vhtfm.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = vhtfm.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = vhtfm.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !vhtfm.utils.is_url(url)) {
			url = vhtfm.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = vhtfm.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = vhtfm.session.user;
				}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			vhtfm.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		vhtfm.realtime.doctype_subscribe(this.doctype);
		vhtfm.realtime.off("list_update");
		vhtfm.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		vhtfm.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		vhtfm.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = vhtfm.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return vhtfm.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (vhtfm.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (vhtfm.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					vhtfm.set_route("list", "data-import", {
		...
		if (
			vhtfm.model.can_create("Custom Field") &&
			vhtfm.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						vhtfm.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						vhtfm.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (vhtfm.user.has_role("System Manager") && vhtfm.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => vhtfm.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		vhtfm.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: vhtfm.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (vhtfm.model.has_workflow(this.doctype)) {
			const actions = vhtfm.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						vhtfm
							.xcall("vhtfm.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!vhtfm.model.has_workflow(this.doctype)) return;
		...
		vhtfm
			.xcall("vhtfm.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				vhtfm.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					vhtfm.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (vhtfm.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			vhtfm.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!vhtfm.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (vhtfm.model.can_cancel(doctype) && !vhtfm.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (vhtfm.model.can_delete(doctype) && !vhtfm.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && vhtfm.route_options) {
			params = new Map(Object.entries(vhtfm.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = vhtfm.meta.get_doctype_for_field(this.doctype, field);
			}
...
vhtfm.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return vhtfm.views.list_view[route];
};

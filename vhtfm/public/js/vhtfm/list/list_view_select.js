...
vhtfm.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = vhtfm.utils.to_title_case(route[2] || "");
		if (route.length > 2 && vhtfm.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			vhtfm.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: vhtfm.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && vhtfm.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(vhtfm.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => vhtfm.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					vhtfm.treeview_settings[this.doctype] ||
					vhtfm.get_meta(this.doctype).is_tree,
		...
		vhtfm.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => vhtfm.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return vhtfm.utils.to_title_case(vhtfm.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(vhtfm.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			vhtfm.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						vhtfm.set_route("list", doctype, "kanban", board.name);
					} else {
						vhtfm.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			vhtfm.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			vhtfm.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					vhtfm.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return vhtfm.db
			...
			.then((result) => {
				...
				if (vhtfm.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = vhtfm.boot.email_accounts;
	...
	slug() {
		return vhtfm.router.slug(vhtfm.router.doctype_layout || this.doctype);
	}
};

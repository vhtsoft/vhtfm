...
$.extend(vhtfm.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: vhtfm.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: vhtfm.session.user,
		};
		vhtfm.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			vhtfm.provide("vhtfm.model.docinfo." + doctype + "." + doc.name);
		}

		vhtfm.model.add_to_locals(doc);

		if (with_mandatory_children) {
			vhtfm.model.create_mandatory_children(doc);
		}
		...
		if (vhtfm.route_options && vhtfm.route_options.name_field) {
			var meta = vhtfm.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = vhtfm.route_options.name_field;
			} else if (meta.autoname && meta.autoname === "prompt") {
				doc.__newname = vhtfm.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = vhtfm.route_options.name_field;
			}

			delete vhtfm.route_options.name_field;
		}

		// set route options
		if (vhtfm.route_options && !doc.parent) {
			$.each(vhtfm.route_options, function (fieldname, value) {
				var df = vhtfm.meta.has_field(doctype, fieldname);
				if (
					df &&
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return vhtfm.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},

	get_new_name: function (doctype) {
		// random hash is added to idenity mislinked files when doc is not saved and file is uploaded.
		return vhtfm.router.slug(`new-${doctype}-${vhtfm.utils.get_random(10)}`);
	},

	set_default_values: function (doc, parent_doc) {
		let doctype = doc.doctype;
		let docfields = vhtfm.meta.get_docfields(doctype);
		let updated = [];

		// Table types should be initialized
		let fieldtypes_without_default = vhtfm.model.no_value_type.filter(
			(fieldtype) => !vhtfm.model.table_fields.includes(fieldtype)
		);
		docfields.forEach((f) => {
			if (
			...
			let v = vhtfm.model.get_default_value(f, doc, parent_doc);
			if (v) {
	...
	create_mandatory_children: function (doc) {
		var meta = vhtfm.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		vhtfm.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				vhtfm.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = vhtfm.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = vhtfm.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = vhtfm.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = vhtfm.defaults.get_user_default(df.fieldname);
				}
				if (
					!user_default &&
					...
					user_default = vhtfm.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = vhtfm.session.user;
			} else if (default_val == "user_fullname") {
				value = vhtfm.session.user_fullname;
			} else if (default_val == "Today") {
				value = vhtfm.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				if (df.fieldtype == "Time") {
					value = vhtfm.datetime.now_time();
				} else {
					// datetime fields are stored in system tz
					value = vhtfm.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = vhtfm.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (vhtfm.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: vhtfm.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? vhtfm.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = vhtfm.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = vhtfm.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = vhtfm.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (var key in doc) {
			// dont copy name and blank fields
			var df = vhtfm.meta.get_docfield(doc.doctype, key);
			...
			if (
				...
				if (vhtfm.model.table_fields.includes(df.fieldtype)) {
					for (var i = 0, j = value.length; i < j; i++) {
						var d = value[i];
						vhtfm.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
	...
	open_mapped_doc: function (opts) {
		...
		return vhtfm.call({
			type: "POST",
			method: "vhtfm.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function (r) {
				if (!r.exc) {
					...
					vhtfm.set_route("Form", r.message.doctype, r.message.name);
				}
			},
		});
	},
...
vhtfm.create_routes = {};
vhtfm.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			vhtfm.route_options = opts;
		}
		vhtfm.model.with_doctype(doctype, function () {
			if (vhtfm.create_routes[doctype]) {
				vhtfm.set_route(vhtfm.create_routes[doctype]).then(() => resolve());
			} else {
				vhtfm.ui.form

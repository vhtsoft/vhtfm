...
$.extend(vhtfm.model, {
	...
	init: function () {
		...
		vhtfm.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					vhtfm.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !vhtfm.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						vhtfm.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return vhtfm.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...vhtfm.model.std_fields_list, ...vhtfm.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(vhtfm.model.std_fields).concat(vhtfm.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				vhtfm.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			vhtfm.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = vhtfm.model.get_from_localstorage(doctype);
			...
			return vhtfm.call({
				method: "vhtfm.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						vhtfm.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						vhtfm.model.sync(meta);
					} else {
						vhtfm.model.set_in_localstorage(doctype, r.docs);
						meta = r.docs[0];
					}
					vhtfm.model.init_doctype(meta);

					if (r.user_settings) {
						// remember filters and other settings from last view
						vhtfm.model.user_settings[doctype] = JSON.parse(r.user_settings);
						vhtfm.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (meta) {
		if (meta.name === "DocType") {
			...
			vhtfm.meta.__doctype_meta = JSON.parse(JSON.stringify(meta));
		}
		...
		if (meta.__templates) {
			$.extend(vhtfm.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(vhtfm.get_doc(doctype, name));
			} else {
				return vhtfm.call({
					method: "vhtfm.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (vhtfm.model.docinfo[doctype] && vhtfm.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (vhtfm.model.docinfo[doctype] && vhtfm.model.docinfo[doctype][name]) {
			vhtfm.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return vhtfm.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = vhtfm.model.scrub(doctype);
		var module = vhtfm.model.scrub(locals.DocType[doctype].module);
		var app = vhtfm.boot.module_app[module];
	...
	can_create: function (doctype) {
		return vhtfm.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (vhtfm.boot.user) {
			return vhtfm.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (vhtfm.boot.user) {
			return vhtfm.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return vhtfm.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return vhtfm.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return vhtfm.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return vhtfm.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return vhtfm.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return vhtfm.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return vhtfm.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (vhtfm.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return vhtfm.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return vhtfm.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return vhtfm.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return vhtfm.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(vhtfm.sys_defaults.disable_document_sharing);

		if (disable_sharing && vhtfm.session.user !== "Administrator") {
			return false;
		}
		...
		return vhtfm.boot.user.can_share.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = vhtfm.meta.get_docfield(dt, fn, dn);

		let ret;
		if (vhtfm.model.table_fields.includes(df.fieldtype)) {
			ret = false;
	...
	get_list: function (doctype, filters) {
		...
		return vhtfm.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = vhtfm.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => vhtfm.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: vhtfm.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	vhtfm.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		vhtfm.provide("vhtfm.model.events." + doctype);
		if (!vhtfm.model.events[doctype][fieldname]) {
			vhtfm.model.events[doctype][fieldname] = [];
		}
		vhtfm.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise vhtfm.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return vhtfm.after_server_call();
					}
				});
			}
		}

		if (vhtfm.model.events[doc.doctype]) {
			enqueue_events(vhtfm.model.events[doc.doctype][fieldname]);
			enqueue_events(vhtfm.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = vhtfm.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = vhtfm.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return vhtfm.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (vhtfm.views.formview[doctype]) {
			delete vhtfm.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = vhtfm.get_meta(doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = vhtfm.get_meta(doctype).title_field;
		if (vhtfm.get_meta(doctype).autoname == "hash" && title_field) {
			const value = vhtfm.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		vhtfm.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return vhtfm.call({
				method: "vhtfm.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						vhtfm.utils.play_sound("delete");
						vhtfm.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = vhtfm.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			vhtfm.throw(
				__("Please specify") +
					": " +
					__(vhtfm.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return vhtfm.model.numeric_fieldtypes.includes(fieldtype);
	},
	...
	set_default_views_for_doctype(doctype, frm) {
		vhtfm.model.with_doctype(doctype, () => {
			...
			if (meta.is_calendar_and_gantt && vhtfm.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && vhtfm.boot.email_accounts.length) {
...
// legacy
vhtfm.get_doc = vhtfm.model.get_doc;
vhtfm.get_children = vhtfm.model.get_children;
vhtfm.get_list = vhtfm.model.get_list;

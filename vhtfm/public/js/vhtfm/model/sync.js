...
Object.assign(vhtfm.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `vhtfm.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					vhtfm.model.update_in_locals(d);
				} else {
					vhtfm.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					vhtfm.model.rename_after_save(d, i);
				}
			}
		}

		vhtfm.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		vhtfm.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			vhtfm.model.docinfo[d.doctype][d.name] = vhtfm.model.docinfo[d.doctype][d.localname];
			vhtfm.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!vhtfm.model.docinfo[doctype]) {
				vhtfm.model.docinfo[doctype] = {};
			}
			vhtfm.model.docinfo[doctype][name] = r.docinfo;

			// copy values to vhtfm.boot.user_info
			Object.assign(vhtfm.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) vhtfm.model.clear_doc(doc);

			doc.name = vhtfm.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				vhtfm.provide("vhtfm.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = vhtfm.meta.get_field(doc.doctype, fieldname);
			if (df && vhtfm.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = vhtfm.model.get_new_name(doc.doctype);
						}

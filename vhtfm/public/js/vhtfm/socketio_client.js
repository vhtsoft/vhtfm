...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = vhtfm.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${vhtfm.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		...
		vhtfm.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			vhtfm.flags.doc_subscribe = false;
		}, 1000);
...
vhtfm.realtime = new RealTimeClient();

// backward compatibility
vhtfm.socketio = vhtfm.realtime;

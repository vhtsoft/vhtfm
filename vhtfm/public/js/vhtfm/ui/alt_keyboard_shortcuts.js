vhtfm.provide("vhtfm.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
vhtfm.ui.keys.shortcut_groups = shortcut_groups;

vhtfm.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new vhtfm.ui.keys.AltShortcutGroup());
	}
...
vhtfm.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (vhtfm.ui.keys.key_map[e.which] || "").toLowerCase();
...
vhtfm.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = vhtfm.ui.keys.standard_shortcuts
			.filter((s) => !s.page)

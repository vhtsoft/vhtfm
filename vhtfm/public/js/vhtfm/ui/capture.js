// vhtfm.ui.Capture
// Author - Achilles Rasquinha <achilles@vhtfm.io>
 ...
 *
 * @example
 * vhtfm._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
vhtfm._.get_data_uri = (element) => {
...
/**
 * @description Vhtfm's Capture object.
 *
 * @example
 * const capture = vhtfm.ui.Capture()
 * capture.show()
 *
 ...
 */
vhtfm.ui.Capture = class {
	constructor(options = {}) {
		this.options = vhtfm.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...vhtfm.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(vhtfm.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (vhtfm.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(vhtfm.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					vhtfm.show_alert(vhtfm.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = vhtfm._.get_data_uri(me.video);
...
vhtfm.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
vhtfm.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
vhtfm.ui.Capture.TEMPLATE = `
<div class="vhtfm-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${vhtfm.ui.Capture.ERR_MESSAGE}</video>
	</div>

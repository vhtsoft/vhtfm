...
vhtfm.ui.color = {
	get: function (color_name, shade) {
		...
		return vhtfm.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(vhtfm.ui.color_map);
		if (color_names.includes(color_name)) {
			return vhtfm.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(vhtfm.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(vhtfm.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in vhtfm.ui.color_map) {
			const colors = vhtfm.ui.color_map[key];
...
vhtfm.ui.color_map = vhtfm.ui.color.get_color_map();

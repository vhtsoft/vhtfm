...
vhtfm.ui.open_dialogs = [];

vhtfm.ui.Dialog = class Dialog extends vhtfm.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				vhtfm.ui.form.get_open_grid_form?.()?.hide_form();

				if (vhtfm.ui.open_dialogs[vhtfm.ui.open_dialogs.length - 1] === me) {
					vhtfm.ui.open_dialogs.pop();
					if (vhtfm.ui.open_dialogs.length) {
						window.cur_dialog =
							vhtfm.ui.open_dialogs[vhtfm.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				vhtfm.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("vhtfm.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(vhtfm.utils.icon(icon));

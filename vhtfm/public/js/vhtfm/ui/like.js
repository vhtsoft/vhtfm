...
vhtfm.ui.is_liked = function (doc) {
	return vhtfm.ui.get_liked_by(doc).includes(vhtfm.session.user);
};

vhtfm.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

vhtfm.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	vhtfm.call({
		method: "vhtfm.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = vhtfm.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(vhtfm.session.user)) {
					liked_by.push(vhtfm.session.user);
				}

				if (add === "No" && liked_by.includes(vhtfm.session.user)) {
					liked_by = liked_by.filter((user) => user !== vhtfm.session.user);
				}
...
vhtfm.ui.click_toggle_like = function () {
	console.warn("`vhtfm.ui.click_toggle_like` is deprecated and has no effect.");
};

vhtfm.ui.setup_like_popover = ($parent, selector) => {
	if (vhtfm.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${vhtfm.avatar(user, "avatar-xs")}
							<span>${vhtfm.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					vhtfm.set_route(link_base + user);
				});

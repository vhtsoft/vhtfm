...
vhtfm.ui.Sidebar = class Sidebar {
	constructor() {
		this.items = {};
	...
	make_dom() {
		...
		this.wrapper = $(
			vhtfm.render_template("sidebar", {
				app_logo_url: vhtfm.boot.app_data[0].app_logo_url,
				app_title: __(vhtfm.boot.app_data[0].app_title),
			})
		).prependTo("body");
		...
		if (this.has_access) {
			this.wrapper
				...
				.removeClass("hidden")
				.on("click", () => {
					vhtfm.quick_edit("Workspace Settings");
				});
		}
	...
	set_all_pages() {
		this.sidebar_pages = vhtfm.boot.sidebar_pages;
	...
	set_default_app() {
		...
		vhtfm.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		vhtfm.current_app = vhtfm.boot.app_data[0].app_name;
	}
	...
	setup_app_switcher() {
		...
		vhtfm.boot.app_data_map = {};
		this.add_private_app(app_switcher_menu);

		for (var app of vhtfm.boot.app_data) {
			vhtfm.boot.app_data_map[app.app_name] = app;
	...
	add_private_app(app_switcher_menu) {
		...
		vhtfm.boot.app_data_map["private"] = app;
	...
	setup_select_app(app_switcher_menu) {
		app_switcher_menu.find(".app-item").on("click", (e) => {
			...
			if (route.startsWith("/app/private")) {
				...
				let ws = Object.values(vhtfm.workspace_map).find((ws) => ws.public === 0);
				route += "/" + vhtfm.router.slug(ws.title);
	...
	set_current_app(app) {
		...
		let app_data = vhtfm.boot.app_data_map[app] || vhtfm.boot.app_data_map["vhtfm"];
		...
		if (vhtfm.current_app === app) return;
		vhtfm.current_app = app;
		...
		this.add_app_item(
			{
				...
				app_route: "/",
				app_logo_url: "/assets/vhtfm/images/web.svg",
			},
			app_switcher_menu
		);
	}
	...
	setup_pages() {
		...
		if (this.all_pages) {
			vhtfm.workspaces = {};
			vhtfm.workspace_list = [];
			vhtfm.workspace_map = {};
			for (let page of this.all_pages) {
				vhtfm.workspaces[vhtfm.router.slug(page.name)] = {
					name: page.name,
					public: page.public,
				};
				if (!page.app && page.module) {
					page.app = vhtfm.boot.module_app[vhtfm.slug(page.module)];
				}
				vhtfm.workspace_map[page.name] = page;
				vhtfm.workspace_list.push(page);
			}
			this.make_sidebar();
		}
	}
	...
	make_sidebar() {
		...
		let app_workspaces = vhtfm.boot.app_data_map[vhtfm.current_app || "vhtfm"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (vhtfm.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!vhtfm.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = vhtfm.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = vhtfm.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + vhtfm.router.slug(item.name);
			} else {
				path = "/app/private/" + vhtfm.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? vhtfm.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(vhtfm.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			Sortable.create(container, {
				...
				onEnd: () => {
					...
					vhtfm.xcall(
						"vhtfm.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	reload() {
		return vhtfm.workspace.get_pages().then((r) => {

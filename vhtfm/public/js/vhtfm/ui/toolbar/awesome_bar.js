...
vhtfm.provide("vhtfm.tags");

vhtfm.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = vhtfm.router.make_url(
						vhtfm.router.convert_from_standard_route(
							vhtfm.router.get_route_from_arguments(
			...
			vhtfm.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(vhtfm.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(vhtfm.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				vhtfm.route_options = item.route_options;
			}
		...
		vhtfm.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				vhtfm.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = vhtfm.search.utils
			.get_creatables(txt)
			.concat(
				vhtfm.search.utils.get_search_in_list(txt),
				vhtfm.search.utils.get_doctypes(txt),
				vhtfm.search.utils.get_reports(txt),
				vhtfm.search.utils.get_pages(txt),
				vhtfm.search.utils.get_workspaces(txt),
				vhtfm.search.utils.get_dashboards(txt),
				vhtfm.search.utils.get_recent_pages(txt || ""),
				vhtfm.search.utils.get_executables(txt),
				vhtfm.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = vhtfm.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${vhtfm.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	vhtfm.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [vhtfm.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = vhtfm.container.page?.list_view?.doctype;
			if (!doctype) return;
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						vhtfm.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: vhtfm.utils.get_random(16),
				onclick: function () {
					vhtfm.msgprint(vhtfm.utils.get_random(16), __("Result"));
				},
			});
		}
	}

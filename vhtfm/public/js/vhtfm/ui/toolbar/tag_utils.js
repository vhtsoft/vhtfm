...
vhtfm.tags.utils = {
	get_tags: function (txt) {
		...
		if (!vhtfm.tags.tags) {
			vhtfm.tags.utils.fetch_tags();
			return [];
		}

		vhtfm.tags.tags.forEach((tag) => {
			const search_result = vhtfm.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						vhtfm.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		vhtfm.call({
			method: "vhtfm.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					vhtfm.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			vhtfm.call({
				method: "vhtfm.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {

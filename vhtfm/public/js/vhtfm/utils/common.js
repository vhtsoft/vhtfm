...
vhtfm.avatar = function (
	...
	if (user) {
		// desk
		user_info = vhtfm.user_info(user);
	} else {
		// website
		let full_name = title || vhtfm.get_cookie("full_name");
		user_info = {
			image: image_url === null ? vhtfm.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: vhtfm.get_abbr(full_name),
			color: vhtfm.get_palette(full_name),
		};
	}
...
vhtfm.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = vhtfm.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = vhtfm.get_palette(title);
...
vhtfm.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			vhtfm.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => vhtfm.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${vhtfm.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
vhtfm.ui.scroll = function (element, animate, additional_offset) {
...
vhtfm.get_palette = function (txt) {
	if (!txt) return vhtfm.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return vhtfm.palette[idx % 8];
};

vhtfm.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
vhtfm.gravatars = {};
vhtfm.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!vhtfm.gravatars[email_id]) {
		// TODO: check if gravatar exists
		vhtfm.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return vhtfm.gravatars[email_id];
};
...
vhtfm.get_cookie = function getCookie(name) {
	return vhtfm.get_cookies()[name];
};

vhtfm.get_cookies = function getCookies() {
...
vhtfm.is_mobile = function () {
	return $(document).width() < 768;
};

vhtfm.is_large_screen = function () {
	return $(document).height() > 1180;
};

vhtfm.utils.xss_sanitise = function (string, options) {
...
vhtfm.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = vhtfm.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(vhtfm.utils.xss_sanitise(url, { strategies: ["js"] }));
};

vhtfm.utils.strip_url = (url) => {
...
vhtfm.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: vhtfm.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	vhtfm.prompt(
		...
		function (values) {
			vhtfm.call({
				method: "vhtfm.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
vhtfm.utils.get_page_view_count = function (route) {
	return vhtfm.call("vhtfm.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};

...
vhtfm.provide("vhtfm.datetime");

vhtfm.defaultDateFormat = "YYYY-MM-DD";
vhtfm.defaultTimeFormat = "HH:mm:ss";
vhtfm.defaultDatetimeFormat = vhtfm.defaultDateFormat + " " + vhtfm.defaultTimeFormat;
moment.defaultFormat = vhtfm.defaultDateFormat;

vhtfm.provide("vhtfm.datetime");

$.extend(vhtfm.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (vhtfm.boot.time_zone && vhtfm.boot.time_zone.system && vhtfm.boot.time_zone.user) {
			date_obj = moment
				.tz(date, vhtfm.boot.time_zone.system)
				.clone()
				.tz(vhtfm.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(vhtfm.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (vhtfm.boot.time_zone && vhtfm.boot.time_zone.system && vhtfm.boot.time_zone.user) {
			date_obj = moment
				.tz(date, vhtfm.boot.time_zone.user)
				.clone()
				.tz(vhtfm.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (vhtfm.boot.time_zone && vhtfm.boot.time_zone.system && vhtfm.boot.time_zone.user) {
			return (
				moment().tz(vhtfm.boot.time_zone.system).utcOffset() ===
				moment().tz(vhtfm.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return vhtfm.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, vhtfm.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(vhtfm.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (vhtfm.sys_defaults && vhtfm.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (vhtfm.sys_defaults && vhtfm.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		if (!val) return "";
		const user_date_fmt = vhtfm.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = vhtfm.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, vhtfm.defaultTimeFormat);
			return date_obj.format(user_format);
		} else if (only_date) {
			let date_obj = moment(val, vhtfm.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			let date_obj = moment.tz(val, vhtfm.boot.time_zone.system);
			if (typeof val !== "string" || val.indexOf(" ") === -1) {
				user_format = user_date_fmt;
			} else {
				user_format = user_date_fmt + " " + user_time_fmt;
			}
			return date_obj.clone().tz(vhtfm.boot.time_zone.user).format(user_format);
		}
	},

	get_datetime_as_string: function (d) {
		let time_format = vhtfm?.boot?.sysdefaults?.time_format || vhtfm.defaultTimeFormat;
		let datetime_format = vhtfm.defaultDateFormat + " " + time_format;
		return moment(d).format(datetime_format);
	},

	user_to_str: function (val, only_time = false) {
		var user_time_fmt = vhtfm.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(vhtfm.defaultTimeFormat);
		}

		var user_fmt = vhtfm.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return vhtfm.datetime.str_to_obj(vhtfm.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return vhtfm.datetime._date(vhtfm.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return vhtfm.datetime._date(vhtfm.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return vhtfm.datetime._date(vhtfm.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return vhtfm.datetime._date(vhtfm.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = vhtfm.boot.time_zone?.system || vhtfm.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = vhtfm.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? vhtfm.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return vhtfm.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[vhtfm.defaultDateFormat, vhtfm.defaultDatetimeFormat, vhtfm.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = vhtfm.sys_defaults.first_day_of_the_week || "Sunday";

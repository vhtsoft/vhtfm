...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(vhtfm.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	const show_symbol_on_right =
		vhtfm.model.get_value(":Currency", currency, "symbol_on_right") ?? false;

	if (decimals === undefined) {
		decimals = vhtfm.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (vhtfm.boot) {
		if (vhtfm.boot.sysdefaults && vhtfm.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency) currency = vhtfm.boot.sysdefaults.currency;

		return vhtfm.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return vhtfm.currency_symbols[currency];
	}
}
...
function get_number_format(currency) {
	...
	return (
		(cint(sysdefaults?.use_number_format_from_currency) &&
			currency &&
			vhtfm.model.get_value(":Currency", currency, "number_format")) ||
...
function get_number_format_info(format) {
	var info = vhtfm.number_format_info[format];
...
function _round(num, precision, rounding_method) {
	rounding_method =
		rounding_method || vhtfm.boot.sysdefaults.rounding_method || "Banker's Rounding (legacy)";
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = vhtfm.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return vhtfm.meta.get_field_precision(df, doc);
	} else {
		return vhtfm.boot.sysdefaults.float_precision;
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(
		vhtfm.model.get_value(":Currency", currency, "smallest_currency_fraction_value")
	);

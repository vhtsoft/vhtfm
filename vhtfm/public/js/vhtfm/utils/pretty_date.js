function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = vhtfm.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(vhtfm.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = vhtfm.datetime.str_to_user ? vhtfm.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="vhtfm-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
vhtfm.datetime.comment_when = comment_when;
vhtfm.datetime.prettyDate = prettyDate;

vhtfm.datetime.refresh_when = function () {
	if (jQuery) {
		$(".vhtfm-timestamp").each(function () {

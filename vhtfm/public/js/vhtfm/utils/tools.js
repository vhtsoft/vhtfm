...
vhtfm.provide("vhtfm.tools");

vhtfm.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		vhtfm.msgprint(
			__("Export not allowed. You need {0} role to export.", [vhtfm.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = vhtfm.tools.to_csv(data);
...
vhtfm.markdown = function (txt) {
	if (!vhtfm.md2html) {
		vhtfm.md2html = new showdown.Converter({ tables: true });
	}
	...
	return vhtfm.md2html.makeHtml(txt);
};

vhtfm.tools.to_csv = function (data) {
	var res = [];

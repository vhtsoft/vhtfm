...
vhtfm.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[vhtfm.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return vhtfm.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[vhtfm.breadcrumbs.current_page()];
		...
		if (
			...
			vhtfm.workspace_map[breadcrumbs.workspace]?.app &&
			vhtfm.workspace_map[breadcrumbs.workspace]?.app != vhtfm.current_app
		) {
			vhtfm.app.sidebar.set_current_app(vhtfm.workspace_map[breadcrumbs.workspace].app);
		}
	...
	get last_route() {
		return vhtfm.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!vhtfm.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${vhtfm.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				vhtfm.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = vhtfm.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = vhtfm.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					vhtfm.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						vhtfm.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = vhtfm.get_meta(doctype);
		if (
			(doctype === "User" && !vhtfm.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = vhtfm.router.slug(vhtfm.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = vhtfm.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = vhtfm.get_route().slice(2).join("/");
		let docname_title;
		...
		let form_route = `/app/${vhtfm.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				vhtfm.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = vhtfm.get_route()[1];
		let dashboard_route = `/app/${vhtfm.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!vhtfm.visible_modules) {
			vhtfm.visible_modules = $.map(vhtfm.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete vhtfm.breadcrumbs.all[old_route_str];
		this.update();
	},

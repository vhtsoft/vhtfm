...
vhtfm.provide("vhtfm.views.calendar");
vhtfm.provide("vhtfm.views.calendars");

vhtfm.views.CalendarView = class CalendarView extends vhtfm.views.ListView {
	static load_last_view() {
		const route = vhtfm.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = vhtfm.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = vhtfm.views.Calendar[this.doctype] || {};
			this.calendar_name = vhtfm.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new vhtfm.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, vhtfm.views.calendar[this.doctype]);
				resolve(options);
			} else {
				vhtfm.model.with_doc("Calendar View", calendar_name, () => {
					const doc = vhtfm.get_doc("Calendar View", calendar_name);
					if (!doc) {
						vhtfm.show_alert(
						...
						vhtfm.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
vhtfm.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(vhtfm.boot.calendars, function (i, doctype) {
			if (vhtfm.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					vhtfm.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = vhtfm.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new vhtfm.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(vhtfm.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(vhtfm.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(vhtfm.utils.icon("today"));
	...
	get_system_datetime(date) {
		return vhtfm.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: vhtfm.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: vhtfm.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return vhtfm.call({
					method: me.get_events_method || "vhtfm.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (vhtfm.model.can_read(doctype)) {
					vhtfm.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = vhtfm.model.get_new_doc(me.doctype);
				...
				vhtfm.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = vhtfm.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = vhtfm.datetime.convert_to_user_tz(d.start);
				d.end = vhtfm.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!vhtfm.datetime.validate(d.start) && d.end) {
				d.start = vhtfm.datetime.add_days(d.end, -1);
			}

			if (d.start && !vhtfm.datetime.validate(d.end)) {
				d.end = vhtfm.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = vhtfm.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = vhtfm.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = vhtfm.ui.color.get(color_name, "extra-light");
			d.textColor = vhtfm.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!vhtfm.ui.color.validate_hex(color) || !color) {
				color = vhtfm.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = vhtfm.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		vhtfm.model.remove_from_locals(me.doctype, event.id);
		return vhtfm.call({
			method: me.update_event_method || "vhtfm.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					vhtfm.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {

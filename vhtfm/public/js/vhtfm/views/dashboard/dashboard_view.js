...
vhtfm.views.DashboardView = class DashboardView extends vhtfm.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				vhtfm.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	reset_dashboard_customization() {
		vhtfm.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			vhtfm.model.user_settings
		...
		const dialog = new vhtfm.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "vhtfm.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					vhtfm
						.xcall(
							"vhtfm.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		vhtfm.get_meta(this.doctype).fields.map((df) => {

...
vhtfm.views.GanttView = class GanttView extends vhtfm.views.ListView {
	get view_name() {
		return "Gantt";
	}
	...
	prepare_tasks() {
		...
		this.tasks = this.data.map(function (item) {
			...
			if (item.color && vhtfm.ui.color.validate_hex(item.color)) {
	...
	render_gantt() {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_click: (task) => {
				vhtfm.set_route("Form", task.doctype, task.id);
			},
			on_date_change: (task, start, end) => {
				if (!me.can_write) return;
				vhtfm.db.set_value(task.doctype, task.id, {
	...
	set_colors() {
		...
		let style = classes
			.map((c) => {
				...
				const progress_color = vhtfm.ui.color.get_contrast_color(bar_color);
				return `
	...
	get required_libs() {
		return [
			"assets/vhtfm/node_modules/vhtfm-gantt/dist/vhtfm-gantt.css",
			"assets/vhtfm/node_modules/vhtfm-gantt/dist/vhtfm-gantt.min.js",
		];
	}
};

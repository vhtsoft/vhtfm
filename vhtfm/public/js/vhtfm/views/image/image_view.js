/**
 * vhtfm.views.ImageView
 */
vhtfm.provide("vhtfm.views");

vhtfm.views.ImageView = class ImageView extends vhtfm.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = vhtfm.utils.escape_html(title);
	...
	get_attached_images() {
		return vhtfm
			.call({
				method: "vhtfm.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new vhtfm.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/vhtfm/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

vhtfm.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = vhtfm.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new vhtfm.PhotoSwipe(options);
		this.pswp.init();
	}
};

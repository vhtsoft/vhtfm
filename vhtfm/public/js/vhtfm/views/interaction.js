...
vhtfm.provide("vhtfm.interaction_settings");

vhtfm.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		vhtfm.model.with_doctype("Event", () => {
			let categories = vhtfm.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new vhtfm.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = vhtfm.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = vhtfm.session.user;
		}
		return vhtfm.call({
	...
	add_attachments(doc, attachments) {
		vhtfm.call({
			method: "vhtfm.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,

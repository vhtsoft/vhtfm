export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			vhtfm.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			vhtfm.call({
				method: "vhtfm.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
		...
		for (let fieldname of this.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order sortable"
					...
					<div class="row">
						<div class="col-md-1">
							${vhtfm.utils.icon("drag", "xs", "", "", "sortable-handle")}
						</div>
	...
	get_docfield(field_name) {
		return (
			vhtfm.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...vhtfm.model.no_value_type,
		];

		return vhtfm.model.std_fields

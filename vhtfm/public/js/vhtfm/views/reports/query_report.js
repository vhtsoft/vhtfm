...
vhtfm.provide("vhtfm.widget.utils");
vhtfm.provide("vhtfm.views");
vhtfm.provide("vhtfm.query_reports");

vhtfm.standard_pages["query-report"] = function () {
	var wrapper = vhtfm.container.add_page("query-report");
	...
	vhtfm.query_report = new vhtfm.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		vhtfm.query_report.show();
	});
};

vhtfm.views.QueryReport = class QueryReport extends vhtfm.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = vhtfm.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = vhtfm.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (vhtfm.get_route_str() == this.page_name) {
	...
	setup_events() {
		vhtfm.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					vhtfm.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (vhtfm.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, vhtfm.route_options);

		if (this.report_name !== vhtfm.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (vhtfm.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!vhtfm.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!vhtfm.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = vhtfm.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = vhtfm.dashboard_utils.get_dashboard_link_field();
		const set_standard = vhtfm.boot.developer_mode;

		const dialog = new vhtfm.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			vhtfm.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"vhtfm.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || vhtfm.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"vhtfm.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		vhtfm.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			vhtfm.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return vhtfm.model
			...
			.then(() => vhtfm.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (vhtfm.query_reports[this.report_name]) {
				this.report_settings = vhtfm.query_reports[this.report_name];
				resolve();
			} else {
				vhtfm
					.xcall("vhtfm.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						vhtfm.dom.eval(settings.script || "");
						vhtfm.after_ajax(() => {
							...
							vhtfm.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return vhtfm.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			vhtfm.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = vhtfm.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					vhtfm.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = vhtfm.route_options;
		...
		if (route_options) {
			...
			return vhtfm.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = vhtfm.call({
				method: "vhtfm.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = vhtfm.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			vhtfm.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = vhtfm.utils.get_query_string(vhtfm.get_route_str());
		return vhtfm.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(
				...
				function () {
					window.open(
						vhtfm.urllib.get_full_url(
							"/api/method/vhtfm.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = vhtfm.datetime.comment_when(doc.report_end_time);
			const days_old = vhtfm.datetime.get_day_diff(
				vhtfm.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					vhtfm.set_route(vhtfm.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		vhtfm
			.xcall(
				"vhtfm.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = vhtfm.warn(
					...
					vhtfm.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		vhtfm
			.xcall("vhtfm.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				vhtfm.call({
					method: "vhtfm.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				vhtfm.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > 1000000) {
			...
			this.toggle_message(true, `${vhtfm.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: vhtfm.boot.lang,
				translations: vhtfm.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: vhtfm.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: vhtfm.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: vhtfm.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new vhtfm.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = vhtfm.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new vhtfm.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = vhtfm.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = vhtfm.datetime.user_to_obj(keyword);
					const cellValue = vhtfm.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = vhtfm.get_meta(this.report_doc.ref_doctype);
		vhtfm.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		vhtfm.call("vhtfm.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = vhtfm.urllib.get_base_url();
		const print_css = vhtfm.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = vhtfm.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = vhtfm.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: vhtfm.boot.lang,
			layout_direction: vhtfm.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		vhtfm.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = vhtfm.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];

				if (vhtfm.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}

				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${vhtfm.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = vhtfm.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				for (const [key, value] of Object.entries(filters)) {
					const df = vhtfm.query_report.get_filter(key).df;
				...
				const args = {
					cmd: "vhtfm.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(vhtfm.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = vhtfm.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => vhtfm.set_route("Form", "Report", this.report_name),
				condition: () => vhtfm.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = vhtfm.ui.get_print_settings(
						false,
				...
				condition: () => vhtfm.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => vhtfm.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					vhtfm.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new vhtfm.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									vhtfm.model.with_doctype(doctype, () => {
										let options = vhtfm.meta
											.get_docfields(doctype)
											.filter(vhtfm.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = vhtfm.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + vhtfm.scrub(values.doctype)
									: df.fieldname,
							...
							vhtfm.call({
								method: "vhtfm.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				action: () =>
					vhtfm.set_route("List", "User Permission", {
						doctype: "Report",
						name: this.report_name,
					}),
				condition: () => vhtfm.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (vhtfm.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new vhtfm.ui.Dialog({
						...
						primary_action: (values) => {
							vhtfm.call({
								method: "vhtfm.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									vhtfm.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + vhtfm.scrub(new_column_data.doctype)] =

...
vhtfm.standard_pages["Workspaces"] = function () {
	var wrapper = vhtfm.container.add_page("Workspaces");
	...
	vhtfm.workspace = new vhtfm.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		vhtfm.workspace.show();
	});
};

vhtfm.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = vhtfm.workspace_block.blocks;
		...
		this.sidebar = vhtfm.app.sidebar;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, vhtfm.boot.sidebar_pages);
		this.has_access = vhtfm.boot.sidebar_pages.has_access;
		this.has_create_access = vhtfm.boot.sidebar_pages.has_create_access;
		this.show();
	...
	get_pages() {
		return vhtfm.xcall("vhtfm.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!vhtfm.router.current_route[0]) {
			vhtfm.route_flags.replace_route = true;
			vhtfm.set_route(vhtfm.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return vhtfm
			.call({
				method: "vhtfm.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return vhtfm.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (vhtfm.boot.user.default_workspace) {
			default_page = {
				name: vhtfm.boot.user.default_workspace.name,
				public: vhtfm.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = vhtfm.boot.module_app[vhtfm.router.slug(this._page.module)];
				}
				if (!app) app = "vhtfm";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await vhtfm.after_ajax(() => this.get_data(current_page));
			}
			...
			vhtfm.app.sidebar.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(vhtfm.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			vhtfm.boot.sidebar_pages = this.cached_pages;
			this.reload();
			vhtfm.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				vhtfm.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new vhtfm.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + vhtfm.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : vhtfm.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: vhtfm.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + vhtfm.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			vhtfm.call({
				method: "vhtfm.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						vhtfm.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!vhtfm.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: vhtfm.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				vhtfm.call({
					method: "vhtfm.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(vhtfm.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			vhtfm.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			vhtfm.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};

...
vhtfm.provide("vhtfm.web_form");
import EventEmitterMixin from "../../vhtfm/event_emitter";

export default class WebForm extends vhtfm.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		vhtfm.web_form = this;
		vhtfm.web_form.events = {};
		Object.assign(vhtfm.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		vhtfm.init_client_script && vhtfm.init_client_script();
		this.setup_listeners();
		vhtfm.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = vhtfm.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${vhtfm.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".vhtfm-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		vhtfm.form_dirty = false;

		vhtfm.call({
			type: "POST",
			method: "vhtfm.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					vhtfm.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						vhtfm.call({
							type: "POST",
							method: "vhtfm.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			vhtfm.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {

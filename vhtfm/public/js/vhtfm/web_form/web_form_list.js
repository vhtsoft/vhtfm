...
vhtfm.provide("vhtfm.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		vhtfm.web_form_list = this;
	...
	make_filters() {
		...
		vhtfm
			.call("vhtfm.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = vhtfm.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = vhtfm.utils.get_filter_from_json(this.condition_json, this.doctype);
			filter = vhtfm.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "vhtfm.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				vhtfm.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			vhtfm.has_permission(this.doctype, "", "create", () => {
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new vhtfm.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		vhtfm.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		vhtfm
			.call({
				type: "POST",
				method: "vhtfm.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
vhtfm.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = vhtfm.form.get_formatter(field.fieldtype);
			let value =

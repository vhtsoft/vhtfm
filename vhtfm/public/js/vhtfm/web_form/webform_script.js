...
vhtfm.ready(function () {
	let web_form_doc = vhtfm.web_form_doc;
	let reference_doc = vhtfm.reference_doc;
	...
	function show_login_prompt() {
		if (vhtfm.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new vhtfm.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(vhtfm.form.formatters.FileSize($(this).text()));
		});
	}

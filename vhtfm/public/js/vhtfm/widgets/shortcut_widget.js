...
export default class ShortcutWidget extends Widget {
	...
	setup_events() {
		this.widget.click((e) => {
			...
			let filters = vhtfm.utils.get_filter_from_json(this.stats_filter);
	...
	set_actions() {
		...
		$(vhtfm.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2")).appendTo(
			this.action_area
		);
		...
		let filters = vhtfm.utils.process_filter_expression(this.stats_filter);

		if (
			this.type == "DocType" &&
			this.doc_view != "New" &&
			!vhtfm.boot.single_types.includes(this.link_to)
		) {
			vhtfm.db
				.count(this.link_to, {
					filters: filters || [],

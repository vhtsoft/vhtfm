		...
		this.dialog = new vhtfm.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${vhtfm.utils.get_random(20)}`;
				}
	...
	get_title() {
		if (this.editing) {
			switch (this.type) {
				...
				default:
					return __("Edit {0}", [__(vhtfm.model.unscrub(this.type))]);
			}
		}
		...
		switch (this.type) {
			...
			default:
				return __("New {0}", [__(vhtfm.model.unscrub(this.type))]);
		}
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = vhtfm.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						this.dialog.fields_dict.link_to.get_query = () => {
							return {
								query: "vhtfm.core.report.permitted_documents_for_user.permitted_documents_for_user.query_doctypes",
								filters: {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						vhtfm.model.with_doctype(doctype, async () => {
							let meta = vhtfm.get_meta(doctype);

							if (doctype && vhtfm.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (vhtfm.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await vhtfm.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = vhtfm.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : vhtfm.model.unscrub(data.link_to);

		if (data.url) {
			let _url = data.url;
			if (data.url.startsWith("/")) {
				_url = vhtfm.urllib.get_base_url() + data.url;
			}
			!validate_url(_url) &&
				vhtfm.throw({
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "vhtfm.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && vhtfm.get_meta(this.document_type)) {
			vhtfm.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "vhtfm.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];

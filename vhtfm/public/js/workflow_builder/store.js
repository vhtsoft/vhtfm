...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await vhtfm.model.clear_doc("Workflow", workflow_name.value);
		await vhtfm.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = vhtfm.get_doc("Workflow", workflow_name.value);
		await vhtfm.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await vhtfm.model.with_doctype("Workflow");
			workflowfields.value = vhtfm.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await vhtfm.model.with_doctype("Workflow Document State");
			statefields.value = vhtfm.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await vhtfm.model.with_doctype("Workflow Transition");
			transitionfields.value = vhtfm.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await vhtfm.model.with_doctype(doc_type);
			workflow_doc_fields.value = vhtfm.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", vhtfm.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await vhtfm.call("vhtfm.client.save", { doc });
			vhtfm.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		vhtfm.breadcrumbs.clear();
		vhtfm.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}

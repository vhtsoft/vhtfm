...
import vhtfm
from vhtfm import sbool
from vhtfm.utils.response import Response

from .vhtfmclient import VhtfmClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = vhtfm.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			vhtfm.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = vhtfm.get_doc("Push Notification Settings")
		...
		token = vhtfm.generate_hash(length=48)
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/vhtfm.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = vhtfm.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = VhtfmClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = VhtfmClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(vhtfm.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(vhtfm.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(vhtfm.utils.get_url())
...
def auth_webhook():
	url = urlparse(vhtfm.utils.get_url()).hostname
	token = vhtfm.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(vhtfm.session.user, fcm_token)
	return {"success": success, "message": message}


@vhtfm.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(vhtfm.session.user, fcm_token)
	return {"success": success, "message": message}

...
def publish_progress(percent, title=None, doctype=None, docname=None, description=None, task_id=None):
	publish_realtime(
		...
		user=None if doctype and docname else vhtfm.session.user,
		doctype=doctype,
		docname=docname,
		task_id=task_id,
	)
...
def publish_realtime(
	...
	if not task_id and hasattr(vhtfm.local, "task_id"):
		task_id = vhtfm.local.task_id
	...
	if after_commit:
		if not hasattr(vhtfm.local, "_realtime_log"):
			vhtfm.local._realtime_log = []
			vhtfm.db.after_commit.add(flush_realtime_log)
			vhtfm.db.after_rollback.add(clear_realtime_log)

		params = [event, message, room]
		if params not in vhtfm.local._realtime_log:
			vhtfm.local._realtime_log.append(params)
	else:
		emit_via_redis(event, message, room)


def flush_realtime_log():
	for args in vhtfm.local._realtime_log:
		vhtfm.realtime.emit_via_redis(*args)

	clear_realtime_log()


def clear_realtime_log():
	if hasattr(vhtfm.local, "_realtime_log"):
		del vhtfm.local._realtime_log
...
def emit_via_redis(event, message, room):
	...
	from vhtfm.utils.background_jobs import get_redis_connection_without_auth
	...
	with suppress(redis.exceptions.ConnectionError):
		...
		r.publish(
			"events",
			vhtfm.as_json(
				{"event": event, "message": message, "room": room, "namespace": vhtfm.local.site}
			),
		)
...
def has_permission(doctype: str, name: str) -> bool:
	if not vhtfm.has_permission(doctype=doctype, doc=name, ptype="read"):
...
def get_user_info():
	return {
		"user": vhtfm.session.user,
		"user_type": vhtfm.session.data.user_type,

...
import vhtfm
from vhtfm import _
from vhtfm.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			vhtfm.throw("You must record one of jobs or requests")

	def store(self):
		vhtfm.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return vhtfm.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		vhtfm.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = vhtfm.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(vhtfm.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if vhtfm.local._recorder.config.capture_stack:
	...
	vhtfm.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from vhtfm.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	"""
	vhtfm.db.rollback()
	vhtfm.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(vhtfm.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = vhtfm.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		vhtfm.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	if vhtfm.cache.get_value(RECORDER_INTERCEPT_FLAG) or force:
		vhtfm.local._recorder = Recorder(force=force)


def dump():
	if hasattr(vhtfm.local, "_recorder"):
		vhtfm.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and vhtfm.request
			and self.config.request_filter in vhtfm.request.path
		):
			self.path = vhtfm.request.path
			self.cmd = vhtfm.local.form_dict.cmd or ""
			self.method = vhtfm.request.method
			self.headers = dict(vhtfm.local.request.headers)
			self.form_dict = vhtfm.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and vhtfm.job and self.config.jobs_filter in vhtfm.job.method:
			self.event_type = "Background Job"
			self.path = vhtfm.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = vhtfm.generate_hash(length=10)
	...
	def dump(self):
		...
		vhtfm.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		vhtfm.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		vhtfm.db._sql = vhtfm.db.sql
		vhtfm.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		vhtfm.db.sql = vhtfm.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(vhtfm.local, "_recorder"):
			vhtfm.local._recorder.cleanup()
			del vhtfm.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if vhtfm.session.user != "Administrator":
			vhtfm.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(vhtfm.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	vhtfm.cache.set_value(RECORDER_INTERCEPT_FLAG, 1, expires_in_sec=RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	vhtfm.cache.delete_value(RECORDER_INTERCEPT_FLAG)
	vhtfm.enqueue(post_process, now=vhtfm.flags.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = vhtfm.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(vhtfm.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(vhtfm.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	vhtfm.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	vhtfm.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		vhtfm.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = vhtfm.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		vhtfm.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		vhtfm.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)

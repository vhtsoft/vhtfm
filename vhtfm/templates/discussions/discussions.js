vhtfm.ready(() => {
	...
	vhtfm.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	vhtfm.realtime.init(window.socketio_port || "9000");

	vhtfm.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	vhtfm.realtime.on("update_message", (data) => {
		update_message(data);
	});

	vhtfm.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == vhtfm.session.user) {
	...
	} else if (topic.owner == vhtfm.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		vhtfm.call({
			method: "vhtfm.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = vhtfm.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	vhtfm.call({
		method: "vhtfm.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	vhtfm.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new vhtfm.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".vhtfm-control").removeClass("hide-control");

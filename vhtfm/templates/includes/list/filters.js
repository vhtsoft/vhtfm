function setup_list_filters() {
	...
	vhtfm.call('vhtfm.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = vhtfm.utils.get_query_params()[df.fieldname];
			const f = vhtfm.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(vhtfm.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = vhtfm.utils.make_query_string(query_params);
...
vhtfm.ready(setup_list_filters);

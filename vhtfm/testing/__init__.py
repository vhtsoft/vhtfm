"""
Vhtfm Testing Module

This module provides a comprehensive framework for running tests in Vhtfm applications.
...
- Integration with Vhtfm's hooks and test record creation system

Usage:
This module is typically used by Vhtfm's CLI commands for running tests, but can also
...
from vhtfm.utils.logger import create_handler as createVhtfmFileHandler
...
LOGGING_CONFIG = {
	...
	"loggers": {
		f"{__name__}": {
			"handlers": [],  # only log to the vhtfm handler
			"propagate": False,
		},
	},
}
...
handlers = createVhtfmFileHandler(__name__)

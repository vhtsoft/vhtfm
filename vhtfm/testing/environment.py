"""
This module handles the setup and teardown of the test environment for Vhtfm applications.
...
def _initialize_test_environment(site, config):
	...
	vhtfm.flags.print_messages = logger.getEffectiveLevel() < logging.INFO
	vhtfm.flags.tests_verbose = logger.getEffectiveLevel() < logging.INFO
...
def _cleanup_after_tests():
	...
	if not scheduler_disabled_by_user:
		vhtfm.utils.scheduler.enable_scheduler()
...
def _disable_scheduler_if_needed():
	...
	global scheduler_disabled_by_user
	scheduler_disabled_by_user = vhtfm.utils.scheduler.is_scheduler_disabled(verbose=False)
	if not scheduler_disabled_by_user:
		vhtfm.utils.scheduler.disable_scheduler()
...
def _decorate_all_methods_and_functions_with_type_checker():
	...
	def _decorate_callable(obj, parent_module):
		# whitelisted methods are already checked, see vhtfm.whitelist
		if getattr(obj, "__func__", obj) in vhtfm.whitelisted:
			return obj
		# Check if the function is already decorated
			...
			config = _get_config_from_pyproject(vhtfm.get_app_source_path(app))
	...
	for app in (apps := vhtfm.get_installed_apps()):
		...
		for _, submodule_name, _ in pkgutil.walk_packages(path=pkg.__path__, prefix=pkg.__name__ + "."):
			...
			submodule = vhtfm.get_module(submodule_name)
...
class IntegrationTestPreparation:
	...
	def _run_before_test_hooks(app: str, category: str):
		...
		for hook_function in vhtfm.get_hooks("before_tests", app_name=app):
			logger.info(f'Running "before_tests" hook function {hook_function}')
			vhtfm.get_attr(hook_function)()
	...
	def _create_global_test_record_dependencies(app: str, category: str):
		...
		try:
			test_module = vhtfm.get_module(f"{app}.tests")

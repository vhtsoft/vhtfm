...
from vhtfm.utils import cint, get_test_client, get_url

try:
	_site = vhtfm.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("vhtfm.app.get_site_name", return_value=self.site):
				header_patch = patch("vhtfm.get_request_header", new=patch_request_header)
...
class VhtfmAPITestCase(IntegrationTestCase):
...
class TestResourceAPI(VhtfmAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = vhtfm.get_doc({"doctype": "ToDo", "description": vhtfm.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			vhtfm.delete_doc_if_exists(cls.DOCTYPE, name)
		vhtfm.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = vhtfm._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": vhtfm.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = vhtfm.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "vhtfm.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(VhtfmAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/vhtfm.realtime.get_user_info
		response = self.get(self.method("vhtfm.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("vhtfm.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "vhtfm.tests.test_api.test"

		def get_message(resp, msg_type):
			return vhtfm.parse_json(vhtfm.parse_json(vhtfm.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "vhtfm.tests.test_api.test_array"
...
class TestReadOnlyMode(VhtfmAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": vhtfm.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(VhtfmAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["vhtfm.tests.test_api.before_request"],
				"after_request": ["vhtfm.tests.test_api.after_request"],
			}
		):
...
class TestResponse(VhtfmAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/vhtfm.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/vhtfm.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from vhtfm.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = vhtfm.generate_hash()
		file = vhtfm.get_doc(
			{
				"doctype": "File",

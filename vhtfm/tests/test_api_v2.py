...
from vhtfm.tests.test_api import VhtfmAPITestCase, suppress_stdout
...
class TestResourceAPIV2(VhtfmAPITestCase):
	version = "v2"
	DOCTYPE = "ToDo"
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "vhtfm")
	...
	def test_delete_document_non_existing(self):
		non_existent_doc = vhtfm.generate_hash(length=12)
...
class TestMethodAPIV2(VhtfmAPITestCase):
	version = "v2"
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("vhtfm.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = vhtfm.generate_hash()
		response = self.post(
...
class TestDocTypeAPIV2(VhtfmAPITestCase):
	version = "v2"

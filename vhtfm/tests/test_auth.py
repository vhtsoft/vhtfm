...
import vhtfm
from vhtfm.auth import LoginAttemptTracker
from vhtfm.vhtfmclient import AuthError, VhtfmClient
from vhtfm.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from vhtfm.tests import IntegrationTestCase, UnitTestCase
from vhtfm.tests.test_api import VhtfmAPITestCase
from vhtfm.utils import get_datetime, get_site_url, now
from vhtfm.utils.data import add_to_date
from vhtfm.www.login import _generate_temporary_login_link
...
class TestAuth(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = vhtfm.get_site_config().host_name or get_site_url(vhtfm.local.site)
	...
	def tearDownClass(cls):
		vhtfm.delete_doc("User", cls.test_user_email, force=True)
		vhtfm.local.request_ip = None
		vhtfm.form_dict.email = None
		vhtfm.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		vhtfm.db.set_single_value("System Settings", k, v)
		vhtfm.clear_cache()
		vhtfm.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		VhtfmClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			VhtfmClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		client = VhtfmClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestAllowedReferrer(UnitTestCase):
	def test_is_allowed_referrer(self):
		...
		vhtfm.cache.set_value("allowed_referrers", ["https://example.com"])
		vhtfm.local.request = create_request({"Referer": "https://example.com/some/path"})
		http_request = vhtfm.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid referrer
		vhtfm.local.request = create_request({"Referer": "https://malicious.com"})
		http_request = vhtfm.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test with valid origin
		vhtfm.local.request = create_request({"Origin": "https://example.com"})
		http_request = vhtfm.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid origin
		vhtfm.local.request = create_request({"Origin": "https://malicious.com"})
		http_request = vhtfm.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Clean up
		vhtfm.cache.delete_value("allowed_referrers")
...
class TestSessionExpirty(VhtfmAPITestCase):
	def test_session_expires(self):
		sid = self.sid  # triggers login for test case login
		s: Session = vhtfm.local.session_obj

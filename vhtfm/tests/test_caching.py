...
from vhtfm.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(vhtfm.local.site)
	return vhtfm.local.site
...
class TestSiteCache(VhtfmAPITestCase):
...
class TestRedisCache(VhtfmAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if vhtfm.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(VhtfmAPITestCase):
	TEST_DOCTYPE = "User"
	TEST_DOCNAME = "Administrator"
	TEST_FIELD = "middle_name"

	def setUp(self) -> None:
		self.test_value = vhtfm.generate_hash()

	def test_caching(self):
		vhtfm.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = vhtfm.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		doc.db_set(self.TEST_FIELD, self.test_value)
		new_doc = vhtfm.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		self.assertIsNot(doc, new_doc)  # Shouldn't be same object from vhtfm.local
		self.assertEqual(new_doc.get(self.TEST_FIELD), self.test_value)  # Cache invalidated and fetched
		vhtfm.db.rollback()

		doc_after_rollback = vhtfm.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(VhtfmAPITestCase):
	def test_delete_keys(self):
		prefix = "test_del_"

		for i in range(5):
			vhtfm.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(vhtfm.cache.get_keys(prefix)), 5)
		vhtfm.cache.delete_keys(prefix)
		self.assertEqual(len(vhtfm.cache.get_keys(prefix)), 0)

	def test_hash(self):
		key = "test_hash"

		# Confirm that there's no data initially
		exists = vhtfm.cache.exists(key)
		self.assertFalse(exists)

		# Insert 5 key-value pairs
		for i in range(5):
			vhtfm.cache.hset(key, f"key_{i}", f"value_{i}")

		# Check that we have 5 values
		values = vhtfm.cache.hgetall(key)
		self.assertEqual(len(values), 5)

		# Check that each value matches
		for i in range(5):
			value = vhtfm.cache.hget(key, f"key_{i}")
			self.assertEqual(value, f"value_{i}")

		# Check the keys themselves
		keys = vhtfm.cache.hkeys(key)
		for i in range(5):
			self.assertIn(f"key_{i}".encode(), keys)

		# Delete a single key and check that we still have the remaining 4
		vhtfm.cache.hdel(key, "key_1")
		values = vhtfm.cache.hgetall(key)
		self.assertEqual(len(values), 4)

		# Delete 2 keys and check that we still have the remaining 2
		vhtfm.cache.hdel(key, ["key_2", "key_3"])
		values = vhtfm.cache.hgetall(key)
		self.assertEqual(len(values), 2)

		# Delete the hash itself and confirm that there's no data
		vhtfm.cache.delete_value(key)
		exists = vhtfm.cache.exists(key)
		self.assertFalse(exists)

	def test_user_cache_clear(self):
		from vhtfm.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = vhtfm.utils.random_string(10)
		user2 = vhtfm.utils.random_string(10)
		for key in user_cache_keys:
			vhtfm.cache.hset(key, user1, key)
			vhtfm.cache.hset(key, user2, key)

		vhtfm.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(vhtfm.cache.hexists(key, user1))
			self.assertTrue(vhtfm.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from vhtfm.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(vhtfm.utils.random_string(10))
		doctype2 = new_doctype(vhtfm.utils.random_string(10))
		for key in doctype_cache_keys:
			vhtfm.cache.hset(key, doctype1.name, key)
			vhtfm.cache.hset(key, doctype2.name, key)

		vhtfm.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(vhtfm.cache.hexists(key, doctype1.name))
			self.assertTrue(vhtfm.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(vhtfm.cache, vhtfm.cache())

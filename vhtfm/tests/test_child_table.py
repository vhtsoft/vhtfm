...
import vhtfm
from vhtfm.model import child_table_fields
...
class TestChildTable(IntegrationTestCase):
	def tearDown(self) -> None:
		try:
			vhtfm.delete_doc("DocType", self.doctype_name, force=1)
		except Exception:
			pass
	...
	def test_child_table_doctype_creation_and_transitioning(self) -> None:
		...
		for column in child_table_fields:
			self.assertTrue(vhtfm.db.has_column(self.doctype_name, column))
	...
	def check_valid_columns(self, assertion_method: Callable) -> None:
		valid_columns = vhtfm.get_meta(self.doctype_name).get_valid_columns()
		for column in child_table_fields:

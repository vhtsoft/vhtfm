...
import vhtfm
from vhtfm.app import process_response
...
class TestCORS(IntegrationTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		vhtfm.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		vhtfm.conf.allow_cors = "*"
		self.make_request_and_test()

		vhtfm.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		vhtfm.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		vhtfm.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)

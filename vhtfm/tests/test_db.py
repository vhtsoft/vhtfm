...
from vhtfm.custom.doctype.custom_field.custom_field import create_custom_field
from vhtfm.database import savepoint
from vhtfm.database.database import get_query_execution_timeout
from vhtfm.database.utils import FallBackDateTimeStr
from vhtfm.query_builder import Field
from vhtfm.query_builder.functions import Concat_ws
from vhtfm.tests import IntegrationTestCase, timeout
from vhtfm.tests.test_query_builder import db_type_is, run_only_if
from vhtfm.utils import add_days, now, random_string, set_request
from vhtfm.utils.data import now_datetime
from vhtfm.utils.testutils import clear_custom_fields


class TestDB(IntegrationTestCase):
	def test_datetime_format(self):
		now_str = now()
		self.assertEqual(vhtfm.db.format_datetime(None), FallBackDateTimeStr)
		self.assertEqual(vhtfm.db.format_datetime(now_str), now_str)

	@run_only_if(db_type_is.MARIADB)
	def test_get_column_type(self):
		desc_data = vhtfm.db.sql("desc `tabUser`", as_dict=1)
		user_name_type = find(desc_data, lambda x: x["Field"] == "name")["Type"]
		self.assertEqual(vhtfm.db.get_column_type("User", "name"), user_name_type)

	def test_get_database_size(self):
		self.assertIsInstance(vhtfm.db.get_database_size(), (float, int))

	def test_db_statement_execution_timeout(self):
		vhtfm.db.set_execution_timeout(2)
		# Setting 0 means no timeout.
		self.addCleanup(vhtfm.db.set_execution_timeout, 0)
		...
		try:
			...
			self.assertTrue(vhtfm.db.is_statement_timeout(e), f"exepcted {e} to be timeout error")
			vhtfm.db.rollback(save_point=savepoint)
		else:
			vhtfm.db.rollback(save_point=savepoint)
			self.fail("Long running queries not timing out")

	@patch.dict(vhtfm.conf, {"http_timeout": 20, "enable_db_statement_timeout": 1})
	...
	def test_get_value(self):
		self.assertEqual(vhtfm.db.get_value("User", {"name": ["=", "Administrator"]}), "Administrator")
		self.assertEqual(vhtfm.db.get_value("User", {"name": ["like", "Admin%"]}), "Administrator")
		self.assertNotEqual(vhtfm.db.get_value("User", {"name": ["!=", "Guest"]}), "Guest")
		self.assertEqual(vhtfm.db.get_value("User", {"name": ["<", "Adn"]}), "Administrator")
		self.assertEqual(vhtfm.db.get_value("User", {"name": ["<=", "Administrator"]}), "Administrator")
		self.assertEqual(
			vhtfm.db.get_value("User", {}, ["Max(name)"], order_by=None),
			vhtfm.db.sql("SELECT Max(name) FROM tabUser")[0][0],
		)
		self.assertEqual(
			vhtfm.db.get_value("User", {}, "Min(name)", order_by=None),
			vhtfm.db.sql("SELECT Min(name) FROM tabUser")[0][0],
		)
		self.assertIn(
			"for update",
			vhtfm.db.get_value("User", Field("name") == "Administrator", for_update=True, run=False).lower(),
		)
		user_doctype = vhtfm.qb.DocType("User")
		self.assertEqual(
			vhtfm.qb.from_(user_doctype).select(user_doctype.name, user_doctype.email).run(),
			vhtfm.db.get_values(
				user_doctype,
				filters={},
		...
		self.assertEqual(
			vhtfm.db.sql("""SELECT name FROM `tabUser` WHERE name > 's' ORDER BY MODIFIED DESC""")[0][0],
			vhtfm.db.get_value("User", {"name": [">", "s"]}),
		)

		self.assertEqual(
			vhtfm.db.sql("""SELECT name FROM `tabUser` WHERE name >= 't' ORDER BY MODIFIED DESC""")[0][0],
			vhtfm.db.get_value("User", {"name": [">=", "t"]}),
		)
		self.assertEqual(
			vhtfm.db.get_values(
				"User",
		...
		self.assertEqual(
			vhtfm.db.sql("select email from tabUser where name='Administrator' order by creation DESC"),
			vhtfm.db.get_values("User", filters=[["name", "=", "Administrator"]], fieldname="email"),
		)

		# test multiple orderby's
		delimiter = '"' if vhtfm.db.db_type == "postgres" else "`"
		self.assertIn(
			"ORDER BY {deli}creation{deli} DESC,{deli}modified{deli} ASC,{deli}name{deli} DESC".format(
				deli=delimiter
			),
			vhtfm.db.get_value("DocType", "DocField", order_by="creation desc, modified asc, name", run=0),
		)

	def test_escape(self):
		vhtfm.db.escape("香港濟生堂製藥有限公司 - IT".encode())
	...
	def test_get_single_value(self):
		...
		for inp in test_inputs:
			...
			vhtfm.db.set_single_value("Print Settings", fieldname, inp["value"])
			self.assertEqual(vhtfm.db.get_single_value("Print Settings", fieldname), inp["value"])

		# teardown
		clear_custom_fields("Print Settings")

	def test_get_single_value_destructuring(self):
		[[lang, date_format]] = vhtfm.db.get_values_from_single(
			["language", "date_format"], None, "System Settings"
		)
		self.assertEqual(lang, vhtfm.db.get_single_value("System Settings", "language"))
		self.assertEqual(date_format, vhtfm.db.get_single_value("System Settings", "date_format"))

	def test_singles_get_values_variant(self):
		[[lang, date_format]] = vhtfm.db.get_values("System Settings", fieldname=["language", "date_format"])
		self.assertEqual(lang, vhtfm.db.get_single_value("System Settings", "language"))
		self.assertEqual(date_format, vhtfm.db.get_single_value("System Settings", "date_format"))
	...
	def test_log_touched_tables(self):
		...
		vhtfm.db.set_single_value("System Settings", "backup_limit", 5)
		self.assertIn("tabSingles", vhtfm.flags.touched_tables)

		vhtfm.flags.touched_tables = set()
		todo = vhtfm.get_doc({"doctype": "ToDo", "description": "Random Description"})
		todo.save()
		self.assertIn("tabToDo", vhtfm.flags.touched_tables)
		...
		if vhtfm.db.db_type != "postgres":
			vhtfm.flags.touched_tables = set()
			vhtfm.db.sql("UPDATE tabToDo SET description = 'Updated Description'")
			self.assertNotIn("tabToDo SET", vhtfm.flags.touched_tables)
			self.assertIn("tabToDo", vhtfm.flags.touched_tables)
		...
		self.assertIn("tabToDo", vhtfm.flags.touched_tables)
		self.assertIn("tabCustom Field", vhtfm.flags.touched_tables)
		if cf:
			cf.delete()
		vhtfm.db.commit()
	...
	def test_db_keywords_as_fields(self):
		...
		fields = all_keywords[vhtfm.conf.db_type][:1]
		test_doctype = "ToDo"
		...
		for _ in range(10):
			...
			doc = vhtfm.get_doc({"doctype": test_doctype, "description": random_string(20), **docfields})
			doc.insert()
		...
		# Testing read
		self.assertEqual(next(iter(vhtfm.get_all("ToDo", fields=[random_field], limit=1)[0])), random_field)
		self.assertEqual(
			next(iter(vhtfm.get_all("ToDo", fields=[f"`{random_field}` as total"], limit=1)[0])), "total"
		)
		...
		self.assertEqual(
			next(iter(vhtfm.get_all("ToDo", fields=[f"count(`{random_field}`)"], limit=1)[0])),
			"count" if vhtfm.conf.db_type == "postgres" else f"count(`{random_field}`)",
		)

		# Testing update
		vhtfm.db.set_value(test_doctype, random_doc, random_field, random_value)
		self.assertEqual(vhtfm.db.get_value(test_doctype, random_doc, random_field), random_value)

		# Cleanup - delete records and remove custom fields
		for doc in created_docs:
			vhtfm.delete_doc(test_doctype, doc)
	...
	def test_savepoints(self):
		...
		for _ in range(5):
			vhtfm.db.savepoint(save_point)
			doc_gone = vhtfm.get_doc(doctype="ToDo", description="nope").save()
			failed_docs.append(doc_gone.name)
			vhtfm.db.rollback(save_point=save_point)
			doc_kept = vhtfm.get_doc(doctype="ToDo", description="nope").save()
			created_docs.append(doc_kept.name)
		vhtfm.db.commit()

		for d in failed_docs:
			self.assertFalse(vhtfm.db.exists("ToDo", d))
		for d in created_docs:
			self.assertTrue(vhtfm.db.exists("ToDo", d))
	...
	def test_transaction_writes_error(self):
		from vhtfm.database.database import Database

		vhtfm.db.rollback()

		vhtfm.db.MAX_WRITES_PER_TRANSACTION = 1
		note = vhtfm.get_last_doc("ToDo")
		note.description = "changed"
		with self.assertRaises(vhtfm.TooManyWritesError):
			note.save()

		vhtfm.db.MAX_WRITES_PER_TRANSACTION = Database.MAX_WRITES_PER_TRANSACTION

	def test_transaction_write_counting(self):
		note = vhtfm.get_doc(doctype="Note", title="transaction counting").insert()

		writes = vhtfm.db.transaction_writes
		vhtfm.db.set_value("Note", note.name, "content", "abc")
		self.assertEqual(1, vhtfm.db.transaction_writes - writes)
		writes = vhtfm.db.transaction_writes
	...
	def test_transactions_disabled_during_writes(self):
		...
		with self.patch_hooks(
			...
			note = vhtfm.new_doc("Note", title=vhtfm.generate_hash())
			note.insert()
		self.assertGreater(vhtfm.db.transaction_writes, 0)  # This would've reset for commit/rollback

		self.assertFalse(vhtfm.db._disable_transaction_control)

	def test_pk_collision_ignoring(self):
		# note has `name` generated from title
		for _ in range(3):
			vhtfm.get_doc(doctype="Note", title="duplicate name").insert(ignore_if_duplicate=True)

		with savepoint():
			self.assertRaises(
				vhtfm.DuplicateEntryError, vhtfm.get_doc(doctype="Note", title="duplicate name").insert
			)
	...
	def test_read_only_errors(self):
		...
		self.addCleanup(vhtfm.db.rollback)

		with self.assertRaises(vhtfm.InReadOnlyMode):
			vhtfm.db.set_value("User", "Administrator", "full_name", "Haxor")

	def test_exists(self):
		dt, dn = "User", "Administrator"
		self.assertEqual(vhtfm.db.exists(dt, dn, cache=True), dn)
		self.assertEqual(vhtfm.db.exists(dt, dn), dn)
		self.assertEqual(vhtfm.db.exists(dt, {"name": ("=", dn)}), dn)

		filters = {"doctype": dt, "name": ("like", "Admin%")}
		self.assertEqual(vhtfm.db.exists(filters), dn)
	...
	def test_datetime_serialization(self):
		...
		self.assertEqual(str(dt), str(vhtfm.db.sql("select %s", dt)[0][0]))

		vhtfm.db.exists("User", {"creation": (">", dt)})
		self.assertIn(str(dt), str(vhtfm.db.last_query))

		before = now_datetime()
		note = vhtfm.get_doc(doctype="Note", title=vhtfm.generate_hash(), content="something").insert()
		after = now_datetime()
		self.assertEqual(note.name, vhtfm.db.exists("Note", {"creation": ("between", (before, after))}))

	def test_bulk_insert(self):
		current_count = vhtfm.db.count("ToDo")
		test_body = f"test_bulk_insert - {random_string(10)}"
		chunk_size = 10

		for number_of_values in (1, 2, 5, 27):
			current_transaction_writes = vhtfm.db.transaction_writes
			...
			self.assertEqual(number_of_values, vhtfm.db.count("ToDo") - current_count)

			# check if inserts were done in chunks
			expected_number_of_writes = ceil(number_of_values / chunk_size)
			self.assertEqual(
				expected_number_of_writes, vhtfm.db.transaction_writes - current_transaction_writes
			)

		vhtfm.db.delete("ToDo", {"description": test_body})
	...
	def test_bulk_update(self):
		...
		vhtfm.db.bulk_update("ToDo", updates)

		priority, status = vhtfm.db.get_value("ToDo", record_names[0], ["priority", "status"])
		...
		priority, status = vhtfm.db.get_value("ToDo", record_names[1], ["priority", "status"])
		self.assertEqual(priority, "Low")
		self.assertEqual(status, "Closed")  # should stay the same

		# cleanup
		vhtfm.db.delete("ToDo", {"name": ("in", record_names)})

	def test_count(self):
		vhtfm.db.delete("Note")

		vhtfm.get_doc(doctype="Note", title="note1", content="something").insert()
		vhtfm.get_doc(doctype="Note", title="note2", content="someting else").insert()

		# Count with no filtes
		self.assertEqual((vhtfm.db.count("Note")), 2)

		# simple filters
		self.assertEqual((vhtfm.db.count("Note", [["title", "=", "note1"]])), 1)

		vhtfm.get_doc(doctype="Note", title="note3", content="something other").insert()
	...
	def test_get_list_return_value_data_type(self):
		...
		note_docs = vhtfm.db.sql("select * from `tabNote`")
	...
	def test_modify_query(self):
		from vhtfm.database.postgres.database import modify_query
		...
		query = 'select locate(".io", "vhtfm.io"), locate("3", cast(3 as varchar)), locate("3", 3::varchar)'
		self.assertEqual(
			'select strpos( "vhtfm.io", ".io"), strpos( cast(3 as varchar), "3"), strpos( 3::varchar, "3")',
			modify_query(query),
		)

	@run_only_if(db_type_is.POSTGRES)
	def test_modify_values(self):
		from vhtfm.database.postgres.database import modify_values
	...
	def test_callbacks(self):
		...
		vhtfm.db.before_commit.add(lambda: f(0))
		vhtfm.db.before_commit.add(lambda: f(1))

		vhtfm.db.after_commit.add(lambda: f(2))
		vhtfm.db.after_commit.add(lambda: f(3))

		vhtfm.db.before_rollback.add(lambda: f("IGNORED"))
		vhtfm.db.before_rollback.add(lambda: f("IGNORED"))

		vhtfm.db.commit()

		vhtfm.db.after_commit.add(lambda: f("IGNORED"))
		vhtfm.db.after_commit.add(lambda: f("IGNORED"))

		vhtfm.db.before_rollback.add(lambda: f(4))
		vhtfm.db.before_rollback.add(lambda: f(5))
		vhtfm.db.after_rollback.add(lambda: f(6))
		vhtfm.db.after_rollback.add(lambda: f(7))
		vhtfm.db.after_rollback(lambda: f(8))
	...
	def test_db_explain(self):
		vhtfm.db.sql("select 1", debug=1, explain=1)
...
class TestDDLCommandsMaria(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		vhtfm.db.sql(f"DROP TABLE tab{self.test_table_name};")
		self.test_table_name = "TestNotes"

	def test_rename(self) -> None:
		new_table_name = f"{self.test_table_name}_new"
		vhtfm.db.rename_table(self.test_table_name, new_table_name)
		check_exists = vhtfm.db.sql(
			f"""
	...
	def test_describe(self) -> None:
		self.assertSequenceEqual(
			...
			vhtfm.db.describe(self.test_table_name),
		)

	def test_change_type(self) -> None:
		def get_table_description():
			return vhtfm.db.sql(f"DESC `tab{self.test_table_name}`")

		# try changing from int to varchar
		vhtfm.db.change_column_type("TestNotes", "id", "varchar(255)")
		self.assertIn("varchar(255)", get_table_description()[0])

		# try changing from varchar to bigint
		vhtfm.db.change_column_type("TestNotes", "id", "bigint")
		self.assertIn("bigint(20)", get_table_description()[0])

	def test_add_index(self) -> None:
		index_name = "test_index"
		vhtfm.db.add_index(self.test_table_name, ["id", "content(50)"], index_name)
		indexs_in_table = vhtfm.db.sql(
			f"""
...
class TestDBSetValue(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.todo1 = vhtfm.get_doc(doctype="ToDo", description="test_set_value 1").insert()
		cls.todo2 = vhtfm.get_doc(doctype="ToDo", description="test_set_value 2").insert()

	def test_update_single_doctype_field(self):
		value = vhtfm.db.get_single_value("System Settings", "deny_multiple_sessions")
		changed_value = not value

		vhtfm.db.set_single_value("System Settings", "deny_multiple_sessions", changed_value)
		current_value = vhtfm.db.get_single_value("System Settings", "deny_multiple_sessions")
	...
	def test_none_no_set_value(self):
		vhtfm.db.set_value("User", None, "middle_name", "test")
		with self.assertQueryCount(0):
			vhtfm.db.set_value("User", None, "middle_name", "test")
			vhtfm.db.set_value("User", "User", "middle_name", "test")

	def test_update_single_row_single_column(self):
		vhtfm.db.set_value("ToDo", self.todo1.name, "description", "test_set_value change 1")
		updated_value = vhtfm.db.get_value("ToDo", self.todo1.name, "description")
		self.assertEqual(updated_value, "test_set_value change 1")

	@patch("vhtfm.db.set_single_value")
	def test_set_single_value_with_set_value(self, single_set):
		vhtfm.db.set_value("Contact Us Settings", None, "country", "India")
	...
	def test_update_single_row_multiple_columns(self):
		...
		updated_desciption, updated_status = vhtfm.db.get_value(
	...
	def test_update_multiple_rows_single_column(self):
		vhtfm.db.set_value("ToDo", {"description": ("like", "%test_set_value%")}, "description", "change 2")

		self.assertEqual(vhtfm.db.get_value("ToDo", self.todo1.name, "description"), "change 2")
		self.assertEqual(vhtfm.db.get_value("ToDo", self.todo2.name, "description"), "change 2")

	def test_update_multiple_rows_multiple_columns(self):
		todos_to_update = vhtfm.get_all(
			"ToDo",
	...
	def test_update_modified_options(self):
		...
		vhtfm.db.set_value("ToDo", todo.name, "description", updated_description, update_modified=False)
		self.assertEqual(updated_description, vhtfm.db.get_value("ToDo", todo.name, "description"))
		self.assertEqual(todo.modified, vhtfm.db.get_value("ToDo", todo.name, "modified"))
		...
		self.assertTupleEqual(
			...
			vhtfm.db.get_value("ToDo", todo.name, ["modified", "modified_by"]),
		)
	...
	def test_set_value(self):
		...
		query = str(vhtfm.db.last_query)

		if vhtfm.conf.db_type == "postgres":
			from vhtfm.database.postgres.database import modify_query
	...
	def test_cleared_cache(self):
		self.todo2.reload()
		vhtfm.get_cached_doc(self.todo2.doctype, self.todo2.name)  # init cache

		description = f"{self.todo2.description}-edit by `test_cleared_cache`"

		vhtfm.db.set_value(self.todo2.doctype, self.todo2.name, "description", description)
		cached_doc = vhtfm.get_cached_doc(self.todo2.doctype, self.todo2.name)
...
class TestDDLCommandsPost(IntegrationTestCase):
	...
	def test_describe(self) -> None:
		self.assertSequenceEqual([("id",), ("content",)], vhtfm.db.describe(self.test_table_name))
	...
	def test_change_type(self) -> None:
		...
		# try changing from int to varchar
		vhtfm.db.change_column_type(self.test_table_name, "id", "varchar(255)")
		self.assertIn("character varying", get_table_description()[0])

		# try changing from varchar to int
		try:
			vhtfm.db.change_column_type(self.test_table_name, "id", "bigint")
		except DatatypeMismatch:
			vhtfm.db.rollback()

		# try changing from varchar to int (using cast)
		vhtfm.db.change_column_type(self.test_table_name, "id", "bigint", use_cast=True)
	...
	def test_is(self):
		...
		self.assertIn(
			'coalesce("name",',
			vhtfm.db.get_values(user, filters={user.name: ("is", "set")}, run=False).lower(),
		)
		self.assertIn(
			'coalesce("name",',
			vhtfm.db.get_values(user, filters={user.name: ("is", "not set")}, run=False).lower(),
		)
...
class TestTransactionManagement(IntegrationTestCase):
	def test_create_proper_transactions(self):
		def _get_transaction_id():
			return vhtfm.db.sql("select txid_current()", pluck=True)
...
class TestReplicaConnections(IntegrationTestCase):
	def test_switching_to_replica(self):
		with patch.dict(vhtfm.local.conf, {"read_from_replica": 1, "replica_host": "127.0.0.1"}):

			def db_id():
				return id(vhtfm.local.db)
...
class TestConcurrency(IntegrationTestCase):
	...
	def test_skip_locking(self):
		with self.primary_connection():
			name = vhtfm.db.get_value("User", "Administrator", for_update=True, skip_locked=True)
	...
	def test_no_wait(self):
		with self.primary_connection():
			name = vhtfm.db.get_value("User", "Administrator", for_update=True)
			self.assertEqual(name, "Administrator")

		with self.secondary_connection():
			self.assertRaises(
				vhtfm.QueryTimeoutError,
				lambda: vhtfm.db.get_value("User", "Administrator", for_update=True, wait=False),
			)

	@timeout(5, "Deletion stuck on lock timeout")
	def test_delete_race_condition(self):
		note = vhtfm.new_doc("Note")
		note.title = note.content = vhtfm.generate_hash()
		note.insert()
		vhtfm.db.commit()  # ensure that second connection can see the document

		with self.primary_connection():
			n1 = vhtfm.get_doc(note.doctype, note.name)
			n1.save()

		with self.secondary_connection():
			self.assertRaises(vhtfm.QueryTimeoutError, vhtfm.delete_doc, note.doctype, note.name)
...
class TestSqlIterator(IntegrationTestCase):
	def test_db_sql_iterator(self):
		...
		for query in test_queries:
			self.assertEqual(
				vhtfm.db.sql(query, as_dict=True),
				list(vhtfm.db.sql(query, as_dict=True, as_iterator=True)),
				msg=f"{query=} results not same as iterator",
			)

			self.assertEqual(
				vhtfm.db.sql(query, pluck=True),
				list(vhtfm.db.sql(query, pluck=True, as_iterator=True)),
				msg=f"{query=} results not same as iterator",
			)

			self.assertEqual(
				vhtfm.db.sql(query, as_list=True),
				list(vhtfm.db.sql(query, as_list=True, as_iterator=True)),
				msg=f"{query=} results not same as iterator",
			)

	@run_only_if(db_type_is.MARIADB)
	def test_unbuffered_cursor(self):
		with vhtfm.db.unbuffered_cursor():
...
class TestPostgresSchemaQueryIndependence(ExtIntegrationTestCase):
	...
	def test_get_tables(self) -> None:
		tables = vhtfm.db.get_tables(cached=False)
	...
	def test_db_table_columns(self) -> None:
		columns = vhtfm.db.get_table_columns(self.test_table_name)

		# should have received the columns of the table from public schema
		self.assertEqual(columns, ["col_a", "col_b"])

		vhtfm.conf["db_schema"] = "alt_schema"
		vhtfm.cache.delete_key("table_columns")  # remove table columns cache for next try from alt_schema

		# should have received the columns of the table from alt_schema
		columns = vhtfm.db.get_table_columns(self.test_table_name)
		self.assertEqual(columns, ["col_c", "col_d"])

		del vhtfm.conf["db_schema"]
		vhtfm.cache.delete_key("table_columns")

	def test_describe(self) -> None:
		self.assertSequenceEqual([("col_a",), ("col_b",)], vhtfm.db.describe(self.test_table_name))
	...
	def test_has_index(self) -> None:
		...
		self.assertFalse(vhtfm.db.has_index(f"tab{self.test_table_name}", f"tab{self.test_table_name}_pkey"))

	def test_add_index(self) -> None:
		vhtfm.conf["db_schema"] = "alt_schema"

		# only dummy tabUser table in alt_schema has "col_c" column
		vhtfm.db.add_index("User", ("col_c",))
		...
		with self.assertSqlException():
			vhtfm.db.add_index(doctype="User", fields=("col_c",))

	# TODO: is there some method like remove_index:
	# TODO: apps/vhtfm/vhtfm/patches/v14_0/drop_unused_indexes.py # def drop_index_if_exists()
	# TODO: apps/vhtfm/vhtfm/database/postgres/schema.py # def alter()
	...
	def test_add_unique(self) -> None:
		...
		with self.assertSqlException():
			vhtfm.db.add_unique(f"{self.test_table_name}", ["col_c", "col_d"])
	...
	def test_get_table_columns_description(self):
		...
		columns = vhtfm.db.get_table_columns_description(f"tab{self.test_table_name}")
	...
	def test_get_column_type(self):
		...
		self.assertEqual(vhtfm.db.get_column_type(self.test_table_name, "col_a"), "character varying")

		# should raise an error for the column in the alt_schema
		with self.assertSqlException():
			vhtfm.db.get_column_type(self.test_table_name, "col_c")

	def test_search_path(self):
		# by default the the public schema tables should be addressed by search path
		rows = vhtfm.db.sql(f'select * from "tab{self.test_table_name}"')
		self.assertEqual(
			rows,
			(
				(
...
class TestDbConnectWithEnvCredentials(IntegrationTestCase):
	current_site = vhtfm.local.site

	def tearDown(self):
		vhtfm.init(self.current_site, force=True)
		vhtfm.connect()
	...
	def test_connect_fails_with_wrong_credentials_by_env(self) -> None:
		...
		# with wrong db name
		with set_env_variable("VHTFM_DB_NAME", "dbiq"):
			vhtfm.init(self.current_site, force=True)
			vhtfm.connect()
		...
		# with wrong host
		with set_env_variable("VHTFM_DB_HOST", "iqx.local"):
			vhtfm.init(self.current_site, force=True)
			vhtfm.connect()
		...
		with set_env_variable("VHTFM_DB_USER", "uname"):
			vhtfm.init(self.current_site, force=True)
			vhtfm.connect()
		...
		# with wrong password
		with set_env_variable("VHTFM_DB_PASSWORD", "pass"):
			vhtfm.init(self.current_site, force=True)
			vhtfm.connect()
		...
		# with wrong password
		with set_env_variable("VHTFM_DB_PORT", "1111"):
			vhtfm.init(self.current_site, force=True)
			vhtfm.connect()

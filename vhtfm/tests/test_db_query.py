...
from vhtfm.database.utils import DefaultOrderBy
from vhtfm.desk.reportview import get_filters_cond
from vhtfm.handler import execute_cmd
from vhtfm.model.db_query import DatabaseQuery, get_between_date_filter
from vhtfm.permissions import add_user_permission, clear_user_permissions_for_doctype
from vhtfm.query_builder import Column
from vhtfm.tests import IntegrationTestCase
from vhtfm.tests.test_query_builder import db_type_is, run_only_if
from vhtfm.utils.testutils import add_custom_field, clear_custom_fields
...
class TestDBQuery(IntegrationTestCase):
	...
	def test_child_table_field_syntax(self):
		note = vhtfm.get_doc(
			doctype="Note",
			title=f"Test {vhtfm.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		vhtfm.delete_doc_if_exists("DocType", "Parent DocType 1")
		vhtfm.delete_doc_if_exists("DocType", "Parent DocType 2")
		vhtfm.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		vhtfm.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		vhtfm.db.delete("Parent DocType 1")
		vhtfm.db.delete("Parent DocType 2")
		vhtfm.db.delete("Child DocType")
		...
		# test query
		results1 = vhtfm.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = vhtfm.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = vhtfm.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = vhtfm.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = vhtfm.get_doc("User", "test2@example.com")
	...
	def test_string_as_field(self):
		self.assertEqual(
			vhtfm.get_all("DocType", as_list=True), vhtfm.get_all("DocType", fields="name", as_list=True)
		)

	def test_none_filter(self):
		query = vhtfm.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = vhtfm._dict(fieldtype="Date")
		datetime_df = vhtfm._dict(fieldtype="Datetime")
		today = vhtfm.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		vhtfm.set_user("test2@example.com")
		self.assertRaises(vhtfm.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(vhtfm.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from vhtfm.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = vhtfm.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = vhtfm.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(vhtfm.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(vhtfm.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		vhtfm.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			vhtfm.get_list("Prepared Report", ["*"])
			vhtfm.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# vhtfm.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = vhtfm.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = vhtfm.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		vhtfm.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		vhtfm.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		vhtfm.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = vhtfm.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from vhtfm.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = vhtfm.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		class VirtualDocType:
			...
			def get_list(args=None, limit_page_length=0, doctype=None):
				from vhtfm.types.filter import FilterTuple
		...
		with patch(
			"vhtfm.controllers",
			new={vhtfm.local.site: {"Virtual DocType": VirtualDocType}},
		):
			vhtfm.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)

	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", vhtfm.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", vhtfm.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			vhtfm.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", vhtfm.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from vhtfm.desk.reportview import get

		if not vhtfm.db.exists("DocType", "Related Todos"):
			vhtfm.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			vhtfm.db.delete("Related Todos")

		todo_one = vhtfm.get_doc(
			{
				"doctype": "ToDo",
		...
		vhtfm.form_dict.doctype = "Related Todos"
		vhtfm.form_dict.fields = [
	...
	def test_select_star_expansion(self):
		count = vhtfm.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], vhtfm.db.count("Language"))
		self.assertEqual(count[1], vhtfm.db.count("Language"))
...
class TestReportView(IntegrationTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("vhtfm.desk.reportview.get_count")
		vhtfm.local.form_dict = vhtfm._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("vhtfm.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("vhtfm.desk.reportview.get_count")
		current_value = vhtfm.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("vhtfm.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(vhtfm.get_list("Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		vhtfm.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		vhtfm.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(vhtfm.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			vhtfm.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = vhtfm.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if vhtfm.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = vhtfm.new_doc("Nested DocType")
		d.update(r)

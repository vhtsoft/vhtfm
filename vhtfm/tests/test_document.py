...
import vhtfm
from vhtfm.app import make_form_dict
from vhtfm.core.doctype.doctype.test_doctype import new_doctype
from vhtfm.desk.doctype.note.note import Note
from vhtfm.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from vhtfm.tests import IntegrationTestCase
from vhtfm.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	...
	def test_load_single(self):
		d = vhtfm.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(vhtfm.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = vhtfm.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(vhtfm.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(vhtfm.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(vhtfm.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from vhtfm.desk.doctype.event.event import Event
		...
		self.assertRaises(vhtfm.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		vhtfm.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(vhtfm.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(vhtfm.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			vhtfm.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except vhtfm.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = vhtfm.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(vhtfm.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = vhtfm.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = vhtfm.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(vhtfm.PermissionError, self.test_insert)
		vhtfm.set_user("Administrator")

	def test_permission_single(self):
		vhtfm.set_user("Guest")
		d = vhtfm.get_doc("Website Settings", "Website Settings")
		self.assertRaises(vhtfm.PermissionError, d.save)
		vhtfm.set_user("Administrator")

	def test_link_validation(self):
		vhtfm.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(vhtfm.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(vhtfm.ValidationError, d.validate)
		self.assertRaises(vhtfm.ValidationError, d.run_method, "validate")
		self.assertRaises(vhtfm.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = vhtfm.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(vhtfm.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(vhtfm.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = vhtfm.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(vhtfm.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		vhtfm.delete_doc_if_exists("Currency", "Vhtfm Coin", 1)

		d = vhtfm.get_doc(
			{"doctype": "Currency", "currency_name": "Vhtfm Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(vhtfm.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(vhtfm.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		vhtfm.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = vhtfm.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_virtual_fields(self):
		...
		vhtfm.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		def patch_note(class_=None):
			return patch("vhtfm.controllers", new={vhtfm.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"vhtfm.utils.now_datetime() - vhtfm.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
			custom_field = vhtfm.get_doc(
				{
	...
	def test_doc_events(self):
		...
		for doctype, doc_hooks in vhtfm.get_doc_hooks().items():
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = vhtfm.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise vhtfm.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(vhtfm.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			vhtfm.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)

	def test_db_set_singles(self):
		c = vhtfm.get_doc("Contact Us Settings")
		key, val = "email_id", "admin1@example.com"
		c.db_set(key, val)
		changed_val = vhtfm.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(vhtfm.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = vhtfm.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from vhtfm.modules.utils import get_doc_path, scrub

		vhtfm.flags.allow_doctype_export = True

		vhtfm.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from vhtfm.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = vhtfm.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = vhtfm.generate_hash()
		...
		all_docs = set(vhtfm.get_all(doctype, pluck="name"))
		all_child_docs = set(
			vhtfm.get_all(

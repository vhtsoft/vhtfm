...
from vhtfm.core.page.permission_manager.permission_manager import get_roles_and_doctypes
from vhtfm.desk.doctype.desktop_icon.desktop_icon import (
	add_user_icon,
...
class TestDomainification(IntegrationTestCase):
	...
	def tearDown(self):
		vhtfm.db.delete("Role", {"name": "_Test Role"})
		vhtfm.db.delete("Has Role", {"role": "_Test Role"})
		vhtfm.db.delete("Domain", {"name": ("in", ("_Test Domain 1", "_Test Domain 2"))})
		vhtfm.delete_doc("DocType", "Test Domainification")
		self.remove_from_active_domains(remove_all=True)
	...
	def add_active_domain(self, domain):
		...
		domain_settings = vhtfm.get_doc("Domain Settings", "Domain Settings")
	...
	def new_domain(self, domain):
		# create new domain
		vhtfm.get_doc({"doctype": "Domain", "domain": domain}).insert()
	...
	def test_active_domains(self):
		self.assertTrue("_Test Domain 1" in vhtfm.get_active_domains())
		self.assertFalse("_Test Domain 2" in vhtfm.get_active_domains())

		self.add_active_domain("_Test Domain 2")
		self.assertTrue("_Test Domain 2" in vhtfm.get_active_domains())

		self.remove_from_active_domains("_Test Domain 1")
		self.assertTrue("_Test Domain 1" not in vhtfm.get_active_domains())
	...
	def test_doctype_and_role_domainification(self):
		...
		test_role = vhtfm.get_doc({"doctype": "Role", "role_name": "_Test Role"}).insert()
	...
	def test_module_def_for_domainification(self):
		...
		test_module_def = vhtfm.get_doc("Module Def", "Contacts")

...
class TestDynamicLinks(IntegrationTestCase):
	...
	def test_delete_normal(self):
		...
		self.assertFalse(vhtfm.db.exists("Email Unsubscribe", unsub.name))
	...
	def test_delete_with_comment(self):
		...
		self.assertTrue(
			vhtfm.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
		)
		event.delete()
		self.assertFalse(
			vhtfm.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
	...
	def test_custom_fields(self):
		...
		self.assertRaises(vhtfm.LinkExistsError, unsub.delete)
		...
		vhtfm.db.commit()  # undo changes done by DDL

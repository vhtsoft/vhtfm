...
class TestFmtDatetime(IntegrationTestCase):
	...
	def setUp(self):
		# create test domain
		self.pre_test_date_format = vhtfm.db.get_default("date_format")
		self.pre_test_time_format = vhtfm.db.get_default("time_format")

	def tearDown(self):
		vhtfm.db.set_default("date_format", self.pre_test_date_format)
		vhtfm.db.set_default("time_format", self.pre_test_time_format)
		vhtfm.local.user_date_format = None
		vhtfm.local.user_time_format = None
		vhtfm.db.rollback()

	# Test utility functions

	def test_set_default_date_format(self):
		vhtfm.db.set_default("date_format", "ZYX321")
		self.assertEqual(vhtfm.db.get_default("date_format"), "ZYX321")

	def test_set_default_time_format(self):
		vhtfm.db.set_default("time_format", "XYZ123")
		self.assertEqual(vhtfm.db.get_default("time_format"), "XYZ123")
	...
	def test_format_date(self):
		...
		for fmt, valid_fmt in test_date_formats.items():
			vhtfm.db.set_default("date_format", fmt)
			vhtfm.local.user_date_format = None
	...
	def test_format_time(self):
		...
		for fmt, valid_fmt in test_time_formats.items():
			vhtfm.db.set_default("time_format", fmt)
			vhtfm.local.user_time_format = None
	...
	def test_format_datetime(self):
		...
		for date_fmt, valid_date in test_date_formats.items():
			vhtfm.db.set_default("date_format", date_fmt)
			vhtfm.local.user_date_format = None
			for time_fmt, valid_time in test_time_formats.items():
				vhtfm.db.set_default("time_format", time_fmt)
				vhtfm.local.user_time_format = None

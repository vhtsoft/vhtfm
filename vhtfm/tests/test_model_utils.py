...
import vhtfm
from vhtfm.model import core_doctypes_list, get_permitted_fields, is_default_field
from vhtfm.model.utils import get_fetch_values
...
class TestModelUtils(IntegrationTestCase):
	def test_get_fetch_values(self):
		...
		full_name = vhtfm.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = vhtfm.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = vhtfm.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = vhtfm.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = vhtfm.session.user or "Administrator"
	vhtfm.set_user(user)
	yield
	vhtfm.set_user(past_user)

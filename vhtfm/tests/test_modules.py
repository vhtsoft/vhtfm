...
from vhtfm.modules import export_customizations, export_module_json, get_module_path
from vhtfm.modules.utils import export_doc, sync_customizations
...
class TestUtils(IntegrationTestCase):
	def setUp(self):
		self._dev_mode = vhtfm.local.conf.developer_mode
		vhtfm.local.conf.developer_mode = True

	def tearDown(self):
		vhtfm.db.rollback()
		vhtfm.local.conf.developer_mode = self._dev_mode
		vhtfm.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		vhtfm.local.flags.in_import = True
		doc = vhtfm.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = vhtfm.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = vhtfm.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=vhtfm.as_json(export_doc_before))

		vhtfm.flags.in_import = False
		vhtfm.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			vhtfm.parse_json(f.read())  # export_doc_after
	...
	def test_reload_doc(self):
		vhtfm.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(vhtfm.db.get_value("DocType", "Note", "migration_hash"))
		vhtfm.db.set_value(
			"DocField",
		...
		self.assertEqual(
			vhtfm.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		vhtfm.reload_doctype("Note")
		self.assertEqual(
			vhtfm.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(vhtfm.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = vhtfm.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			vhtfm.get_app_path("vhtfm", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = vhtfm.scrub(doctype.name)
			path = vhtfm.get_app_path("vhtfm", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		vhtfm.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(vhtfm.get_module_path("Desk", "Note"))

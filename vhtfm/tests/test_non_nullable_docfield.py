...
class TestNonNullableDocfield(IntegrationTestCase):
	...
	def test_non_nullable_field(self):
		doc = vhtfm.new_doc(doctype=self.doctype_name)
		doc.insert()
		inserted_doc = vhtfm.db.get(self.doctype_name, {"name": doc.name})
		self.assertEqual(inserted_doc.test_field, "")

	def test_edit_field_nullable_status(self):
		doc = vhtfm.new_doc(doctype=self.nullable_doctype_name)
		doc.insert()
		inserted_doc = vhtfm.db.get(self.nullable_doctype_name, {"name": doc.name})
		...
		for column in vhtfm.db.sql(query, table.table_name, as_dict=True):
			if column.name == "test_field":
				self.assertFalse(column.not_nullable)

		doctype_doc = vhtfm.get_doc("DocType", self.nullable_doctype_name)

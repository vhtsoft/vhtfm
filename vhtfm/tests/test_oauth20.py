...
import vhtfm
from vhtfm.integrations.oauth2 import encode_params
from vhtfm.tests import IntegrationTestCase
from vhtfm.tests.test_api import get_test_client, make_request, suppress_stdout
...
class VhtfmRequestTestCase(IntegrationTestCase):
	...
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			vhtfm.local.login_manager.login_as("test@example.com")
			self._sid = vhtfm.session.sid
...
class TestOAuth20(VhtfmRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Vhtfm server URL reqired for id_token generation
		vhtfm_login_key: SocialLoginKey = vhtfm.new_doc("Social Login Key")
		vhtfm_login_key.get_social_login_provider("Vhtfm", initialize=True)
		vhtfm_login_key.base_url = vhtfm.utils.get_url()
		vhtfm_login_key.enable_social_login = 0
		vhtfm_login_key.insert(ignore_if_duplicate=True)
		vhtfm.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = vhtfm.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/vhtfm.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/vhtfm.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = vhtfm.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/vhtfm.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = vhtfm.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/vhtfm.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "VhtfmRequestTestCase" = None):
	...
	# Use token in header
	headers = {}
	URL = "/api/method/vhtfm.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(vhtfm.utils.get_url(), endpoint)

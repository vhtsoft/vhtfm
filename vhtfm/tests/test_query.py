...
from vhtfm.query_builder.functions import Abs, Count, Ifnull, Max, Now, Timestamp
from vhtfm.tests import IntegrationTestCase
from vhtfm.tests.test_query_builder import db_type_is, run_only_if
from vhtfm.utils.nestedset import get_ancestors_of, get_descendants_of
...
def create_tree_docs():
	...
	for record in records:
		d = vhtfm.new_doc("Test Tree DocType")
		d.update(record)
		d.insert()
...
class TestQuery(IntegrationTestCase):
	...
	def test_string_fields(self):
		self.assertEqual(
			vhtfm.qb.get_query("User", fields="name, email", filters={"name": "Administrator"}).get_sql(),
			vhtfm.qb.from_("User")
	...
	def test_functions_fields(self):
		self.assertEqual(
			vhtfm.qb.get_query("User", fields="Count(name)", filters={}).get_sql(),
			vhtfm.qb.from_("User").select(Count(Field("name"))).get_sql(),
		)

		self.assertEqual(
			vhtfm.qb.get_query("User", fields=["Count(name)", "Max(name)"], filters={}).get_sql(),
			vhtfm.qb.from_("User").select(Count(Field("name")), Max(Field("name"))).get_sql(),
		)

		self.assertEqual(
			vhtfm.qb.get_query("User", fields=["abs(name-email)", "Count(name)"], filters={}).get_sql(),
			vhtfm.qb.from_("User")
		...
		self.assertEqual(
			vhtfm.qb.get_query("User", fields=[Count("*")], filters={}).get_sql(),
			vhtfm.qb.from_("User").select(Count("*")).get_sql(),
		)

		self.assertEqual(
			vhtfm.qb.get_query("User", fields="timestamp(creation, modified)", filters={}).get_sql(),
			vhtfm.qb.from_("User").select(Timestamp(Field("creation"), Field("modified"))).get_sql(),
		)
	...
	def test_qb_fields(self):
		...
		self.assertEqual(
			...
			vhtfm.qb.from_(user_doctype).select(user_doctype.name, user_doctype.email).get_sql(),
		)

		self.assertEqual(
			vhtfm.qb.get_query(user_doctype, fields=user_doctype.email, filters={}).get_sql(),
			vhtfm.qb.from_(user_doctype).select(user_doctype.email).get_sql(),
		)

	def test_aliasing(self):
		user_doctype = vhtfm.qb.DocType("User")
		self.assertEqual(
			vhtfm.qb.get_query("User", fields=["name as owner", "email as id"], filters={}).get_sql(),
		...
		self.assertEqual(
			vhtfm.qb.get_query(user_doctype, fields="name as owner, email as id", filters={}).get_sql(),
			vhtfm.qb.from_(user_doctype)
			.select(user_doctype.name.as_("owner"), user_doctype.email.as_("id"))
			.get_sql(),
		)
	...
	def test_filters(self):
		self.assertEqual(
			vhtfm.qb.get_query(
				"DocType",
				fields=["name"],
				filters={"module.app_name": "vhtfm"},
			).get_sql(),
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='vhtfm'".replace(
				"`", '"' if vhtfm.db.db_type == "postgres" else "`"
			),
		)
		...
		self.assertEqual(
			...
			"SELECT `name` FROM `tabDocType`".replace("`", '"' if vhtfm.db.db_type == "postgres" else "`"),
		)
	...
	def test_comment_stripping(self):
		self.assertNotIn(
			"email", vhtfm.qb.get_query("User", fields=["name", "#email"], filters={}).get_sql()
		)

	def test_nestedset(self):
		vhtfm.db.sql("delete from `tabDocType` where `name` = 'Test Tree DocType'")
		vhtfm.db.sql_ddl("drop table if exists `tabTest Tree DocType`")
		create_tree_docs()
		descendants_result = vhtfm.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		ancestors_result = vhtfm.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_descendants_result = vhtfm.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_ancestors_result = vhtfm.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
	...
	def test_child_field_syntax(self):
		note1 = vhtfm.get_doc(doctype="Note", title="Note 1", seen_by=[{"user": "Administrator"}]).insert()
		note2 = vhtfm.get_doc(
			doctype="Note", title="Note 2", seen_by=[{"user": "Administrator"}, {"user": "Guest"}]
		).insert()

		result = vhtfm.qb.get_query(
			"Note",

...
import vhtfm
import vhtfm.utils
from vhtfm.desk.query_report import build_xlsx_data, export_query, run
...
class TestQueryReport(IntegrationTestCase):
	...
	def test_csv(self):
		...
		if not vhtfm.db.exists("Report", REPORT_NAME):
			...
			report.query = vhtfm.qb.from_(REF_DOCTYPE).select(*REPORT_COLUMNS).limit(10).get_sql()
		...
		vhtfm.delete_doc("Report", REPORT_NAME, delete_permanently=True)
	...
	def test_report_for_duplicate_column_names(self):
		...
		try:
			...
			for i in range(1, 3):
				vhtfm.get_doc({"doctype": "Doc A", "first_name": f"John{i}", "last_name": "Doe"}).insert()
				vhtfm.get_doc({"doctype": "Doc B", "last_name": f"Doe{i}", "first_name": "John"}).insert()

			if not vhtfm.db.exists("Report", "Doc A Report"):
				...
				report = vhtfm.get_doc("Report", "Doc A Report")

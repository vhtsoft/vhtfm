...
from vhtfm.utils import get_fox_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	vhtfm.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def tearDown(self):
		vhtfm.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(vhtfm.cache.hget(USER_TRANSLATION_KEY, vhtfm.local.lang))
		self.assertIsNotNone(vhtfm.cache.hget(MERGED_TRANSLATION_KEY, vhtfm.local.lang))

		clear_cache()

		self.assertIsNone(vhtfm.cache.hget(USER_TRANSLATION_KEY, vhtfm.local.lang))
		self.assertIsNone(vhtfm.cache.hget(MERGED_TRANSLATION_KEY, vhtfm.local.lang))

	def test_extract_message_from_file(self):
		data = vhtfm.translate.get_messages_from_file(translation_string_file)
		fox_path = get_fox_path()
		file_path = vhtfm.get_app_path("vhtfm", "tests", "translation_test_file.txt")
		exp_filename = os.path.relpath(file_path, fox_path)
	...
	def test_translation_with_context(self):
		t1 = vhtfm.new_doc("Translation")
		t1.language = "fr"
		t1.source_text = "Change"
		t1.translated_text = "Changement"
		t1.save()

		t2 = vhtfm.new_doc("Translation")
		t2.language = "fr"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for vhtfm.translate.get_language

		Case 1: vhtfm.form_dict._lang is set
		"""

		vhtfm.form_dict._lang = first_lang

		with patch.object(vhtfm.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: vhtfm.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(vhtfm.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: vhtfm.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: vhtfm.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: vhtfm.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("vhtfm")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			vhtfm._("attr")
			_("name")
			vhtfm._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(vhtfm.get_app_path(app)) / "translations"

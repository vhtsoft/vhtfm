...
import vhtfm
from vhtfm.installer import parse_app_name
from vhtfm.model.document import Document
from vhtfm.tests import IntegrationTestCase, MockedRequestTestCase
...
from vhtfm.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from vhtfm.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from vhtfm.utils.dateutils import get_dates_from_timegrain
from vhtfm.utils.diff import _get_value_from_version, get_version_diff, version_query
from vhtfm.utils.identicon import Identicon
from vhtfm.utils.image import optimize_image, strip_exif_data
from vhtfm.utils.make_random import can_make, get_random, how_many
from vhtfm.utils.response import json_handler
from vhtfm.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/vhtfm/test.jpg">
			</div>
			<div style="background-image: url('/assets/vhtfm/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/vhtfm/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/vhtfm/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from vhtfm.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from vhtfm.utils.html_utils import sanitize_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://vhtfm.io", throw=True))
		...
		self.assertRaises(vhtfm.ValidationError, validate_url, "vhtfm", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://vhtfm.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://vhtfm.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@vhtfm.com"))
		self.assertTrue(validate_email_address("someone@vhtfm.com, anyone@vhtfm.io"))
		self.assertTrue(validate_email_address("test%201@vhtfm.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@vhtfm.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			vhtfm.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@vhtfm.com"), "Some%20One@vhtfm.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@vhtfm.com"),
			"erp+Job%20Applicant=JA00004@vhtfm.com",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(vhtfm.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(vhtfm.get_app_path("vhtfm", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			vhtfm.get_app_path("vhtfm", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = vhtfm.get_app_path("vhtfm", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(vhtfm.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = vhtfm.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(vhtfm.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				vhtfm.utils.get_first_day_of_week("2020-12-25"),
				vhtfm.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				vhtfm.utils.get_first_day_of_week("2020-12-20"),
				vhtfm.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			vhtfm.utils.get_first_day_of_week("2020-12-25"),
			vhtfm.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			vhtfm.utils.get_first_day_of_week("2020-12-21"),
			vhtfm.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			vhtfm.utils.get_last_day_of_week("2020-12-24"),
			vhtfm.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			vhtfm.utils.get_last_day_of_week("2020-12-28"),
			vhtfm.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(vhtfm.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(vhtfm.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from vhtfm.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from vhtfm.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = vhtfm.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/vhtfm/healthcare",
			status=200,
			json={},
		)
		fox_path = get_fox_path()
		vhtfm_app = os.path.join(fox_path, "apps", "vhtfm")
		self.assertEqual("vhtfm", parse_app_name(vhtfm_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/vhtfm/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:vhtfm/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("vhtfm/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(vhtfm.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(vhtfm.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(vhtfm.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(vhtfm.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from vhtfm.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("vhtfm.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `vhtfm.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# developers@vhtfm.io has a gravatar linked so str URL will be returned
		vhtfm.flags.in_test = False
		gravatar_url = get_gravatar("developers@vhtfm.io")
		vhtfm.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@vhtfm.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(vhtfm.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("vhtfm", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(vhtfm.VhtfmTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = vhtfm.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = vhtfm._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(vhtfm.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from vhtfm.core.doctype.doctype.doctype import DocType
		from vhtfm.utils.typing_validations import (
			VhtfmTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(VhtfmTypeError):
		...
		doctype = vhtfm.get_last_doc("DocType")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("vhtfm.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": vhtfm.get_doc("UTM Source", "test_source"),
			"utm_medium": vhtfm.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": vhtfm.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}

...
UI_TEST_USER = "vhtfm@example.com"


def whitelist_for_tests(fn):
	if vhtfm.request and not vhtfm.flags.in_test and not getattr(vhtfm.local, "dev_server", 0):
		vhtfm.throw("Cannot run UI tests. Use a development server with `fox start`")

	return vhtfm.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = vhtfm.db.exists(doc.doctype, filters)
		if not name:
			d = vhtfm.get_doc(doc)
...
def prepare_webform_test():
	for note in vhtfm.get_all("Note", pluck="name"):
		vhtfm.delete_doc("Note", note, force=True)

	vhtfm.delete_doc_if_exists("Web Form", "note")
	...
	doc = vhtfm.get_doc(
		{
			...
			"communication_date": vhtfm.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from vhtfm.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if vhtfm.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = vhtfm.new_doc("Contact")
...
def create_contact_records():
	if vhtfm.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if vhtfm.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	vhtfm.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = vhtfm.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if vhtfm.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return
...
def create_web_page(title, route, single_thread):
	web_page = vhtfm.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = vhtfm.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": vhtfm.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not vhtfm.db.exists("Web Form", "update-profile-duplicate"):
		doc = vhtfm.get_doc("Web Form", "edit-profile")
		_doc = vhtfm.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if vhtfm.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not vhtfm.db.exists("doc"):
			vhtfm.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = vhtfm.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = vhtfm.get_doc(
		{
...
def create_test_user(username=None):
	...
	if vhtfm.db.exists("User", name):
		return

	user = vhtfm.new_doc("User")
	user.email = name
	user.first_name = "Vhtfm"
	user.new_password = vhtfm.local.conf.admin_password
	...
	all_roles = set(vhtfm.get_all("Role", pluck="name"))
	...
	vhtfm.db.set_single_value("Workspace Settings", "workspace_setup_completed", 1)


@whitelist_for_tests
def setup_tree_doctype():
	vhtfm.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not vhtfm.db.exists("Custom Tree", "All Trees"):
		vhtfm.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	vhtfm.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
def setup_default_view(view, force_reroute=None):
	vhtfm.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	vhtfm.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not vhtfm.db.exists("Custom Field", "Note-kanban"):
		vhtfm.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not vhtfm.db.exists("Kanban Board", "_Note _Kanban"):
		vhtfm.get_doc(
			{
...
def create_todo(description):
	return vhtfm.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not vhtfm.db.exists("Kanban Board", "Admin Kanban"):
		vhtfm.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	vhtfm.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		vhtfm.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)

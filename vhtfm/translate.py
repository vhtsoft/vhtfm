...
from vhtfm.utils import cstr, get_fox_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `vhtfm.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = vhtfm.session.user != "Guest"

	# fetch language from form_dict
	if vhtfm.form_dict._lang:
		language = get_lang_code(vhtfm.form_dict._lang or get_parent_language(vhtfm.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(vhtfm.request.accept_languages.values())
	...
	return vhtfm.db.get_default("lang") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set vhtfm.local.lang from user preferences on session beginning or resumption"""
	user = user or vhtfm.session.user

	# User.language => Session Defaults => vhtfm.local.lang => 'en'
	return (
		vhtfm.get_cached_value("User", user, "language")
		or vhtfm.db.get_default("lang")
		or vhtfm.local.lang
		or "en"
	)
...
def get_lang_code(lang: str) -> str | None:
	return vhtfm.db.get_value("Language", {"name": lang}) or vhtfm.db.get_value(
...
def set_default_language(lang):
	...
	if vhtfm.db.get_default("lang") != lang:
		vhtfm.db.set_default("lang", lang)
	vhtfm.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		vhtfm.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(vhtfm.local.lang)


@vhtfm.whitelist(allow_guest=True)
def get_app_translations():
	if vhtfm.session.user != "Guest":
		language = vhtfm.db.get_value("User", vhtfm.session.user, "language")
	else:
		language = vhtfm.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return vhtfm.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		vhtfm.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from vhtfm.gettext.translate import get_translations_from_mo

	for app in apps or vhtfm.get_installed_apps(_ensure_on_fox=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(vhtfm.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				vhtfm.log_error(message=msg, title="Error in translation file")
				if throw:
					vhtfm.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return vhtfm.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`vhtfm.cache`"""
	vhtfm.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [vhtfm.unscrub(m) for m in vhtfm.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			vhtfm.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			vhtfm.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = vhtfm.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from vhtfm.gettext.extractors.utils import is_translatable

	messages = []
	meta = vhtfm.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = vhtfm.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = vhtfm.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = vhtfm.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = vhtfm.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(vhtfm.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = vhtfm.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`vhtfm.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`vhtfm.core.doctype.Report`."""
	from vhtfm.gettext.extractors.utils import is_translatable

	report = vhtfm.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, vhtfm.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = vhtfm.db.get_value(doctype, name, "module")

	doc_path = vhtfm.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, vhtfm.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`vhtfm._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(vhtfm.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	messages = []
	app_include_js = vhtfm.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = vhtfm.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(vhtfm.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else vhtfm.get_installed_apps(_ensure_on_fox=True):
		if os.path.exists(vhtfm.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(vhtfm.get_app_path(app, "public")):  # noqa: B007
				if "vhtfm/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from vhtfm.gettext.extractors.utils import extract_messages_from_code

	vhtfm.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in vhtfm.flags.scanned_files:
		return []

	vhtfm.flags.scanned_files.add(path)
	...
	with open(path) as sourcefile:
		...
		return [
			(os.path.relpath(path, fox_path), message, context, line)
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from vhtfm.gettext.extractors.javascript import extract_javascript
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		vhtfm.get_file_items(untranslated_file, ignore_empty_lines=False),
		vhtfm.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in vhtfm.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in vhtfm.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in vhtfm.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = vhtfm.translate.get_all_languages()

	source_app_translations_dir = vhtfm.get_app_path(source_app, "translations")
	target_app_translations_dir = vhtfm.get_app_path(target_app, "translations")
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = vhtfm.get_app_path(app, "translations")
	vhtfm.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `vhtfm.local.response`"""
	if "__messages" not in vhtfm.local.response:
		vhtfm.local.response["__messages"] = {}

	vhtfm.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = vhtfm.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_language_codes():
		return vhtfm.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return vhtfm.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return vhtfm.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		return vhtfm.cache.get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return vhtfm.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = vhtfm.get_all("DocType", {"translated_doctype": 1}, pluck="name")
	custom_dts = vhtfm.get_all(
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = vhtfm.get_print(...)
	```
	"""
	if not language or language == vhtfm.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = vhtfm.local.lang
	_jenv = vhtfm.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	vhtfm.local.lang = language
	vhtfm.local.jenv = None

	yield

	# restore original values
	vhtfm.local.lang = _lang
	vhtfm.local.jenv = _jenv

...
from vhtfm.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(vhtfm.local, "fullnames"):
		vhtfm.local.fullnames = {}

	if not vhtfm.local.fullnames.get(user):
		p = vhtfm.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			vhtfm.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			vhtfm.local.fullnames[user] = user

	return vhtfm.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return vhtfm.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			vhtfm.throw(
				error_message.format(vhtfm.bold(phone_number), vhtfm.bold(fieldname)),
				title=error_title,
				exc=vhtfm.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		vhtfm.throw(
			vhtfm._("{0} is not a valid Phone Number").format(phone_number), vhtfm.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Vhtfm doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		vhtfm.throw(vhtfm._("{0} is not a valid Name").format(name), vhtfm.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = vhtfm.utils.escape_html(e)
				vhtfm.throw(
					vhtfm._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		vhtfm.throw(vhtfm._("'{0}' is not a valid URL").format(vhtfm.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if vhtfm.flags.in_import or vhtfm.flags.in_install or vhtfm.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(vhtfm.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", vhtfm._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	vhtfm.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return vhtfm.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return vhtfm.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise vhtfm.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = vhtfm.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return vhtfm.local.site_path
...
def get_fox_path():
	return os.environ.get("VHTFM_FOX_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(vhtfm.__file__), "..", "..", "..")
	)


def get_fox_id():
	return vhtfm.get_conf().get("fox_id", get_fox_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or vhtfm.local.site}@{get_fox_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Vhtfm WSGI."""
	from vhtfm.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = vhtfm.get_hooks().get(hook_name)
	if method:
		method = vhtfm.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in vhtfm.get_hooks(hook):
		out = out or vhtfm.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(vhtfm.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in vhtfm.get_installed_apps():
				include_path = vhtfm.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(vhtfm.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else vhtfm.utils.md_to_html(text)
...
def get_site_info():
	from vhtfm.email.queue import get_emails_sent_this_month
	from vhtfm.utils.user import get_system_managers

	# only get system users
	users = vhtfm.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", vhtfm.STANDARD_USERS)},
	...
	system_settings = vhtfm.db.get_singles_dict("System Settings")
	space_usage = vhtfm._dict((vhtfm.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": vhtfm.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in vhtfm.get_hooks("get_site_info"):
		site_info.update(vhtfm.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(vhtfm.as_json(site_info))
...
def get_db_count(*args):
	...
	db_count = {}
	for doctype in args:
		db_count[doctype] = vhtfm.db.count(doctype)

	return json.loads(vhtfm.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: vhtfm function to be called
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = vhtfm.as_unicode(filters)
...
def set_request(**kwargs):
	...
	vhtfm.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return vhtfm.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(vhtfm.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return vhtfm.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = vhtfm.parse_json(vhtfm.read_file("assets/assets.json"))

		if assets_rtl := vhtfm.read_file("assets/assets-rtl.json"):
			assets.update(vhtfm.parse_json(assets_rtl))

		return assets

	if not hasattr(vhtfm.local, "assets_json"):
		if not vhtfm.conf.developer_mode:
			vhtfm.local.assets_json = vhtfm.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			vhtfm.local.assets_json = _get_assets()

	return vhtfm.local.assets_json


def get_fox_relative_path(file_path):
	"""Fix paths relative to the fox root directory if exists and return the absolute path.
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, vhtfm._dict()).update(
...
class CallbackManager:
	...
	Example usage: vhtfm.db.after_commit
	"""

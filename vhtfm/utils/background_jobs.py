...
from vhtfm import _
from vhtfm.utils import CallbackManager, cint, get_fox_id
from vhtfm.utils.commands import log
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = vhtfm.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via vhtfm.call()
	...
	if deduplicate:
		if not job_id:
			vhtfm.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			vhtfm.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not vhtfm.flags.in_test:
		from vhtfm.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not vhtfm.flags.in_test)
	if call_directly:
		return vhtfm.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if vhtfm.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=vhtfm.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=vhtfm.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		vhtfm.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"vhtfm.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(vhtfm.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in vhtfm.get_hooks("before_job"):
		vhtfm.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (vhtfm.db.InternalError, vhtfm.RetryBackgroundJobError) as e:
		vhtfm.db.rollback()

		if retry < 5 and (
			isinstance(e, vhtfm.RetryBackgroundJobError)
			or (vhtfm.db.is_deadlocked(e) or vhtfm.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			vhtfm.log_error(title=method_name)
			raise

	except Exception as e:
		vhtfm.db.rollback()
		vhtfm.log_error(title=method_name)
		vhtfm.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		vhtfm.db.commit()
		...
		for after_job_task in vhtfm.get_hooks("after_job"):
			vhtfm.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		vhtfm.local.job.after_job.run()
...
class VhtfmWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_vhtfm_scheduler()
	...
	def start_vhtfm_scheduler(self):
		from vhtfm.utils.scheduler import start_scheduler
...
def start_worker_pool(
	...
	import vhtfm.database.query  # sqlparse and indirect imports
	import vhtfm.query_builder  # pypika
	import vhtfm.utils  # common utils
	import vhtfm.utils.safe_exec
	import vhtfm.utils.scheduler
	...
	pool = WorkerPool(
		...
		worker_class=VhtfmWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		vhtfm.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(vhtfm.local, "conf"):
		raise Exception("You need to call vhtfm.init")
	...
	if conf.get("use_rq_auth"):
		if username:
			...
			cred["username"] = conf.rq_username or get_fox_id()
	...
	try:
		...
		log(
			...
			"You can reset credentials using `fox create-rq-users` CLI and restart the server",
			colour="red",
		)
		raise
	except Exception as e:
		log(
			f"Please make sure that Redis Queue runs @ {vhtfm.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def get_queues(connection=None) -> list[Queue]:
	"""Get all the queues linked to the current fox."""
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{vhtfm.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from vhtfm.utils import create_batch

	conf = vhtfm.conf if vhtfm.conf else vhtfm.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(vhtfm.local, "posthog", None)
	with suppress(Exception):
		ph and ph.flush()


def _start_sentry():
	sentry_dsn = os.getenv("VHTFM_SENTRY_DSN")
	if not sentry_dsn:
		return

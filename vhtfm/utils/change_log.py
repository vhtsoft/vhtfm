...
import vhtfm
from vhtfm import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = vhtfm._dict(
		json.loads(vhtfm.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "vhtfm" in current_versions:
		set_in_change_log("vhtfm", current_versions["vhtfm"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(vhtfm.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = vhtfm.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "vhtfm": {
	                        "title": "Vhtfm Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in vhtfm.get_installed_apps(_ensure_on_fox=True):
		app_hooks = vhtfm.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = vhtfm.get_attr(app + ".__version__")
...
def check_for_update():
	if vhtfm.get_system_settings("disable_system_update_notification"):
		return

	updates = vhtfm._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(vhtfm.cache.sismember("changelog-update-user-set", vhtfm.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for vhtfm
...
def add_message_to_redis(update_json):
	...
	vhtfm.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in vhtfm.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in vhtfm.get_roles(user)]
	vhtfm.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = vhtfm.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if vhtfm.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				...
				if app.security_issues:
					...
					security_msg = security_msg.format(vhtfm.bold(app.security_issues))
	...
	if on_vhtfmcloud():
		primary_action = {
			"label": _("Update from Vhtfm Cloud"),
			"client_action": "window.open",
			"args": f"https://vhtfmcloud.com/dashboard/sites/{vhtfm.local.site}",
		}
	...
	if update_message:
		...
		vhtfm.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = vhtfm.get_app_path(app, "..", "pyproject.toml")

...
from vhtfm.core.doctype.file.file import FILE_ENCODING_OPTIONS
from vhtfm.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = vhtfm.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = vhtfm.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	content = [vhtfm.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	try:
		...
		dialect = sniffer.sniff(sample="\n".join(sample_content), delimiters=vhtfm.flags.delimiter_options)
	except csv.Error:
	...
	try:
		...
		vhtfm.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	vhtfm.response["result"] = cstr(to_csv(args.data))
	vhtfm.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	vhtfm.response["result"] = cstr(to_csv(data))
	vhtfm.response["doctype"] = filename
	vhtfm.response["type"] = "csv"
...
def check_record(d):
	...
	from vhtfm.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				vhtfm.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and vhtfm.db.exists(doctype, d["name"]):
		if overwrite:
			doc = vhtfm.get_doc(doctype, d["name"])

...
from vhtfm.locale import get_date_format, get_first_day_of_the_week, get_number_format, get_time_format
from vhtfm.types.filter import Filters, FilterSignature, FilterTuple
from vhtfm.utils.deprecations import deprecated
from vhtfm.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
def getdate(
	...
	try:
		...
		vhtfm.throw(
			vhtfm._("{} is not a valid date string.").format(vhtfm.bold(string_date)),
			title=vhtfm._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			vhtfm.throw(vhtfm._("Please select a valid date filter"), title=vhtfm._("Invalid Date"))
...
def get_system_timezone() -> str:
	...
	return vhtfm.get_system_settings("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def now() -> str:
	...
	if vhtfm.flags.current_date:
		return (
			getdate(vhtfm.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(vhtfm.local, "user_date_format", None) is None:
		vhtfm.local.user_date_format = get_date_format()

	return vhtfm.local.user_date_format
...
def get_user_time_format() -> str:
	...
	if getattr(vhtfm.local, "user_time_format", None) is None:
		vhtfm.local.user_time_format = get_time_format()

	return vhtfm.local.user_time_format
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(vhtfm.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(vhtfm.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(vhtfm.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		vhtfm.throw(
			vhtfm._("Value {0} must be in the valid duration format: d h m s").format(vhtfm.bold(duration))
		)
...
def global_date_format(date: DateTimeLikeObject, format="long") -> str:
	...
	date = getdate(date)
	return babel.dates.format_date(date, locale=(vhtfm.local.lang or "en").replace("-", "_"), format=format)
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `vhtfm.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `vhtfm.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the Vhtfm fieldtype provided.
	...
	Mapping of Python types => Vhtfm types:
...
def flt(
	...
	try:
		...
		if isinstance(e, vhtfm.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8") -> str:
	...
	return vhtfm.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or vhtfm.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding (legacy)":
		...
		vhtfm.throw(
			vhtfm._("Unknown Rounding Method: {}").format(rounding_method),
			exc=vhtfm.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		vhtfm.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	if precision is None:
		precision = cint(vhtfm.db.get_default("currency_precision")) or None
	...
	if number_format.decimal_separator:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = vhtfm.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and vhtfm.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = vhtfm.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = vhtfm.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {vhtfm._(symbol)}"
		else:
			amount = f"{vhtfm._(symbol)} {amount}"

	return amount


from vhtfm.deprecation_dumpster import get_number_format_info
...
def __getattr__(name):
	if name == "number_format_info":
		...
		deprecation_warning(
			"unkown",
			"v16",
			"use vhtfm.utils.number_format.NUMBER_FORMAT_MAP instead of vhtfm.utils.data.number_format_info",
		)
	else:
		try:
			globals()[name]
...
def money_in_words(
	...
	from vhtfm.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = vhtfm.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = get_number_format()

	fraction_units = vhtfm.db.get_value("Currency", main_currency, "fraction_units", cache=True)
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else vhtfm.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src: str) -> dict[str, str] | None:
	...
	from vhtfm import cache, safe_decode
	from vhtfm.core.doctype.file.utils import get_local_image

	if not src:
		vhtfm.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = vhtfm.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	...
	from vhtfm.utils.file_manager import get_file_path
...
def pretty_date(iso_datetime: datetime.datetime | str) -> str:
	...
	locale = vhtfm.local.lang.replace("-", "_") if vhtfm.local.lang else None
...
def comma_or(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' or 'c'"
	"""
	return comma_sep(some_list, vhtfm._("{0} or {1}"), add_quotes)
...
def comma_and(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' and 'c'"
	"""
	return comma_sep(some_list, vhtfm._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list: list | tuple, pattern: str, add_quotes=True) -> str:
	...
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(vhtfm._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri: str | None = None, full_address: bool = False) -> str:
	...
	host_name = vhtfm.local.conf.host_name or vhtfm.local.conf.hostname
	...
	if not host_name:
		...
		elif vhtfm.local.site:
			protocol = "http://"

			if vhtfm.local.conf.ssl_certificate:
				protocol = "https://"

			elif vhtfm.local.conf.wildcard:
				domain = vhtfm.local.conf.wildcard.get("domain")
				if (
					domain
					and vhtfm.local.site.endswith(domain)
					and vhtfm.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + vhtfm.local.site

		else:
			host_name = vhtfm.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = vhtfm.get_request_header("REQUEST_URI", "")

	port = vhtfm.conf.http_port
	if not port and vhtfm.conf.developer_mode:
		port = vhtfm.conf.webserver_port

	if (
		# XXX: This config is used as proxy for "is production mode enabled?"
		not vhtfm.conf.restart_supervisor_on_update
		and not vhtfm.conf.restart_systemd_on_update
		and host_name
...
def get_host_name_from_request() -> str:
	...
	if hasattr(vhtfm.local, "request") and vhtfm.local.request and vhtfm.local.request.host:
		protocol = "https://" if "https" == vhtfm.get_request_header("X-Forwarded-Proto", "") else "http://"
		return protocol + vhtfm.local.request.host
...
def get_host_name() -> str:
	...
	e.g. If site is 'https://cloud.vhtfm.io', returns 'cloud.vhtfm.io'.
	"""
...
def get_link_to_form(doctype: str, name: str | None = None, label: str | None = None) -> str:
	...
	e.g. get_link_to_form("Sales Invoice", "INV-0001", "Link Label") returns:
	    '<a href="https://vhtfm.io/app/sales-invoice/INV-0001">Link Label</a>'.
	"""
	from vhtfm import _
...
def get_link_to_report(
	...
	e.g. get_link_to_report("Revenue Report", "Link Label") returns:
	        '<a href="https://vhtfm.io/app/query-report/Revenue%20Report">Link Label</a>'.
	"""
	if not label:
		label = name
...
def get_url_to_form(doctype: str, name: str | None = None) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://vhtfm.io",
	         returns 'https://vhtfm.io/app/sales-invoice/INV-00001'
	"""
	if not name:
...
def get_url_to_list(doctype: str) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://vhtfm.io",
	         returns 'https://vhtfm.io/app/sales-invoice'
	"""
...
def get_url_to_report(name, report_type: str | None = None, doctype: str | None = None) -> str:
	...
	e.g. when name="Sales Register" and your site URL is "https://vhtfm.io",
	         returns 'https://vhtfm.io/app/query-report/Sales%20Register'

	You can optionally pass `report_type` and `doctype` to get the URL for a Report Builder report.

	get_url_to_report("Revenue", "Report Builder", "Sales Invoice") -> 'https://vhtfm.io/app/sales-invoice/view/report/Revenue'
	"""
...
def filter_operator_is(value: str, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		vhtfm.throw(vhtfm._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, filters: FilterSignature, filters_config=None) -> "vhtfm._dict":
	"""Return a `_dict` like:
	...
	"""
	from vhtfm.database.utils import NestedSetHierarchy
	from vhtfm.model import child_table_fields, default_fields, optional_fields
	...
	f = vhtfm._dict(doctype=ft[0], fieldname=ft[1], operator=ft[2], value=ft[3])
	...
	if f.operator.lower() not in valid_operators:
		vhtfm.throw(vhtfm._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = vhtfm.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):
			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if vhtfm.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = vhtfm.get_meta(f.doctype).get_field(f.fieldname) if f.doctype else None
	except vhtfm.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> None:
	...
	if vhtfm.db and vhtfm.db.db_type == "mariadb":
	...
	def _raise_exception():
		vhtfm.throw(_("Invalid field name {0}").format(column_name), vhtfm.DataError)
...
def scrub_urls(html: str) -> str:
	...
	e.g. If HTML is '<a href="/files/abc.jpeg">View Image</a>' and site URL is 'https://vhtfm.io',
	        returns '<a href="https://vhtfm.io/files/abc.jpeg">View Image</a>'.
	"""
...
def quoted(url: str) -> str:
	...
	e.g. 'https://vhtfm.io/files/my Image file.jpeg' -> 'https://vhtfm.io/files/my%20Image%20file.jpeg'
	"""
...
def generate_hash(*args, **kwargs) -> str:
	...
	"""
	return vhtfm.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	e.g. {
	        "email": "faris@vhtfm.io",
	        "image": "/assets/vhtfm/images/ui/avatar.png",
	        "name": "Faris Ansari"
	}
	"""
	try:
		user = vhtfm.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = vhtfm._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		vhtfm.throw(msg, title=vhtfm._("Syntax Error"))
	except Exception as e:
		vhtfm.msgprint(
			vhtfm._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta | str) -> str:
	...
	# but Vhtfm doesn't - I think via babel : only allows 0..23 range for hour
...
def is_site_link(link: str) -> bool:
	...
	if link.startswith("/"):
		return True
	return urlparse(link).netloc == urlparse(vhtfm.utils.get_url()).netloc
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = vhtfm.db.get_value("UTM Source", source, "slug") or source
		_campaign = vhtfm.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = vhtfm.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = vhtfm.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = vhtfm.db.get_value("UTM Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not vhtfm.db.exists("UTM Source", source):
			source = vhtfm.new_doc("UTM Source")
			source.name = url_source
			source.description = f"Autogenerated from {url_trackers}"
			source.save(ignore_permissions=True)
		vhtfm_trackers["utm_source"] = source

	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = vhtfm.db.get_value("UTM Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not vhtfm.db.exists("UTM Medium", medium):
			medium = vhtfm.new_doc("UTM Medium")
			medium.name = url_medium
			medium.description = f"Autogenerated from {url_trackers}"
			medium.save(ignore_permissions=True)
		vhtfm_trackers["utm_medium"] = medium

	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = vhtfm.db.get_value("UTM Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		if create and campaign == url_campaign and not vhtfm.db.exists("UTM Campaign", campaign):
			campaign = vhtfm.new_doc("UTM Campaign")
			campaign.name = url_campaign
			campaign.campaign_description = f"Autogenerated from {url_trackers}"
			campaign.save(ignore_permissions=True)
		vhtfm_trackers["utm_campaign"] = campaign

	if url_content := url_trackers.get("utm_content", url_trackers.get("content")):
		vhtfm_trackers["utm_content"] = url_content

	return vhtfm_trackers

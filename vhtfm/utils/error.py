...
EXCLUDE_EXCEPTIONS = (
	...
	vhtfm.CSRFTokenError,  # CSRF covers OAuth too
	vhtfm.SecurityException,
	vhtfm.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from vhtfm.utils.sentry import capture_exception
	...
	traceback = vhtfm.as_unicode(traceback or vhtfm.get_traceback(with_context=True))
	...
	if vhtfm.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = vhtfm.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				vhtfm.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For vhtfm.throws the exception source is possibly present after skipping vhtfm.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "vhtfm":

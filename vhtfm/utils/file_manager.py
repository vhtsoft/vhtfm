...
import vhtfm
from vhtfm import _, conf
from vhtfm.query_builder.utils import DocType
from vhtfm.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = vhtfm.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = vhtfm.form_dict.doctype
	dn = vhtfm.form_dict.docname
	file_url = vhtfm.form_dict.file_url
	filename = vhtfm.form_dict.filename
	vhtfm.form_dict.is_private = cint(vhtfm.form_dict.is_private)

	if not filename and not file_url:
		vhtfm.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = vhtfm.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	vhtfm.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = vhtfm.form_dict.file_size
	...
	try:
		...
		return vhtfm.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using vhtfm.form
	if "filedata" in vhtfm.form_dict:
		if "," in vhtfm.form_dict.filedata:
			vhtfm.form_dict.filedata = vhtfm.form_dict.filedata.rsplit(",", 1)[1]
		vhtfm.uploaded_content = safe_b64decode(vhtfm.form_dict.filedata)
		vhtfm.uploaded_filename = vhtfm.form_dict.filename
		return vhtfm.uploaded_filename, vhtfm.uploaded_content
	else:
		vhtfm.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in vhtfm.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = vhtfm.get_doc("File", name)
		return {k: b.get(k) for k in vhtfm.get_hooks()["write_file_keys"]}
	return False
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in vhtfm.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				vhtfm.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = vhtfm.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = vhtfm.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = vhtfm.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = vhtfm.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = vhtfm.utils.get_site_path("public", "files", parts[-1])

		else:
			path = vhtfm.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = vhtfm.get_all("File", {"file_name": fname}, pluck="name")

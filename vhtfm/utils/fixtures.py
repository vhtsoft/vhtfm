...
def sync_fixtures(app=None):
	...
	vhtfm.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	vhtfm.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = vhtfm.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = vhtfm.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path, sort=True)
		except (ImportError, vhtfm.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = vhtfm.get_app_path(app, "fixtures", "custom_scripts")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = vhtfm.get_installed_apps()
	for app in apps:
		fixture_auto_order = bool(next(iter(vhtfm.get_hooks("fixture_auto_order", app_name=app)), False))
		fixtures = vhtfm.get_hooks("fixtures", app_name=app)
		prefix = None
		for index, fixture in enumerate(fixtures, start=1):
			...
			if not os.path.exists(vhtfm.get_app_path(app, "fixtures")):
				os.mkdir(vhtfm.get_app_path(app, "fixtures"))

			filename = vhtfm.scrub(fixture)
			if prefix:
			...
			export_json(
				fixture,
				vhtfm.get_app_path(app, "fixtures", filename + ".json"),
				filters=filters,

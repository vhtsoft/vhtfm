...
def VhtfmClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	commands.update({"get-vhtfm-commands": get_vhtfm_commands, "get-vhtfm-help": get_vhtfm_help})
	VhtfmClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="fox")


def get_app_groups() -> dict[str, click.Group | click.Command]:
	"""Get all app groups, put them in main group "vhtfm" since fox is
	...
	return dict(
		vhtfm=click.group(
			name="vhtfm", commands=commands, cls=VhtfmClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=VhtfmClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "vhtfm":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return vhtfm.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif env_site := os.environ.get("VHTFM_SITE"):
		return [env_site]
	elif default_site := vhtfm.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := vhtfm.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-vhtfm-commands")
def get_vhtfm_commands():
	commands = list(get_app_commands("vhtfm"))
...
@click.command("get-vhtfm-help")
def get_vhtfm_help():
	print(click.Context(get_app_groups()["vhtfm"]).get_help())


def get_apps():
	return vhtfm.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not vhtfm._dev_server:

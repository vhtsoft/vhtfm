...
from vhtfm.utils import cint, strip_html_tags
from vhtfm.utils.data import cstr
...
def reset():
	...
	:return:
	"""
	vhtfm.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in vhtfm.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = vhtfm.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = vhtfm.get_installed_apps()
		module_app = vhtfm.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(vhtfm.scrub(d.module)) and module_app[vhtfm.scrub(d.module)] in installed_apps
		]

		return doctypes

	return vhtfm.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if vhtfm.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = vhtfm._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = vhtfm.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", vhtfm.model.table_fields], "options": doctype},
		)
	...
	all_records = vhtfm.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = vhtfm.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": vhtfm.db.escape(doctype),
					"name": vhtfm.db.escape(doc.name),
					"content": vhtfm.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": vhtfm.db.escape((title or "")[: int(vhtfm.db.VARCHAR_LEN)]),
					"route": vhtfm.db.escape((route or "")[: int(vhtfm.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	vhtfm.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = vhtfm._dict()
	child_search_fields = vhtfm._dict()

	for child in meta.get_table_fields():
		child_meta = vhtfm.get_meta(child.options)
		...
		if search_fields:
			...
			for record in child_records:
				all_children.setdefault(record.parent, vhtfm._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in vhtfm.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(vhtfm.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if vhtfm.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = vhtfm.get_app_path(app, "www")
		path_to_index = vhtfm.get_app_path(app, "www")
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(vhtfm.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := vhtfm.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = vhtfm.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = vhtfm.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
	...
	for field in conflict_fields:
		...
		elif vhtfm.db.db_type == "postgres":
...
def sync_value_in_queue(value):
	try:
		...
		vhtfm.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	vhtfm.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from vhtfm.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from vhtfm.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(vhtfm.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = vhtfm.get_meta(r.doctype)
					if meta.image_field:
						r.image = vhtfm.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			vhtfm.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({vhtfm.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = vhtfm.db.multisql(
			{

...
import vhtfm
from vhtfm import _
from vhtfm.query_builder.functions import DateFormat, Function
from vhtfm.query_builder.utils import DocType
from vhtfm.utils.data import add_to_date, cstr, flt, now_datetime
...
def get_monthly_results(
	...
	date_format = "%m-%Y" if vhtfm.db.db_type != "postgres" else "MM-YYYY"
...
def get_monthly_goal_graph_data(
	...
	if history and "{" in cstr(history):
		with suppress(ValueError):
			month_to_value_dict = vhtfm.parse_json(history)

	if month_to_value_dict is None:  # nosemgrep
		doc_filter = {}
		with suppress(ValueError):
			doc_filter = vhtfm.parse_json(filters or "{}")

...
def inspect(var, render=True):
	from vhtfm.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import vhtfm
	from vhtfm import _, _dict, throw
	from vhtfm.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not vhtfm.flags.web_block_scripts:
...
def include_script(path, preload=True):
	...
	if preload:
		...
		vhtfm.local.preload_assets["script"].append(path)
...
def include_icons(path, preload=True):
	...
	if preload:
		...
		vhtfm.local.preload_assets["icons"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		vhtfm.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from vhtfm.utils import get_assets_json
	from vhtfm.website.utils import abs_url

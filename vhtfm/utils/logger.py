...
default_log_level = logging.WARNING if vhtfm._dev_server else logging.ERROR
stream_logging = os.environ.get("VHTFM_STREAM_LOGGING")


def create_handler(module, site=None, max_size=100_000, file_count=20, stream_only=False):
	"""Create and return a Vhtfm-specific logging handler."""
...
def get_logger(
	...
	Return a Python logger object with Site and Fox level logging capabilities.
	"""

	if allow_site is True:
		site = getattr(vhtfm.local, "site", None)
	...
	try:
		return vhtfm.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(vhtfm.log_level or default_log_level)
	...
	vhtfm.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(vhtfm.local, "site", None)
			form_dict = sanitized_dict(getattr(vhtfm.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	vhtfm.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	vhtfm.loggers = {}

...
from vhtfm.query_builder.functions import Coalesce, Max
from vhtfm.query_builder.terms import SubQuery
from vhtfm.query_builder.utils import DocType


class NestedSetRecursionError(vhtfm.ValidationError):
	pass


class NestedSetMultipleRootsError(vhtfm.ValidationError):
	pass


class NestedSetChildExistsError(vhtfm.ValidationError):
	pass


class NestedSetInvalidMergeError(vhtfm.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + vhtfm.scrub(doc.doctype)
	...
	vhtfm.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
	vhtfm.clear_document_cache(doc.doctype)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = vhtfm.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
	...
	vhtfm.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(Table.rgt >= right).run()
	vhtfm.qb.update(Table).set(Table.lft, Table.lft + 2).where(Table.lft >= right).run()

	if vhtfm.qb.from_(Table).select("*").where((Table.lft == right) | (Table.rgt == right + 1)).run():
		vhtfm.throw(_("Nested set error. Please contact the Administrator."))

	# update index of new node
	vhtfm.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(Table.name == name).run()
	return right
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	vhtfm.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	vhtfm.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	vhtfm.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		vhtfm.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = vhtfm.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
		new_diff = max_rgt + 1 - doc.lft

	# bring back from dark side
	vhtfm.qb.update(Table).set(Table.lft, -Table.lft + new_diff).set(Table.rgt, -Table.rgt + new_diff).where(
		Table.lft < 0
	).run()
...
def rebuild_tree(doctype: str) -> None:
	...
	if vhtfm.request and vhtfm.local.form_dict.cmd == "rebuild_tree":
		vhtfm.only_for("System Manager")

	meta = vhtfm.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		vhtfm.throw(
			_("Rebuilding of tree is not supported for {}").format(vhtfm.bold(doctype)),
			title=_("Invalid Action"),
		)

	parent_field = meta.nsm_parent_field or f"parent_{vhtfm.scrub(doctype)}"
	...
	vhtfm.db.auto_commit_on_many_writes = 0
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (vhtfm.qb.from_(table).where(column == parent).select(table.name)).run()
	...
	vhtfm.db.set_value(doctype, parent, {"lft": left, "rgt": right}, update_modified=False)
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in vhtfm.get_all(doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"):
		vhtfm.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	vhtfm.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	vhtfm.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()

	vhtfm.clear_document_cache(doctype)
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		...
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = vhtfm.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not getattr(self, "allow_root_deletion", True):
			vhtfm.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def validate_if_child_exists(self):
		has_children = vhtfm.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			vhtfm.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = vhtfm.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				vhtfm.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return vhtfm.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if vhtfm.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				vhtfm.throw(
	...
	def get_parent(self) -> "NestedSet":
		...
		if parent_name:
			return vhtfm.get_doc(self.doctype, parent_name)

	def get_children(self) -> Iterator["NestedSet"]:
		"""Return a generator that yields child Documents."""
		child_names = vhtfm.get_list(self.doctype, filters={self.nsm_parent_field: self.name}, pluck="name")
		for name in child_names:
			yield vhtfm.get_doc(self.doctype, name)
...
def get_root_of(doctype):
	...
	node_query = SubQuery(vhtfm.qb.from_(t2).select(Count("*")).where((t2.lft < t1.lft) & (t2.rgt > t1.rgt)))
	result = vhtfm.qb.from_(t1).select(t1.name).where((node_query == 0) & (t1.rgt > t1.lft)).run()

...
class SignupDisabledError(vhtfm.PermissionError): ...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = vhtfm.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := vhtfm.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": vhtfm.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": vhtfm.utils.get_url(),
		"token": vhtfm.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = vhtfm.conf.get(f"{provider}_login")
	...
	return vhtfm.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
	...
	if not (info.get("email_verified") or info.get("email")):
		vhtfm.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	*,
	...
	if not (state and state["token"]):
		vhtfm.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return vhtfm.respond_as_web_page(
			"Signup is Disabled",
	...
	vhtfm.local.login_manager.user = user
	vhtfm.local.login_manager.post_login()

	# because of a GET request!
	vhtfm.db.commit()

	if vhtfm.utils.cint(generate_login_token):
		login_token = vhtfm.generate_hash(length=32)
		vhtfm.cache.set_value(f"login_token:{login_token}", vhtfm.local.session.sid, expires_in_sec=120)

		vhtfm.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=vhtfm.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict, provider: str) -> "User":
	from vhtfm.integrations.doctype.social_login_key.social_login_key import provider_allows_signup

	try:
		return vhtfm.get_doc("User", user)
	except vhtfm.DoesNotExistError:
		if not provider_allows_signup(provider):
			raise SignupDisabledError

	user: User = vhtfm.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": vhtfm.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		vhtfm.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if update_user_record:
		...
		if default_role := vhtfm.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(desk_user: bool, redirect_to: str | None = None, provider: str | None = None):
	...
	if not redirect_to:
		...
		redirect_to = vhtfm.utils.get_url(desk_uri if desk_user else "/me")

	vhtfm.local.response["location"] = redirect_to

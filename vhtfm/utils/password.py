...
import vhtfm
from vhtfm import _
from vhtfm.query_builder import Table
from vhtfm.utils import cstr, encode
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	# TODO: Simplify this via aliasing methods in `vhtfm.qb`
	...
	try:
		...
		if vhtfm.db.is_data_too_long(e):
			vhtfm.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	vhtfm.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises vhtfm.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise vhtfm.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	vhtfm.cache.hdel("login_failed_count", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		vhtfm.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not vhtfm.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	vhtfm.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		vhtfm.throw(_("Encryption key is in invalid format!"))
...
def decrypt(txt, encryption_key=None, key: str | None = None):
	...
	try:
		...
		vhtfm.throw(
			...
			+ "<br><br>"
			+ _(
				"Please visit https://vhtfmcloud.com/docs/sites/migrate-an-existing-site#encryption-key for more information."
			),
		)


def get_encryption_key():
	if "encryption_key" not in vhtfm.local.conf:
		from vhtfm.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		vhtfm.local.conf.encryption_key = encryption_key

	return vhtfm.local.conf.encryption_key


def get_password_reset_limit():
	return vhtfm.get_system_settings("password_reset_limit") or 3

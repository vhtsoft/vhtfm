...
from vhtfm.utils import cstr, scrub_urls
from vhtfm.utils.caching import redis_cache
from vhtfm.utils.jinja_globals import bundled_asset, is_rtl

cssutils.log.setLog(vhtfm.logger("cssutils"))
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				vhtfm.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or vhtfm.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or vhtfm.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or vhtfm.db.get_single_value(
...
def _get_base64_image(src):
	...
	try:
		...
		vhtfm.logger("pdf").error("Failed to convert inline images to base64", exc_info=True)
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = vhtfm.read_file(os.path.join(vhtfm.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = vhtfm.get_hooks(id_map.get(html_id))
			html = vhtfm.get_attr(hook_func[-1])(
				soup=soup,
				head=head,
				content=content,
				styles=styles,
			...
			# create temp file
			fname = os.path.join("/tmp", f"vhtfm-pdf-{vhtfm.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/vhtfm-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = vhtfm.cache.hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			vhtfm.cache.hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass

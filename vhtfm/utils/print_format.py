...
from vhtfm.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	vhtfm.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					vhtfm.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			vhtfm.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						vhtfm.publish_realtime(task_id=task_id, message="Failed")
					vhtfm.log_error(
		...
		if task_id is None:
			vhtfm.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			vhtfm.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			vhtfm.local.response.filecontent = merged_pdf.getvalue()
			vhtfm.local.response.type = "pdf"


from vhtfm.deprecation_dumpster import read_multi_pdf
...
def download_pdf(doctype, name, format=None, doc=None, no_letterhead=0, language=None, letterhead=None):
	doc = doc or vhtfm.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = vhtfm.get_print(
			doctype, name, format, doc=doc, as_pdf=True, letterhead=letterhead, no_letterhead=no_letterhead
		)

	vhtfm.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	vhtfm.local.response.filecontent = pdf_file
	vhtfm.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	vhtfm.local.response.filename = "report.pdf"
	vhtfm.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	vhtfm.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = vhtfm.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		vhtfm.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if (
			...
			vhtfm.throw(_("PDF generation failed"))
	except cups.IPPError:
		vhtfm.throw(_("Printing failed"))

...
import vhtfm
from vhtfm.utils import get_fox_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return vhtfm._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		conf = vhtfm.get_site_config()
		if conf.redis_queue_sentinel_enabled:
			from vhtfm.utils.redis_wrapper import get_sentinel_connection
	...
	def get_acl_key_rules(cls, include_key_prefix=False):
		...
		rules = ["rq:[^q]*", "rq:queues", f"rq:queue:{get_fox_id()}:*"]
	...
	def gen_acl_list(cls, set_admin_password=False):
		...
		This list contains default ACL user and the fox ACL user(used by all sites incase of ACL is enabled).
		"""
		fox_username = get_fox_id()
		fox_user_rules = cls.get_acl_key_rules(include_key_prefix=True) + cls.get_acl_command_rules()
		fox_user_rule_str = " ".join(fox_user_rules).strip()
		fox_user_password = random_string(20)
		...
		return [
			...
			f"user {fox_username} on >{fox_user_password} {fox_user_rule_str}",
		], {
			"fox": (fox_username, fox_user_password),

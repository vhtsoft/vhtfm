...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support vhtfm.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{vhtfm.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in vhtfm.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = vhtfm.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(vhtfm.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		vhtfm.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(
		...
		:param shared: shared vhtfm key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in vhtfm.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in vhtfm.local.cache[_name]:
				del vhtfm.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in vhtfm.local.cache[_name]:
					del vhtfm.local.cache[_name][key]
...
def setup_cache():
	if vhtfm.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in vhtfm.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=vhtfm.conf.get("redis_cache_sentinel_username"),
			sentinel_password=vhtfm.conf.get("redis_cache_sentinel_password"),
			master_username=vhtfm.conf.get("redis_cache_master_username"),
			master_password=vhtfm.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			vhtfm.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(vhtfm.conf.get("redis_cache"))

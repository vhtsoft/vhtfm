...
import vhtfm
from vhtfm.utils import cint, get_fox_path, get_datetime, get_sites, now_datetime
from vhtfm.utils.background_jobs import set_niceness
...
def _proctitle(message):
	setproctitle.setthreadtitle(f"vhtfm-scheduler: {message}")
...
def start_scheduler() -> NoReturn:
	...
	lock_path = os.path.abspath(os.path.join(get_fox_path(), "config", "scheduler_process"))

	try:
		lock = FileLock(lock_path)
		lock.acquire(blocking=False)
	except Timeout:
		vhtfm.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			vhtfm.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		vhtfm.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		vhtfm.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if vhtfm.db.is_access_denied(e):
			vhtfm.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = vhtfm.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = vhtfm.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				vhtfm.logger("scheduler").error(
					f"Invalid Job on {vhtfm.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if vhtfm.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{vhtfm.local.site}: Maintenance mode is ON")
		return True

	if vhtfm.local.conf.pause_scheduler:
		if verbose:
			cprint(f"{vhtfm.local.site}: vhtfm.conf.pause_scheduler is SET")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	if vhtfm.conf.disable_scheduler:
		if verbose:
			cprint(f"{vhtfm.local.site}: vhtfm.conf.disable_scheduler is SET")
		return True

	scheduler_disabled = not vhtfm.utils.cint(
		vhtfm.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{vhtfm.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	vhtfm.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	since = (vhtfm.get_system_settings("dormant_days") or 4) * 86400
...
def _get_last_creation_timestamp(doctype):
	timestamp = vhtfm.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
...
def activate_scheduler():
	...
	if vhtfm.local.conf.maintenance_mode:
		vhtfm.throw(vhtfm._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if vhtfm.conf.pause_scheduler:

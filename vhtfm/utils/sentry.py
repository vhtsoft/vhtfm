...
from vhtfm.database.database import Database, EmptyQueryValues


class VhtfmIntegration(Integration):
	identifier = "vhtfm"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = vhtfm._dict({"scheduled": False, "wait": 0})
		...
		if vhtfm.form_dict.cmd:
			path = f"/api/method/{vhtfm.form_dict.cmd}"
		else:
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if vhtfm.request:
				evt_processor = _make_wsgi_event_processor(vhtfm.request.environ, False)
				scope.add_event_processor(evt_processor)
				if vhtfm.request.is_json:
					scope.set_context("JSON Body", vhtfm.request.json)
				elif vhtfm.request.form:
					scope.set_context("Form Data", vhtfm.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], vhtfm.ValidationError | vhtfm.PermissionError):
					return
		...
		vhtfm.logger().error("Failed to capture exception", exc_info=True)

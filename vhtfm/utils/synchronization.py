...
import vhtfm
from vhtfm import _
from vhtfm.utils import get_fox_path, get_site_path
from vhtfm.utils.file_lock import LockTimeoutError
...
def filelock(lock_name: str, *, timeout=30, is_global=False):
	...
	args:
	        ...
	        is_global: if set lock is global to fox

	Lock file location:
	        global - {fox_dir}/config/{name}.lock
	        site - {fox_dir}/sites/sitename/{name}.lock
	...
	if not is_global:
		...
		lock_path = os.path.abspath(os.path.join(get_fox_path(), "config", lock_filename))

	try:
		with _StrongFileLock(lock_path, timeout=timeout):
			yield
	except Timeout as e:
		vhtfm.log_error(f"Filelock: Failed to aquire {lock_path}")

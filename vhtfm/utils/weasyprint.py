...
def download_pdf(doctype, name, print_format, letterhead=None):
	...
	vhtfm.local.response.filecontent = pdf
...
class PrintFormatGenerator:
	...
	def __init__(self, print_format, doc, letterhead=None):
		...
		"""
		self.base_url = vhtfm.utils.get_url()
		self.print_format = vhtfm.get_doc("Print Format", print_format)
		self.doc = doc

		if letterhead == _("No Letterhead"):
			letterhead = None
		self.letterhead = vhtfm.get_doc("Letter Head", letterhead) if letterhead else None
	...
	def build_context(self):
		self.print_settings = vhtfm.get_doc("Print Settings")
		...
		print_style = (
			vhtfm.get_doc("Print Style", self.print_settings.print_style)
	...
	def get_main_html(self):
		self.context.css = vhtfm.render_template("templates/print_format/print_format.css", self.context)
		return vhtfm.render_template("templates/print_format/print_format.html", self.context)

	def get_header_footer_html(self):
		header_html = footer_html = None
		if self.letterhead:
			header_html = vhtfm.render_template("templates/print_format/print_header.html", self.context)
		if self.letterhead:
			footer_html = vhtfm.render_template("templates/print_format/print_footer.html", self.context)
	...
	def get_layout(self, print_format):
		layout = vhtfm.parse_json(print_format.format_data)
	...
	def process_margin_texts(self, layout):
		...
		for key in margin_texts:
			...
			if text and "{{" in text:
				layout["text_" + key] = vhtfm.render_template(text, self.context)

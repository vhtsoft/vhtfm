"""
VhtfmClient is a library that helps you connect with other vhtfm systems
"""
...
class VhtfmException(Exception):
	pass
...
class VhtfmClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		vhtfm_authorization_source=None,
	):
		import requests
		...
		self.vhtfm_authorization_source = vhtfm_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.vhtfm_authorization_source:
				auth_source = {"Vhtfm-Authorization-Source": self.vhtfm_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": vhtfm.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return vhtfm._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "vhtfm.client.insert_many", "docs": vhtfm.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": vhtfm.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return vhtfm._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "vhtfm.client.bulk_update", "docs": vhtfm.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "vhtfm.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "vhtfm.client.submit", "doc": vhtfm.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "vhtfm.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": vhtfm.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "vhtfm.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not vhtfm.db.exists("User", doc.get("owner")):
				vhtfm.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = vhtfm.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "VhtfmClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise VhtfmException(exc)
		if "message" in rjson:
...
class VhtfmOAuth2Client(VhtfmClient):

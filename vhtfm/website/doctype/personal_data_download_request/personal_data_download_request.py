...
class PersonalDataDownloadRequest(Document):
	...
	def generate_file_and_send_mail(self, personal_data):
		...
		file_link = (
			vhtfm.utils.get_url("/api/method/vhtfm.utils.file_manager.download_file")
			+ "?"
			+ get_signed_params({"file_url": f.file_url})
		)
		host_name = vhtfm.local.site
		vhtfm.sendmail(
...
def get_user_data(user):
	...
	hooks = vhtfm.get_hooks("user_data_fields")
	data = {}
	for hook in hooks:
		d = data.get(hook.get("doctype"), [])
		d += vhtfm.get_all(hook.get("doctype"), {hook.get("filter_by", "owner"): user}, ["*"])
		if d:

...
from vhtfm.website.doctype.personal_data_download_request.personal_data_download_request import (
	get_user_data,
)
...
class TestRequestPersonalData(IntegrationTestCase):
	...
	def test_file_and_email_creation(self):
		vhtfm.set_user("test_privacy@example.com")
		download_request = vhtfm.get_doc(
		...
		email_queue = vhtfm.get_all("Email Queue", fields=["message"], order_by="creation DESC", limit=1)
		self.assertIn(vhtfm._("Download Your Data"), email_queue[0].message)

		vhtfm.db.delete("Email Queue")


def create_user_if_not_exists(email, first_name=None):
	vhtfm.delete_doc_if_exists("User", email)
	...
	user = vhtfm.get_doc(
		{
			...
			"birth_date": vhtfm.utils.now_datetime(),
		}

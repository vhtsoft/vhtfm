...
vhtfm.ui.form.on("Web Page", {
	layout: function (frm) {
		if (frm.is_new()) {
			if (frm.doc.insert_code) {
				if (!frm.doc.javascript) {
					frm.set_value("javascript", `vhtfm.ready(() => {\n\t\n});`);
				}
			}
		}
	},
	...
	validate: function (frm) {
		if (frm.doc.title && !frm.doc.route) {
			frm.set_value("route", vhtfm.scrub(frm.doc.title, "-"));
		}
	},
	...
	published: function (frm) {
		...
		if (!frm.doc.published && frm.doc.end_date) {
			var end_date = vhtfm.datetime.str_to_obj(vhtfm.datetime.now_datetime());
	...
	set_meta_tags(frm) {
		vhtfm.utils.set_meta_tag(frm.doc.route);
	},
});

vhtfm.ui.form.on("Web Page Block", {
	edit_values(frm, cdt, cdn) {
		...
		open_web_template_values_editor(row.web_template, values).then((new_values) => {
			vhtfm.model.set_value(cdt, cdn, "web_template_values", JSON.stringify(new_values));
		});
	},
});
	...
	},
	{
		...
		description: `${__("You can select one from the following,")} <br>
					<ul>
						...
						<li><b>${__("Page Builder")}</b>: ${__("Vhtfm page builder using components")}</li>
					</ul>
					`,
	},
	{

...
class WebPageView(Document):
	...
	def clear_old_logs(days=180):
		...
		table = vhtfm.qb.DocType("Web Page View")
...
def make_view_log(
	...
	# real path
	path = vhtfm.request.headers.get("Referer")

	if not vhtfm.utils.is_site_link(path):
		return

	path = urlparse(path).path

	request_dict = vhtfm.request.__dict__
	...
	is_unique = visitor_id and not bool(vhtfm.db.exists("Web Page View", {"visitor_id": visitor_id}))

	view = vhtfm.new_doc("Web Page View")
	view.path = path
...
def get_page_view_count(path):
	return vhtfm.db.count("Web Page View", filters={"path": path})


def is_tracking_enabled():
	return vhtfm.db.get_single_value("Website Settings", "enable_view_tracking")

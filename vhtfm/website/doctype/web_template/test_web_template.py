...
class TestWebTemplate(IntegrationTestCase):
	def test_render_web_template_with_values(self):
		doc = vhtfm.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		vhtfm.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not vhtfm.db.exists("Web Page", "test-web-template"):
			vhtfm.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not vhtfm.db.exists("Website Theme", "Custom"):
			theme = vhtfm.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = vhtfm.get_doc("Website Theme", "Custom")
		return theme

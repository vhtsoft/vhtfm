...
class WebsiteSlideshow(Document):
	...
	if TYPE_CHECKING:
		...
		from vhtfm.website.doctype.website_slideshow_item.website_slideshow_item import (
	...
	def validate_images(self):
		...
		if files:
			result = vhtfm.get_all("File", filters={"file_url": ("in", list(files))}, fields="is_private")
			if any(file.is_private for file in result):
				vhtfm.throw(_("All Images attached to Website Slideshow should be public"))
...
def get_slideshow(doc):
	...
	slideshow = vhtfm.get_doc("Website Slideshow", doc.slideshow)

...
vhtfm.provide("website");
vhtfm.provide("vhtfm.awesome_bar_path");
window.cur_frm = null;
...
$.extend(vhtfm, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => vhtfm.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = vhtfm.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (vhtfm._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				vhtfm._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					vhtfm.msgprint(__("Not found"));
				},
				403: function () {
					vhtfm.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			vhtfm.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				vhtfm.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return vhtfm.call({
			type: "GET",
			method: "vhtfm.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (vhtfm.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", vhtfm.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				vhtfm.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				vhtfm.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				vhtfm.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!vhtfm.freeze_count) return; // anything open?
		vhtfm.freeze_count--;
		if (!vhtfm.freeze_count) {
	...
	trigger_ready: function () {
		vhtfm.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = vhtfm.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		vhtfm.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				vhtfm.do_search(val);
	...
	do_search: function (val) {
		var path =
			(vhtfm.awesome_bar_path && vhtfm.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		vhtfm.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return vhtfm.get_cookie("user_id") !== "Guest" && vhtfm.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (vhtfm.session.user === "Guest" && window.show_language_picker) {
			vhtfm
				.call("vhtfm.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = vhtfm.get_cookie("preferred_language");
					let language_codes = [];
...
vhtfm.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		vhtfm.utils.debounce(() => {
			...
			vhtfm
				.call({
					method: "vhtfm.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from vhtfm/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = vhtfm.get_cookie("full_name");
	var logged_in = vhtfm.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	vhtfm.bind_navbar_search();

	// add apps link
	let apps = vhtfm.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = vhtfm.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && vhtfm.add_apps(obj);
	}

	// switch to app link
	if (vhtfm.get_cookie("system_user") === "yes" && logged_in) {
		vhtfm.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(vhtfm, vhtfm.get_cookies());
	vhtfm.session = { user: vhtfm.user_id };

	vhtfm.datetime.refresh_when();
	vhtfm.trigger_ready();
	vhtfm.bind_filters();
	vhtfm.highlight_code_blocks();
	vhtfm.add_link_to_headings();
	vhtfm.make_navbar_active();
	// scroll to hash
...
vhtfm.ready(function () {
	vhtfm.show_language_picker();
	vhtfm.setup_videos();
	vhtfm.realtime.init(window.socketio_port, true); // lazy connection
});

...
class DocumentPage(BaseTemplatePage):
	...
	def search_in_doctypes_with_web_view(self):
		if document := _find_matching_document_webview(self.path):
			...
			doc = vhtfm.get_cached_doc(self.doctype, self.docname)
			return doc.meta.allow_guest_to_view or doc.has_permission() or vhtfm.has_website_permission(doc)
	...
	@cache_html
	def get_html(self):
		self.doc = vhtfm.get_cached_doc(self.doctype, self.docname)
		self.init_context()
		self.update_context()
		self.post_process_context()
		return vhtfm.get_template(self.template_path).render(self.context)
...
def _find_matching_document_webview(route: str) -> tuple[str, str] | None:
	for doctype in get_doctypes_with_web_view():
		...
		try:
			docname = None
			if meta.is_virtual:
				if doclist := vhtfm.get_all(doctype, filters=filters, fields=["name"], limit=1):
					docname = doclist[0].get("name")
			else:
				docname = vhtfm.db.get_value(doctype, filters, "name")

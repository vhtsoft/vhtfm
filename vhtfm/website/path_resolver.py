...
import vhtfm
from vhtfm.website.page_renderers.document_page import DocumentPage
from vhtfm.website.page_renderers.list_page import ListPage
from vhtfm.website.page_renderers.not_found_page import NotFoundPage
from vhtfm.website.page_renderers.print_page import PrintPage
...
from vhtfm.website.page_renderers.web_form import WebFormPage
from vhtfm.website.router import evaluate_dynamic_routes
from vhtfm.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		if hasattr(vhtfm.local, "request"):
			request = vhtfm.local.request or request

		# check if the request url is in 404 list
		if request.url and can_cache() and vhtfm.cache.hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except vhtfm.Redirect as e:
			return vhtfm.flags.redirect_location, RedirectPage(self.path, e.http_status_code)

		if vhtfm.get_hooks("website_path_resolver"):
			for handler in vhtfm.get_hooks("website_path_resolver"):
				endpoint = vhtfm.get_attr(handler)(self.path)
		else:
			try:
				endpoint = resolve_path(self.path)
			except werkzeug.routing.exceptions.RequestRedirect as e:
				vhtfm.flags.redirect_location = e.new_url
				return vhtfm.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in vhtfm.get_hooks("page_renderer") or []:
			try:
				renderer = vhtfm.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	"""
	redirects = vhtfm.get_hooks("website_redirects")
	redirects += vhtfm.get_all(
	...
	redirect_to = vhtfm.cache.hget("website_redirects", path)

	if redirect_to:
		if isinstance(redirect_to, dict):
			vhtfm.flags.redirect_location = redirect_to["path"]
			raise vhtfm.Redirect(redirect_to["status_code"])
		vhtfm.flags.redirect_location = redirect_to
	...
	for rule in redirects:
		...
		if query_string and rule.get("match_with_query_string"):
			path_to_match = path + "?" + vhtfm.safe_decode(query_string)

		try:
			match = re.match(pattern, path_to_match)
		except re.error:
			vhtfm.log_error("Broken Redirect: " + pattern)
		...
		if match:
			...
			raise vhtfm.Redirect(status_code)
...
def get_website_rules():
	...
	def _get():
		rules = vhtfm.get_hooks("website_route_rules")
		for d in vhtfm.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return vhtfm.cache.get_value("website_route_rules", _get)


def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		vhtfm.throw(vhtfm._("Path {0} it not a valid path").format(vhtfm.bold(path)))

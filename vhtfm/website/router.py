...
import vhtfm
from vhtfm.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from vhtfm.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from vhtfm.website.doctype.web_form.web_form import get_published_web_forms
	...
	for d in get_published_web_forms():
		...
		if end_point:
			if path.endswith("/list"):
				vhtfm.form_dict.is_list = True
			elif path.endswith("/new"):
				vhtfm.form_dict.is_new = True
			elif path.endswith("/edit"):
				vhtfm.form_dict.is_edit = True
			else:
				vhtfm.form_dict.is_read = True
			return d
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(vhtfm.local, "request") and vhtfm.local.request.environ:
		urls = route_map.bind_to_environ(vhtfm.local.request.environ)
		try:
			endpoint, args = urls.match("/" + path)
			if args:
				# don't cache when there's a query string!
				vhtfm.local.no_cache = 1
				vhtfm.local.form_dict.update(args)
		except NotFound:
			pass
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = vhtfm.local.flags.web_pages_apps or vhtfm.get_installed_apps()
	...
	return vhtfm.cache.get_value("website_pages", lambda: _build(app))
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = vhtfm.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = vhtfm.local.request.path

	base_template_map = vhtfm.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = vhtfm.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = vhtfm.get_hooks("website_generators")
		doctypes_with_web_view = vhtfm.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = vhtfm.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(vhtfm.scrub(d.module)) in installed_apps
		]
		return doctypes

	return vhtfm.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return vhtfm.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from vhtfm.website.doctype.web_form.web_form import get_published_web_forms
	from vhtfm.website.doctype.web_page.web_page import get_dynamic_web_pages
	from vhtfm.website.page_renderers.document_page import _find_matching_document_webview
	from vhtfm.www.sitemap import get_public_pages_from_doctypes
	...
	vhtfm.cache.delete_value("home_page")

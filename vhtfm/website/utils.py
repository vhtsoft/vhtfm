...
def delete_page_cache(path):
	...
	if path:
		vhtfm.cache.hdel_names(groups, path)
		vhtfm.cache.delete_value("full_index")
	else:
		groups.append("full_index")
		vhtfm.cache.delete_value(groups)
...
def can_cache(no_cache=False):
	if vhtfm.flags.force_website_cache:
		return True
	if vhtfm.conf.disable_website_cache or vhtfm.conf.developer_mode:
		return False
	if getattr(vhtfm.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = vhtfm.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", vhtfm.session.user], ["published", "=", 1]],
	)

	communications = vhtfm.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{vhtfm.session.user}%"],
			["cc", "like", f"%{vhtfm.session.user}%"],
			["bcc", "like", f"%{vhtfm.session.user}%"],
		],
	)
...
def get_home_page():
	if vhtfm.local.flags.home_page and not vhtfm.flags.in_test:
		return vhtfm.local.flags.home_page
	...
	def _get_home_page():
		...
		if vhtfm.session.user != "Guest":
			# by role
			for role in vhtfm.get_roles():
				home_page = vhtfm.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = vhtfm.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = vhtfm.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if vhtfm.session.user == "Guest" else "me"

		home_page = home_page.strip("/")

		return home_page

	if vhtfm.local.dev_server:
		# dont return cached homepage in development
		return _get_home_page()

	return vhtfm.cache.hget("home_page", vhtfm.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = vhtfm.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = vhtfm.get_attr(home_page_method[-1])(vhtfm.session.user)
	elif vhtfm.get_hooks("website_user_home_page"):
		home_page = vhtfm.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = vhtfm.get_hooks("role_home_page")
		if role_home_page:
			for role in vhtfm.get_roles():
	...
	if not home_page:
		home_page = vhtfm.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(vhtfm.get_system_settings("float_precision")) or 3,
			"date_format": get_date_format(),
			"time_format": get_time_format(),
			"first_day_of_the_week": get_first_day_of_the_week(),
			"number_format": get_number_format().string,
			"currency": vhtfm.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": vhtfm.db.get_value("User", vhtfm.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": vhtfm.local.site,
	}


def is_signup_disabled():
	return vhtfm.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return vhtfm.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from vhtfm.website.router import get_pages

	if not vhtfm.local.flags.children_map:
		...
		children_map = vhtfm.cache.get_value("website_full_index", _build)

		vhtfm.local.flags.children_map = children_map

	return vhtfm.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		vhtfm.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		vhtfm.clear_cache("Guest")
		keys += [
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	vhtfm.cache.delete_value(keys)

	for method in vhtfm.get_hooks("website_clear_cache"):
		vhtfm.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = vhtfm.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else vhtfm.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = vhtfm.cache.hget("portal_menu_items", vhtfm.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = vhtfm.get_roles()
		portal_settings = vhtfm.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = vhtfm.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		vhtfm.cache.hset("portal_menu_items", vhtfm.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			vhtfm.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = vhtfm.cache.hget("website_page", args[0].path)
			if page_cache and vhtfm.local.lang in page_cache:
				html = page_cache[vhtfm.local.lang]
			if html:
				vhtfm.local.response.from_cache = True
				vhtfm.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = vhtfm.cache.hget("website_page", args[0].path) or {}
			page_cache[vhtfm.local.lang] = html
			vhtfm.cache.hset("website_page", args[0].path, page_cache)
			vhtfm.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = vhtfm.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in vhtfm.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in vhtfm.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in vhtfm.local.preload_assets["icons"]
	)

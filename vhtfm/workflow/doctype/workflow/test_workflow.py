...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("vhtfm.attach_print", return_value={})
		self.patcher.start()
		vhtfm.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		vhtfm.set_user("Administrator")
		self.patcher.stop()
		vhtfm.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = vhtfm.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from vhtfm.tests.ui_test_helpers import UI_TEST_USER

	if vhtfm.db.exists("Workflow", "Test ToDo"):
		vhtfm.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not vhtfm.db.exists("Role", TEST_ROLE):
		vhtfm.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if vhtfm.db.exists("User", UI_TEST_USER):
			vhtfm.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = vhtfm.new_doc("Workflow")
...
def create_new_todo():
	return vhtfm.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()

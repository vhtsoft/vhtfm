vhtfm.provide("vhtfm.core");

vhtfm.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		let message = `
		<div class="flex">
			<div class="mr-3"><img style="border-radius: var(--border-radius-md)" width="600" src="/assets/vhtfm/images/workflow-builder.gif"></div>
			<div>
				...
				<div>
					<a class="btn btn-primary btn-sm" href="${workflow_builder_url}">
						${__("Workflow Builder")} ${vhtfm.utils.icon("right", "xs")}
					</a>
				</div>
			</div>
		</div>
		...
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.document_type]), () => {
				vhtfm.set_route("List", frm.doc.document_type);
			});
		}
	...
	update_field_options: function (frm) {
		...
		vhtfm.model.with_doctype(doc.document_type, () => {
			const fieldnames = vhtfm
				.get_meta(doc.document_type)
				.fields.filter((field) => !vhtfm.model.no_value_type.includes(field.fieldtype))
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return vhtfm.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return vhtfm
			.xcall("vhtfm.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					vhtfm.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

vhtfm.ui.form.on("Workflow Document State", {
...
vhtfm.ui.form.on("Workflow Transition", {
	state: function (_, cdt, cdn) {
		var row = locals[cdt][cdn];
		delete row.workflow_builder_id;
	},

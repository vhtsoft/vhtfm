...
import vhtfm
from vhtfm import _
from vhtfm.desk.form.utils import get_pdf_link
from vhtfm.desk.notifications import clear_doctype_notifications
from vhtfm.email.doctype.email_template.email_template import get_email_template
from vhtfm.model.document import Document
from vhtfm.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from vhtfm.utils import get_datetime, get_url
from vhtfm.utils.background_jobs import enqueue
from vhtfm.utils.data import get_link_to_form
from vhtfm.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from vhtfm.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	vhtfm.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(vhtfm.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = vhtfm.session.user
...
def return_success_page(doc):
	vhtfm.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), vhtfm.bold(doc.get("name")), vhtfm.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	vhtfm.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			vhtfm.bold(doc.get("name")),
			vhtfm.bold(doc_workflow_state),
			vhtfm.bold(vhtfm.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else vhtfm.session.user

	allowed_roles = vhtfm.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(vhtfm.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		vhtfm.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = vhtfm._dict(
					{
						"possible_actions": [],
						"email": vhtfm.db.get_value("User", user, "email"),
					}
				)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/vhtfm.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/vhtfm.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		vhtfm.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = vhtfm.db.get_value(

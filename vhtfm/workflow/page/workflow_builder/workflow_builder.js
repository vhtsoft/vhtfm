vhtfm.pages["workflow-builder"].on_page_load = function (wrapper) {
	...
	if (vhtfm.boot.developer_mode) {
		...
		vhtfm.hot_update.push(() => load_workflow_builder(wrapper));
	}
};

vhtfm.pages["workflow-builder"].on_page_show = function (wrapper) {
...
function load_workflow_builder(wrapper) {
	...
	if (route.length > 1) {
		vhtfm.require("workflow_builder.bundle.js").then(() => {
			vhtfm.workflow_builder = new vhtfm.ui.WorkflowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new vhtfm.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					vhtfm.set_route("workflow-builder", workflow);
					...
					vhtfm.db
						...
						.then((doc) => {
							vhtfm.set_route("workflow-builder", doc.name);
						})
						.finally(() => {

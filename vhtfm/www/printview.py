...
import vhtfm
from vhtfm import _, cstr, get_module_path
from vhtfm.core.doctype.access_log.access_log import make_access_log
from vhtfm.core.doctype.document_share_key.document_share_key import is_expired
from vhtfm.utils import cint, escape_html, strip_html
...
if TYPE_CHECKING:
	...
	from vhtfm.printing.doctype.print_format.print_format import PrintFormat
	from vhtfm.printing.doctype.print_settings.print_settings import PrintSettings
...
def get_context(context) -> PrintContext:
	...
	if not ((vhtfm.form_dict.doctype and vhtfm.form_dict.name) or vhtfm.form_dict.doc):
		return PrintContext(
			print_style="",
			comment="",
			title="Error",
...
<pre>{escape_html(vhtfm.as_json(vhtfm.form_dict, indent=2))}</pre>
""",
		)

	if vhtfm.form_dict.doc:
		doc = vhtfm.form_dict.doc
	else:
		doc = vhtfm.get_doc(vhtfm.form_dict.doctype, vhtfm.form_dict.name)

	set_link_titles(doc)

	settings = vhtfm.parse_json(vhtfm.form_dict.settings)

	letterhead = vhtfm.form_dict.letterhead or None
	...
	make_access_log(
		doctype=vhtfm.form_dict.doctype, document=vhtfm.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=vhtfm.form_dict.trigger_print,
		no_letterhead=vhtfm.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(vhtfm.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": vhtfm.session.user,
		"title": vhtfm.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": vhtfm.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": vhtfm.form_dict.doctype,
		"name": vhtfm.form_dict.name,
		"key": vhtfm.form_dict.get("key"),
...
def get_print_format_doc(print_format_name: str, meta: "Meta") -> Optional["PrintFormat"]:
	...
	if not print_format_name:
		print_format_name = vhtfm.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return vhtfm.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = vhtfm.get_single("Print Settings").as_dict()
	...
	if not vhtfm.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			vhtfm.throw(_("Not allowed to print draft documents"), vhtfm.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			vhtfm.throw(_("Not allowed to print cancelled documents"), vhtfm.PermissionError)
	...
	if print_format:
		...
		template = None
		if hook_func := vhtfm.get_hooks("get_print_format_template"):
			template = vhtfm.get_attr(hook_func[-1])(jenv=jenv, print_format=print_format)
	...
	letter_head = vhtfm._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = vhtfm.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = vhtfm.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	hook_func = vhtfm.get_hooks("pdf_body_html")
	html = vhtfm.get_attr(hook_func[-1])(jenv=jenv, template=template, print_format=print_format, args=args)
...
def set_title_values_for_link_and_dynamic_link_fields(
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = vhtfm.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta: "Meta", doc: "Document") -> None:
	for field in meta.get_table_fields():
		...
		_meta = vhtfm.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, vhtfm.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = vhtfm.get_doc(doc, name)
	else:
		document = vhtfm.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=vhtfm.parse_json(settings),
		)
	except vhtfm.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None) -> dict:
	...
	if not print_format or (print_format and not print_format.raw_printing):
		vhtfm.throw(
			_("{0} is not a raw printing format.").format(print_format), vhtfm.TemplateNotFoundError
		)
...
def validate_print_permission(doc: "Document") -> None:
	for ptype in ("read", "print"):
		if vhtfm.has_permission(doc.doctype, ptype, doc) or vhtfm.has_website_permission(doc):
			return

	key = vhtfm.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise vhtfm.PermissionError(_("You do not have permission to view this document"))


def validate_key(key: str, doc: "Document") -> None:
	document_key_expiry = vhtfm.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise vhtfm.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if vhtfm.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise vhtfm.exceptions.InvalidKeyError
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
	if print_format.disabled:
		vhtfm.throw(_("Print Format {0} is disabled").format(print_format.name), vhtfm.DoesNotExistError)

	# server, find template
	module = print_format.module or vhtfm.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
		vhtfm.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		vhtfm.throw(_("No template found at path: {0}").format(path), vhtfm.TemplateNotFoundError)
...
def make_layout(doc: "Document", meta: "Meta", format_data=None) -> list:
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/vhtfm/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = vhtfm.get_template("templates/styles/standard.css").render(context)

	if style and vhtfm.db.exists("Print Style", style):
		css = css + "\n" + vhtfm.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data: list, table_meta: "Meta", df: "DocField") -> list["DocField"]:
	...
	columns = []
	doc = data[0] or vhtfm.new_doc(df.options)

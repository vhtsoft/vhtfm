import vhtfm
from vhtfm.email.doctype.newsletter.newsletter import confirmed_unsubscribe
...
def get_context(context):
	...
	if "email" in vhtfm.form_dict and vhtfm.request.method == "GET":
		if verify_request():
			user_email = vhtfm.form_dict["email"]
			context.email = user_email
			title = vhtfm.form_dict.get("name")
	...
	elif "user_email" in vhtfm.form_dict and vhtfm.request.method == "POST":
		context.status = "unsubscribed"
		email = vhtfm.form_dict["user_email"]
		email_group = get_email_groups(email)
		for group in email_group:
			if group.email_group in vhtfm.form_dict:
